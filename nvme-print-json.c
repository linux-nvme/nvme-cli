...
#define obj_add_format json_object_add_format
...
static void json_nvme_id_ns_lbaf(struct nvme_id_ns *ns, int i, struct json_object *lbafs)
{
	...
	if (verbose_mode()) {
		...
		obj_add_format(lbaf, "Metadata Size", "%d bytes", le16_to_cpu(ns->lbaf[i].ms));
		obj_add_format(lbaf, "Data Size", "%d bytes", 1 << ns->lbaf[i].ds);
		obj_add_format(lbaf, "Relative Performance", "0x%x %s", ns->lbaf[i].rp,
