...
static int send_rpmb_req(struct dev_handle *hnd, unsigned char tgt, int size,
...
static int recv_rpmb_rsp(struct dev_handle *hnd, int tgt, int size,
 ...
 */
static struct rpmb_data_frame_t *
rpmb_read_request(struct dev_handle *hnd,
		  struct rpmb_data_frame_t *req,
	          int req_size,
	          int rsp_size)
{
	...
	error = send_rpmb_req(hnd, req->target, req_size, req);
	if (error != 0) {
	...
	error = recv_rpmb_rsp(hnd, req->target, rsp_size, rsp);
	if (error) {
...
static int rpmb_read_write_counter(struct dev_handle *hnd,
				   unsigned char target,
				   unsigned int *counter)
{
	...
	if ((rsp = rpmb_read_request(hnd, req, req_size, req_size)) == NULL) {
		goto out;
	}	
 ...
 * returns 0
 */
static unsigned int rpmb_read_config_block(struct dev_handle *hnd, unsigned char **config_buf)
{
	int req_size = sizeof(struct rpmb_data_frame_t);
...
static int rpmb_auth_data_read(struct dev_handle *hnd, unsigned char target,
			       unsigned int offset,
				 ...
				 unsigned char *keybuf, int keysize)
{
	...
	error = send_rpmb_req(hnd, tgt, req_size, req);
	if (error != 0) {
	...
	error = send_rpmb_req(hnd, tgt, rsp_size, rsp);
	...
	/* Read final response */
	memset(rsp, 0, rsp_size);
	error = recv_rpmb_rsp(hnd, tgt, rsp_size, rsp);
	if (error != 0)
...
/* send the request and get response */
static int rpmb_auth_data_write(struct dev_handle *hnd, unsigned char target,
				...
				unsigned char *keybuf, int keysize)
{
	...
	while (xfer > 0 ) {
		if (auth_data_write_chunk(hnd, target, (addr + offset / 512),
				          msg_buf + offset, xfer,
				          keybuf, keysize) != 0)
		{
...
static int rpmb_write_config_block(struct dev_handle *hnd, unsigned char *cfg_buf,
				   unsigned char *keybuf, int keysize)
{
	int cfg_size = sizeof(struct rpmb_config_block_t);
	int rsp_size = sizeof(struct rpmb_data_frame_t);
	...
	write_cntr = rpmb_read_config_block(hnd, &cfg_buf_read);
	...
	error = send_rpmb_req(hnd, 0, req_size, req);
	if (error != 0) {
