...
static int send_rpmb_req(struct dev_handle *hdl, unsigned char tgt, int size,
...
static int recv_rpmb_rsp(struct dev_handle *hdl, int tgt, int size,
 ...
 */
static struct rpmb_data_frame_t *
rpmb_read_request(struct dev_handle *hdl,
		  struct rpmb_data_frame_t *req,
	          int req_size,
	          int rsp_size)
{
...
static int rpmb_read_write_counter(struct dev_handle *hdl,
				   unsigned char target,
 ...
 * returns 0
 */
static unsigned int rpmb_read_config_block(struct dev_handle *hdl, unsigned char **config_buf)
{
	int req_size = sizeof(struct rpmb_data_frame_t);
...
static int rpmb_auth_data_read(struct dev_handle *hdl, unsigned char target,
			       unsigned int offset,
...
static int rpmb_program_auth_key(struct dev_handle *hdl, unsigned char target,
 ...
 * on failures.
 */
static int auth_data_write_chunk(struct dev_handle *hdl, unsigned char tgt, unsigned int addr,
...
/* send the request and get response */
static int rpmb_auth_data_write(struct dev_handle *hdl, unsigned char target,
...
static int rpmb_write_config_block(struct dev_handle *hdl, unsigned char *cfg_buf,
				   unsigned char *keybuf, int keysize)
{
	int cfg_size = sizeof(struct rpmb_config_block_t);
	int rsp_size = sizeof(struct rpmb_data_frame_t);
...
int rpmb_cmd_option(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	switch (cfg.opt) {
		case RPMB_REQ_READ_WRITE_CNTR:
			err = rpmb_read_write_counter(dev_hdl(dev), cfg.target,
						      &write_cntr);
			if (err == 0)
		...
		case RPMB_REQ_AUTH_DCB_READ:
			write_cntr = rpmb_read_config_block(dev_hdl(dev),
							    &msg_buf);
			if (msg_buf == NULL) {
		...
		case RPMB_REQ_AUTH_DATA_READ:
			...
			err = rpmb_auth_data_read(dev_hdl(dev), cfg.target,
						  cfg.address, &msg_buf,
						  cfg.blocks,
						  (regs.access_size + 1));
		...
		case RPMB_REQ_AUTH_DATA_WRITE:
			...
			err = rpmb_auth_data_write(dev_hdl(dev), cfg.target,
						   cfg.address,
		...
		case RPMB_REQ_AUTH_DCB_WRITE:
			err = rpmb_write_config_block(dev_hdl(dev), msg_buf,
						      key_buf, key_size);
			break;
	
		case RPMB_REQ_AUTH_KEY_PROGRAM:
			err = rpmb_program_auth_key(dev_hdl(dev), cfg.target,
						    key_buf, key_size);
			break;
		default:
			break;
