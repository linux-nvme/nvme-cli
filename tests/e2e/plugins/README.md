...
└── TestNVMe (tests/nvme_test.py)
    └── TestPlugin (tests/e2e/plugins/plugin_test.py)
        └── Test<Plugin> (tests/e2e/plugins/<plugin>/<plugin>_test.py)
```
...
1. `e2e-tests=true` (gates the entire `tests/e2e/` subtree)
...
1. Create a file in the appropriate plugin directory (e.g.,
   `tests/e2e/plugins/ocp/ocp_smart_add_log_test.py`).
...
2. Add a conditional `subdir()` in `tests/e2e/plugins/meson.build`.


3. Create the directory structure:

```
tests/e2e/plugins/<plugin>/
├── meson.build
├── <plugin>_test.py
