...
#define _NVME_IEEE_STR_MAX_LEN			7
/* The IEEE is hex string for 24 bites number. */
...
struct nvme_psd {
	struct nvme_spec_psd raw_data;
};

struct nvme_ctrl {
	const char *dev_path;
	struct _nvme_ctrl_strs strs;
	struct nvme_id_ctrl raw_data;
	struct nvme_psd **psds;
};

static int _nvme_psds_new(struct nvme_ctrl *cnt, char *err_msg);
static int _init_strs(struct nvme_ctrl *cnt, char *err_msg);
static uint8_t *_nvme_ctrl_ieee_get(struct nvme_ctrl *cnt);
...
_getter_func_gen_str(nvme_ctrl, ieee);
...
_getter_func_gen_uint16_t(nvme_ctrl, rrls);
_getter_func_gen_uint8_t(nvme_ctrl, cntrltype);
_getter_func_gen_str(nvme_ctrl, fguid);
_getter_func_gen_uint16_t(nvme_ctrl, crdt1);
_getter_func_gen_uint16_t(nvme_ctrl, crdt2);
_getter_func_gen_uint16_t(nvme_ctrl, crdt3);
...
_getter_func_gen_uint32_t(nvme_ctrl, hmminds);
_getter_func_gen_uint16_t(nvme_ctrl, hmmaxd);
_getter_func_gen_uint16_t(nvme_ctrl, nsetidmax);
_getter_func_gen_uint16_t(nvme_ctrl, endgidmax);
_getter_func_gen_uint8_t(nvme_ctrl, anatt);
_getter_func_gen_uint8_t(nvme_ctrl, anacap);
_getter_func_gen_uint32_t(nvme_ctrl, anagrpmax);
_getter_func_gen_uint32_t(nvme_ctrl, nanagrpid);
_getter_func_gen_uint32_t(nvme_ctrl, pels);
			 ...
			 char *err_msg)
{
	...
	*cnt = (struct nvme_ctrl *) calloc(1, sizeof(struct nvme_ctrl));
	...
	_good(_init_strs(*cnt, err_msg), rc, out);
...
static int _init_strs(struct nvme_ctrl *cnt, char *err_msg)
{
	...
	ver = (struct nvme_spec_ver *) &(cnt->raw_data.ver);
	...
	ieee_str = (char *) calloc(_NVME_VER_STR_MAX_LEN, sizeof(char));
	_alloc_null_check(err_msg, ieee_str, rc, out);
	cnt->strs.ieee = ieee_str;

	ieee = _nvme_ctrl_ieee_get(cnt);

	snprintf(ieee_str, _NVME_IEEE_STR_MAX_LEN,
		 "%02x%02x%02x", ieee[2], ieee[1], ieee[0]);
