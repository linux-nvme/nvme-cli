...
/*
 * FreeBSD's nvme(4) driver implements only four of the "Linux
 * compatible" NVMe ioctls the Linux kernel has: NVME_IOCTL_ID,
 * NVME_IOCTL_ADMIN_CMD, NVME_IOCTL_IO_CMD, NVME_IOCTL_RESET (see
 * sys/dev/nvme/nvme_linux.h in the FreeBSD source), so this mirrors
 * ioctl-linux.c only for those four. What FreeBSD doesn't have:
 *
 *   - A subsystem-level reset or a namespace-rescan ioctl -- neither
 *     concept exists in the driver, so libnvme_reset_subsystem() and
 *     libnvme_rescan_ns() below return -ENOTSUP instead of issuing an
 *     ioctl the driver would never recognize.
 *   - The 64-bit passthru ioctls: only the 32-bit struct is supported,
 *     which the existing ioctl32/64 probing in
 *     ioctl_admin_passthru()/ioctl_io_passthru() already falls back
 *     away from via -ENOTTY.
 *   - Linux's BLKBSZSET/BLKRRPART block-device ioctls: the driver
 *     already determines the block size at open time, so there is
 *     nothing for libnvme_update_block_size() to set.
 */
