...
LIBNVME_PUBLIC __u8 nvme_status_to_errno(int status, bool fabrics)
{
	__u16 sc;
...
LIBNVME_PUBLIC const char *nvme_status_to_string(int status, bool fabrics)
{
...
LIBNVME_PUBLIC const char *nvme_errno_to_string(int status)
{
...
LIBNVME_PUBLIC const char *nvme_strerror(int errnum)
{
...
LIBNVME_PUBLIC struct nvmf_ext_attr *nvmf_exat_ptr_next(struct nvmf_ext_attr *p)
{
	return (struct nvmf_ext_attr *)
		((uintptr_t)p + (ptrdiff_t)nvmf_exat_size(le16_to_cpu(p->exatlen)));
}

LIBNVME_PUBLIC const char *nvme_get_version(enum nvme_version type)
{
	switch(type) {
...
LIBNVME_PUBLIC int nvme_uuid_to_string(
...
LIBNVME_PUBLIC int nvme_uuid_from_string(
		const char *str,
		unsigned char uuid[NVME_UUID_LEN])
{
	int n;
...
LIBNVME_PUBLIC int nvme_random_uuid(unsigned char uuid[NVME_UUID_LEN])
{
...
LIBNVME_PUBLIC int nvme_find_uuid(
