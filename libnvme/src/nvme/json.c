				   ...
				   struct json_object *ctrl_obj)
{
	...
	json_object_object_foreach(ctrl_obj, key_str, val_obj) {
		...
		if (!strcmp("keyring", key_str))
			libnvme_ctrl_set_keyring(c,
				json_object_get_string(val_obj));
		if (!strcmp("tls_key_identity", key_str)) {
			libnvme_ctrl_set_tls_key_identity(c,
				json_object_get_string(val_obj));
		}
		if (!strcmp("tls_key", key_str)) {
			libnvme_ctrl_set_tls_key(c,
				json_object_get_string(val_obj));
		}
	}
}
...
static void json_parse_subsys(libnvme_host_t h, struct json_object *subsys_obj)
{
	...
	attr_obj = json_object_object_get(subsys_obj,
					  "persistent_discovery_ctrl");
	if (attr_obj)
		libnvme_subsystem_set_pdc_enabled(s,
...
static void json_parse_host(struct libnvme_global_ctx *ctx, struct json_object *host_obj)
{
...
int json_update_config(struct libnvme_global_ctx *ctx, int fd)
{
	...
	libnvme_for_each_host(ctx, h) {
		...
		dhchap_key = libnvme_host_get_dhchap_host_key(h);
		if (dhchap_key)
				...
				struct json_object *ns_array)
{
	...
	libnvme_subsystem_for_each_ns(s, n) {
		...
		json_object_object_add(ns_obj, "nsid",
				json_object_new_int(libnvme_ns_get_nsid(n)));
		json_object_object_add(ns_obj, "name",
				json_object_new_string(libnvme_ns_get_name(n)));
		...
		libnvme_namespace_for_each_path(n, p) {
			...
			json_object_object_add(path_obj, "path",
				json_object_new_string(libnvme_path_get_name(p)));
			json_object_object_add(path_obj, "ANAState",
				json_object_new_string(libnvme_path_get_ana_state(p)));
			json_object_object_add(path_obj, "NUMANodes",
				json_object_new_string(libnvme_path_get_numa_nodes(p)));
			json_object_object_add(path_obj, "qdepth",
				json_object_new_int(libnvme_path_get_queue_depth(p)));
...
int json_dump_tree(struct libnvme_global_ctx *ctx)
{
	...
	libnvme_for_each_host(ctx, h) {
		...
		host_obj = json_object_new_object();
		json_object_object_add(host_obj, "hostnqn",
				       json_object_new_string(libnvme_host_get_hostnqn(h)));
		hostid = libnvme_host_get_hostid(h);
		if (hostid)
			json_object_object_add(host_obj, "hostid",
					       json_object_new_string(hostid));
