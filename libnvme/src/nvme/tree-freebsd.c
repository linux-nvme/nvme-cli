...
/*
 * Linux's "state" comes from a sysfs attribute (live/dead/connecting/
 * ...). FreeBSD has no equivalent concept exposed anywhere -- same as
 * Windows, see tree-win.c -- so this always reports the empty string
 * rather than guessing at a value nothing backs.
 */
...
/*
 * Linux uses this to attach to an already-existing controller found by
 * instance number, e.g. on an NVMe-oF reconnect. Fabrics aren't wired
 * up on FreeBSD yet (want_fabrics is Linux-only, see top-level
 * meson.build), so there is no such pre-existing instance to attach
 * to -- same reasoning as tree-win.c.
 */
