 ...
 * posix_memalign() path regardless of size. This does not give up
 * superpage promotion the way it would on Linux: FreeBSD's VM system
 * automatically promotes eligible, sufficiently-aligned mappings to
 * superpages on its own, with no explicit flag needed from the caller.
 */
