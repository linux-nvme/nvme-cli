...
static void print_usage(const char *prog)
{
	printf(
		...
		prog,
		OUTPUT_FNAME_DEFAULT_C,
		OUTPUT_FNAME_DEFAULT_H,
...
static void args_parse(Conf_t *conf, int argc, char *argv[])
{
	...
	conf->l_fname = OUTPUT_FNAME_DEFAULT_LD;
	conf->prefix = "";
	...
	for (int opt = getopt_long(argc, argv, optstr, longopts, NULL);
		...
		case 'l': conf->l_fname = optarg; break;
...
int main(int argc, char *argv[])
{
	...
	STRLST_FOREACH(&conf.hdr_files, in_hdr) {
		...
		STRUCT_LIST_FOREACH(&sl, si) {
			...
			generate_ld(tmp_ld_map, si, &conf);
		}
	}
