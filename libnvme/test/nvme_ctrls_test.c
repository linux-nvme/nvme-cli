...
#define _ctrl_print_pro_str(cnt, prop_name) \
...
static void _test_nvme_ctrl_pro_query(struct nvme_ctrl *cnt)
{
	...
	_ctrl_print_pro_str(cnt, dev_path);
	_ctrl_print_pro_u16(cnt, vid);
	_ctrl_print_pro_u16(cnt, ssvid);
	_ctrl_print_pro_str(cnt, sn);
	_ctrl_print_pro_str(cnt, mn);
	_ctrl_print_pro_str(cnt, fr);
	_ctrl_print_pro_u8(cnt, rab);
	_ctrl_print_pro_str(cnt, ieee);
	_ctrl_print_pro_u8(cnt, cmic);
	_ctrl_print_pro_u8(cnt, mdts);
	_ctrl_print_pro_u16(cnt, cntlid);
	_ctrl_print_pro_u32(cnt, ver);
	_ctrl_print_pro_str(cnt, ver_str);
	_ctrl_print_pro_u32(cnt, rtd3r);
	_ctrl_print_pro_u32(cnt, rtd3e);
	_ctrl_print_pro_u16(cnt, oaes);
	_ctrl_print_pro_u32(cnt, ctratt);
	_ctrl_print_pro_u16(cnt, rrls);
	_ctrl_print_pro_u8(cnt, cntrltype);
	_ctrl_print_pro_str(cnt, fguid);
	_ctrl_print_pro_u16(cnt, crdt1);
	_ctrl_print_pro_u16(cnt, crdt2);
	_ctrl_print_pro_u16(cnt, crdt3);
	...
	_ctrl_print_pro_u32(cnt, hmminds);
	_ctrl_print_pro_u16(cnt, hmmaxd);
	_ctrl_print_pro_u16(cnt, nsetidmax);
	_ctrl_print_pro_u16(cnt, endgidmax);
	_ctrl_print_pro_u8(cnt, anatt);
	_ctrl_print_pro_u8(cnt, anacap);
	_ctrl_print_pro_u32(cnt, anagrpmax);
	_ctrl_print_pro_u32(cnt, nanagrpid);
	_ctrl_print_pro_u32(cnt, pels);
	...
	_ctrl_print_pro_u8(cnt, nwpc);
	_ctrl_print_pro_u16(cnt, acwu);

	_ctrl_print_pro_u32(cnt, sgls);
	_ctrl_print_pro_u32(cnt, mnan);
	_ctrl_print_pro_str(cnt, subnqn);
...
static void _test_nvme_ctrl_raw_id_data(struct nvme_ctrl *cnt)
{
	...
	for(; i < NVME_SPEC_CTRL_RAW_IDENTIFY_DATA_LEN; ++i) {
		++j;
		if (j >= 50) {
			printf("%02x\n", raw_id_data[i]);
			j = 0;
