...
static struct test_data *find_test_data(struct libnvme_ctrl *c)
{
...
static void show_ctrl(struct libnvme_ctrl *c)
{
...
static bool match_ctrl(struct test_data *d, struct libnvme_ctrl *c)
{
	bool pass = true;
...
static bool tcp_ctrl_lookup(struct libnvme_subsystem *s, struct test_data *d)
{
	struct libnvme_ctrl *c;
	bool pass = true;
...
static bool default_ctrl_lookup(struct libnvme_subsystem *s, struct test_data *d)
{
	struct libnvme_ctrl *c;
	bool pass = true;
	struct libnvmf_context f = d->f;
		       ...
		       bool should_match)
{
	struct libnvme_global_ctx *ctx;
	struct libnvme_host *h;
	struct libnvme_ctrl *reference_ctrl; /* Existing controller (from sysfs) */
	struct libnvme_ctrl *candidate_ctrl;
	struct libnvme_ctrl *found_ctrl;
	struct libnvme_subsystem *s;
			      ...
			      bool should_match)
{
	...
	match = libnvmf_ctrl_match_config(reference_ctrl, &candidate->f.ctrl_params);
...
static bool test_well_known_nqn(void)
{
	struct libnvme_global_ctx *ctx;
	struct libnvme_host *h;
	struct libnvme_subsystem *s_disc, *s_regular;
	struct libnvme_ctrl *disc_ctrl, *regular_ctrl, *found;
	bool pass = true;
...
static bool test_lookup_ctrl_pagination(void)
{
	...
	struct libnvme_host *h;
	struct libnvme_subsystem *s;
	struct libnvme_ctrl *ctrl_a, *ctrl_b, *found, *ctrl_new;
	bool pass = true;
