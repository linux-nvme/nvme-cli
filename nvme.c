...
static int get_telemetry_log(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	/* Setting offset at blocksize since the first block is the header
	 * and is already pulled
	 */
	offset = bs;
	...
	/* In case full_size is somehow not a multiple of 512, return from this function
	 * to prevent an infinite loop.
	 */
	if (full_size % bs) {
		fprintf(stderr, "The full size of the data area requested is not rounded to the nearest block\n");
		goto free_mem;
	}

	/* Continuously pull data until the offset hits the end
	 * of the last block.
	 */
