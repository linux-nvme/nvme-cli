...
static bool is_set_reg(struct get_reg_config *cfg, int offset)
{
	switch (offset) {
	case NVME_REG_CAP:
		return cfg->cap;
...
static int get_register_properties(int fd, void **pbar, struct get_reg_config *cfg)
{
	...
	int size = offset + get_reg_size(offset);
	__u64 value;
	void *bar = malloc(size);
	struct nvme_get_property_args args = {
		.args_size = sizeof(args),
	...
	for (offset = NVME_REG_CAP; offset <= NVME_REG_CRTO; offset += get_reg_size(offset)) {
		if ((cfg->offset != offset && !is_set_reg(cfg, offset)) ||
				...
				enum nvme_print_flags flags)
{
	...
	for (offset = NVME_REG_CAP; offset <= NVME_REG_PMRMSCU; offset += get_reg_size(offset)) {
		if (!nvme_is_ctrl_reg(offset) || offset == cfg->offset || !is_set_reg(cfg, offset))
			continue;
