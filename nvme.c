...
static int get_effects_log(int argc, char **argv, struct command *acmd, struct plugin *plugin)
{
	...
	if (cfg.csi < 0) {
		...
		if (bar) {
			...
			err = nvme_submit_admin_passthru(hdl, &cmd, &cap);
			if (err)
				goto cleanup_list;
		}
...
static int virtual_mgmt(int argc, char **argv, struct command *acmd, struct plugin *plugin)
{
	...
	err = nvme_submit_admin_passthru(hdl, &cmd, &result);
	if (!err)
		printf("success, Number of Controller Resources Modified (NRM):%" PRIu64"\n",
			(uint64_t)result);
	else if (err > 0)
		nvme_show_status(err);
	else
...
static int nvme_get_single_property(struct nvme_transport_handle *hdl, struct get_reg_config *cfg, __u64 *value)
{
	...
	err = nvme_submit_admin_passthru(hdl, &cmd, value);
	if (!err)
		return 0;
...
static int dir_send(int argc, char **argv, struct command *acmd, struct plugin *plugin)
{
	...
	if (!err) {
		printf("dir-send: type %#x, operation %#x, spec_val %#x, nsid %#x, result %#" PRIx64 "\n",
		       cfg.dtype, cfg.doper, cfg.dspec, cfg.namespace_id, (uint64_t)result);
		if (buf) {
			if (!cfg.raw_binary)
				d(buf, cfg.data_len, 16, 1);
			else
...
static int copy_cmd(int argc, char **argv, struct command *acmd, struct plugin *plugin)
{
	...
	nvme_init_var_size_tags(&cmd, NVME_NVM_PIF_32B_GUARD, 0, cfg.ilbrt, 0);
	nvme_init_app_tag(&cmd, cfg.lbat, cfg.lbatm);
	err = nvme_submit_admin_passthru(hdl, &cmd, NULL);
	if (err < 0)
...
static int capacity_mgmt(int argc, char **argv, struct command *acmd, struct plugin *plugin)
{
	...
	if (!err) {
		...
		if (cfg.operation == 1)
			printf("Created Element Identifier for Endurance Group is: %" PRIu64 "\n",
			       (uint64_t)result);
		else if (cfg.operation == 3)
			printf("Created Element Identifier for NVM Set is: %" PRIu64 "\n",
			       (uint64_t)result);
	} else if (err > 0) {
		nvme_show_status(err);
	} else if (err < 0) {
		...
		const char *desc, struct command *acmd)
{
	...
	if (err < 0) {
		...
		fprintf(stderr, "%s Command %s is Success and result: 0x%" PRIx64 "\n",
			...
			cmd_name : "Vendor Specific", (uint64_t)result);
		if (cfg.read)
...
static int nvme_mi(int argc, char **argv, __u8 admin_opcode, const char *desc)
{
	...
	err = nvme_submit_admin_passthru(hdl, &cmd, &res);
	if (err < 0) {
