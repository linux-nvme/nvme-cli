					...
					struct nvme_transport_handle **hdl)
{
	...
	*hdl = nvme_open(ctx, devname);
	if (*hdl && log_level >= LOG_DEBUG)
		nvme_show_init();
...
static int append_keyfile(const char *keyring, long id, const char *keyfile)
{
	...
	exported_key = nvme_export_tls_key_versioned(ver, hmac, key_data,
						     key_len);
	if (!exported_key) {
			   ...
			   char *desc, int desc_len, void *data)
{
	...
	encoded_key = nvme_export_tls_key_versioned(ver, hmac, key_data, key_len);
	if (!encoded_key)
		return;
	fprintf(fd, "%s %s\n", desc, encoded_key);
}
...
static int import_key(const char *keyring, FILE *fd)
{
	...
	while (fgets(tls_str, 512, fd)) {
		...
		*tls_key = '\0';
		...
		key = nvme_update_key(keyring_id, "psk", tls_str,
				psk, key_len);
		if (!key) {
			continue;
		}
