...
int getlogpage_format_type(char *model_name)
{
	...
	const char *boundary_model_name =
	    "P5920"; // Use INTEL_FORMAT from Raisin P5920.
	if (strncmp(model_name, boundary_model_name,
		    strlen(boundary_model_name)) < 0) {
...
static void get_memblaze_new_smart_info(struct nvme_p4_smart_log *smart,
					int index, u8 *nm_val, u8 *raw_val)
{
					...
					unsigned int nsid, const char *devname)
{
	...
	get_memblaze_new_smart_info(smart, RAISIN_SI_VD_WEARLEVELING_COUNT, nm,
				    raw);
	printf("%-31s : %3d%%       %s%u%s%u%s%u\n", STR02_01, *nm, STR02_03,
	       *raw, STR02_04, *(raw + 2), STR02_05, *(raw + 4));
	/* 03 RAISIN_SI_VD_E2E_DECTECTION_COUNT */
	get_memblaze_new_smart_info(smart, RAISIN_SI_VD_E2E_DECTECTION_COUNT,
				    nm, raw);
	printf("%-31s: %3d%%       %" PRIu64 "\n", STR03_01, *nm,
	       int48_to_long(raw));
	/* 04 RAISIN_SI_VD_PCIE_CRC_ERR_COUNT */
	get_memblaze_new_smart_info(smart, RAISIN_SI_VD_PCIE_CRC_ERR_COUNT, nm,
				    raw);
	printf("%-32s: %3d%%       %" PRIu64 "\n", STR04_01, *nm,
	       int48_to_long(raw));
	/* 05 RAISIN_SI_VD_TIMED_WORKLOAD_MEDIA_WEAR */
	get_memblaze_new_smart_info(
	    smart, RAISIN_SI_VD_TIMED_WORKLOAD_MEDIA_WEAR, nm, raw);
	printf("%-32s: %3d%%       %.3f%%\n", STR05_01, *nm,
	       ((float)int48_to_long(raw)) / 1000);
	/* 06 RAISIN_SI_VD_TIMED_WORKLOAD_HOST_READ */
	get_memblaze_new_smart_info(
	    smart, RAISIN_SI_VD_TIMED_WORKLOAD_HOST_READ, nm, raw);
	printf("%-32s: %3d%%       %" PRIu64 "%%\n", STR06_01, *nm,
	       int48_to_long(raw));
	/* 07 RAISIN_SI_VD_TIMED_WORKLOAD_TIMER */
	get_memblaze_new_smart_info(smart, RAISIN_SI_VD_TIMED_WORKLOAD_TIMER,
				    nm, raw);
	printf("%-32s: %3d%%       %" PRIu64 "%s\n", STR07_01, *nm,
	       int48_to_long(raw), STR07_02);
	/* 08 RAISIN_SI_VD_THERMAL_THROTTLE_STATUS */
	get_memblaze_new_smart_info(smart, RAISIN_SI_VD_THERMAL_THROTTLE_STATUS,
				    nm, raw);
	printf("%-32s: %3d%%       %" PRIu64 "%%%s%" PRIu64 "\n", STR08_01, *nm,
	       int48_to_long(raw), STR08_02, int48_to_long(raw + 1));
	/* 09 RAISIN_SI_VD_RETRY_BUFF_OVERFLOW_COUNT */
	get_memblaze_new_smart_info(
	    smart, RAISIN_SI_VD_RETRY_BUFF_OVERFLOW_COUNT, nm, raw);
	printf("%-32s: %3d%%       %" PRIu64 "\n", STR09_01, *nm,
	       int48_to_long(raw));
	/* 10 RAISIN_SI_VD_PLL_LOCK_LOSS_COUNT */
	get_memblaze_new_smart_info(smart, RAISIN_SI_VD_PLL_LOCK_LOSS_COUNT, nm,
				    raw);
	printf("%-32s: %3d%%       %" PRIu64 "\n", STR10_01, *nm,
	       int48_to_long(raw));
	/* 11 RAISIN_SI_VD_TOTAL_WRITE */
	get_memblaze_new_smart_info(smart, RAISIN_SI_VD_TOTAL_WRITE, nm, raw);
	printf("%-32s: %3d%%       %s%" PRIu64 "\n", STR11_01, *nm, STR11_03,
	       int48_to_long(raw));
	/* 12 RAISIN_SI_VD_HOST_WRITE */
	get_memblaze_new_smart_info(smart, RAISIN_SI_VD_HOST_WRITE, nm, raw);
	printf("%-32s: %3d%%       %s%" PRIu64 "\n", STR12_01, *nm, STR12_03,
	       int48_to_long(raw));
	/* 13 RAISIN_SI_VD_SYSTEM_AREA_LIFE_LEFT */
	get_memblaze_new_smart_info(smart, RAISIN_SI_VD_SYSTEM_AREA_LIFE_LEFT,
				    nm, raw);
	printf("%-32s: %3d%%       %" PRIu64 "\n", STR13_01, *nm,
	       int48_to_long(raw));
	...
	get_memblaze_new_smart_info(smart, RAISIN_SI_VD_TEMPT_SINCE_BORN, nm,
				    raw);
	printf("%-32s: %3d%%       %s%u%s%u%s%u\n", STR15_01, *nm, STR15_03,
	       *raw, STR15_04, *(raw + 2), STR15_05, *(raw + 4));
	/* 16 RAISIN_SI_VD_POWER_CONSUMPTION */
	get_memblaze_new_smart_info(smart, RAISIN_SI_VD_POWER_CONSUMPTION, nm,
				    raw);
	printf("%-32s: %3d%%       %s%u%s%u%s%u\n", STR16_01, *nm, STR16_03,
	       *raw, STR16_04, *(raw + 2), STR16_05, *(raw + 4));
	/* 17 RAISIN_SI_VD_TEMPT_SINCE_BOOTUP */
	get_memblaze_new_smart_info(smart, RAISIN_SI_VD_TEMPT_SINCE_BOOTUP, nm,
				    raw);
	printf("%-32s: %3d%%       %s%u%s%u%s%u\n", STR17_01, *nm, STR17_03,
	       *raw, STR17_04, *(raw + 2), STR17_05, *(raw + 4));
	/* 18 RAISIN_SI_VD_POWER_LOSS_PROTECTION */
	get_memblaze_new_smart_info(smart, RAISIN_SI_VD_POWER_LOSS_PROTECTION,
				    nm, raw);
	printf("%-32s: %3d%%       %" PRIu64 "\n", STR18_01, *nm,
	       int48_to_long(raw));
	...
	get_memblaze_new_smart_info(smart, RAISIN_SI_VD_THERMAL_THROTTLE_TIME,
				    nm, raw);
	printf("%-32s: %3d%%       %" PRIu64 "\n", STR20_01, *nm,
	       int48_to_long(raw));
	/* 21 RAISIN_SI_VD_FLASH_MEDIA_ERROR */
	get_memblaze_new_smart_info(smart, RAISIN_SI_VD_FLASH_MEDIA_ERROR, nm,
				    raw);
	printf("%-32s: %3d%%       %" PRIu64 "\n", STR21_01, *nm,
	       int48_to_long(raw));
					...
					const char *fw_ver)
{
	...
	*(fw_ver_local + STR_VER_SIZE - 1) = '\0';
	...
	printf("Total write in GB since last factory reset		"
	       "	: %" PRIu64 "\n",
	       int48_to_long(smart->items[TOTAL_WRITE].rawval));
	printf("Total read in GB since last factory reset		"
	       "	: %" PRIu64 "\n",
	       int48_to_long(smart->items[TOTAL_READ].rawval));

	printf("Thermal throttling status[1:HTP in progress]		"
	       "	: %u\n",
	       smart->items[THERMAL_THROTTLE].thermal_throttle.on);
	printf("Total thermal throttling minutes since power on		"
	       "	: %u\n",
	       smart->items[THERMAL_THROTTLE].thermal_throttle.count);

	printf("Maximum temperature in Kelvin since last factory reset	"
	       "	: %u\n",
	       le16_to_cpu(smart->items[TEMPT_SINCE_RESET].temperature.max));
	printf("Minimum temperature in Kelvin since last factory reset	"
	       "	: %u\n",
	       le16_to_cpu(smart->items[TEMPT_SINCE_RESET].temperature.min));
	if (compare_fw_version(fw_ver, "0.09.0300") != 0) {
		printf("Maximum temperature in Kelvin since power on	"
		       "		: %u\n",
		       le16_to_cpu(
			   smart->items[TEMPT_SINCE_BOOTUP].temperature_p.max));
		printf("Minimum temperature in Kelvin since power on	"
		       "		: %u\n",
		       le16_to_cpu(
			   smart->items[TEMPT_SINCE_BOOTUP].temperature_p.min));
	}
	printf("Current temperature in Kelvin				"
	       "	: %u\n",
	       le16_to_cpu(smart->items[TEMPT_SINCE_RESET].temperature.curr));

	printf("Maximum power in watt since power on			"
	       "	: %u\n",
	       le16_to_cpu(smart->items[POWER_CONSUMPTION].power.max));
	printf("Minimum power in watt since power on			"
	       "	: %u\n",
	       le16_to_cpu(smart->items[POWER_CONSUMPTION].power.min));
	printf("Current power in watt					"
	       "	: %u\n",
	       le16_to_cpu(smart->items[POWER_CONSUMPTION].power.curr));

	item = &smart->items[POWER_LOSS_PROTECTION];
	if (item_id_2_u32(item) == 0xEC)
		printf("Power loss protection normalized value		"
		       "		: %u\n",
		       item->power_loss_protection.curr);

	item = &smart->items[WEARLEVELING_COUNT];
	if (item_id_2_u32(item) == 0xAD) {
		printf("Percentage of wearleveling count left		"
		       "		: %u\n",
		       le16_to_cpu(item->nmval));
		printf("Wearleveling count min erase cycle		"
		       "		: %u\n",
		       le16_to_cpu(item->wearleveling_count.min));
		printf("Wearleveling count max erase cycle		"
		       "		: %u\n",
		       le16_to_cpu(item->wearleveling_count.max));
		printf("Wearleveling count avg erase cycle		"
		       "		: %u\n",
	...
	if (item_id_2_u32(item) == 0xF5)
		printf("Total host write in GiB since device born 	"
		       "		: %llu\n",
	...
	if (item_id_2_u32(item) == 0xEB)
		printf("Thermal throttling count since device born 	"
		       "		: %u\n",
		       item->thermal_throttle_cnt.cnt);

	item = &smart->items[CORRECT_PCIE_PORT0];
	if (item_id_2_u32(item) == 0xED)
		printf("PCIE Correctable Error Count of Port0    	"
		       "		: %llu\n",
		       (unsigned long long)raw_2_u64(item->rawval,
						     sizeof(item->rawval)));
	...
	if (item_id_2_u32(item) == 0xEE)
		printf("PCIE Correctable Error Count of Port1 	        "
		       "		: %llu\n",
		       (unsigned long long)raw_2_u64(item->rawval,
						     sizeof(item->rawval)));
	...
	if (IS_PAPAYA(fw_ver_local)) {
		...
		printf("%-31s                                 : %3d%%       "
		       "%s%u%s%u%s%u\n",
		       STR02_01, *nm, STR02_03, *raw, STR02_04, *(raw + 2),
		       STR02_05, *(raw + 4));
		/* 11 RAISIN_SI_VD_TOTAL_WRITE */
				   ...
				   struct nvme_memblaze_smart_log *smart)
{
	...
	snprintf(fw_ver, sizeof(fw_ver), "%c.%c%c.%c%c%c%c", ctrl.fr[0],
		 ctrl.fr[1], ctrl.fr[2], ctrl.fr[3], ctrl.fr[4], ctrl.fr[5],
		 ctrl.fr[6]);
...
static int mb_get_additional_smart_log(int argc, char **argv,
				       struct command *cmd,
				       struct plugin *plugin)
{
	...
	int err, fd;
	char *desc =
	    "Get Memblaze vendor specific additional smart log (optionally, "
	...
	err = nvme_get_nsid_log(fd, 0xca, cfg.namespace_id, sizeof(smart_log),
				&smart_log);
	if (!err) {
		if (!cfg.raw_binary)
			err = show_memblaze_smart_log(fd, cfg.namespace_id,
						      devicename, &smart_log);
		else
...
static int mb_get_powermanager_status(int argc, char **argv,
				      struct command *cmd,
				      struct plugin *plugin)
{
...
static int mb_set_powermanager_status(int argc, char **argv,
				      struct command *cmd,
				      struct plugin *plugin)
{
	...
	OPT_ARGS(opts) = {OPT_UINT("value", 'v', &cfg.value, value),
			  OPT_FLAG("save", 's', &cfg.save, save), OPT_END()};
	...
	err =
	    nvme_set_features(fd, cfg.feature_id, 0, cfg.value, 0, cfg.save, 0,
			      0, 0, NULL, NVME_DEFAULT_IOCTL_TIMEOUT, &result);
	if (err < 0) {
		perror("set-feature");
	}
	if (!err) {
				   ...
				   struct plugin *plugin)
{
	...
	OPT_ARGS(opts) = {OPT_LIST("param", 'p', &cfg.param, param), OPT_END()};
...
static int glp_high_latency_show_bar(FILE *fdi, int print)
{
	...
	fPRINT_PARAM1("--------------------------------------------------------"
		      "-------------------------------------\n");
	fPRINT_PARAM1("Timestamp                        Type    QID    CID    "
		      "NSID     StartLBA      NumLBA   Latency\n");
	fPRINT_PARAM1("--------------------------------------------------------"
		      "-------------------------------------\n");
	return 0;
}
	...
	for (i = 0; i < buflen; i += entrySize) {
		...
		} else //  sort
		{
			...
			snprintf(string, sizeof(string),
				 "%4d%02d%02d--%02d:%02d:%02d.%03d UTC",
		...
		fprintf(fdi, "%-32s %-7x %-6x %-6x %-8x %4x%08x  %-8x %-d\n",
			...
			logEntry->nsid, (__u32)(logEntry->sLBA >> 32),
				     ...
				     struct plugin *plugin)
{
	...
	err = nvme_get_log_simple(fd, GLP_ID_VU_GET_HIGH_LATENCY_LOG,
				  sizeof(buf), &buf);
...
static int mb_selective_download(int argc, char **argv, struct command *cmd,
				 struct plugin *plugin)
{
	const char *desc =
	    ...
	    "requires a power cycle once the "
	...
	if (err == 0x10B || err == 0x20B) {
		err = 0;
		fprintf(stderr, "Update successful! Please power cycle for "
...
static void ioLatencyHistogramOutput(FILE *fd, int index, int start, int end,
				     char *unit0, char *unit1,
				     unsigned int *pHistogram, int print)
{
	int len;
	...
	if (end != 0x7FFFFFFF) {
		len = snprintf(subString1, sizeof(subString1), "%d%s", end,
			       unit1);
	} else {
...
int io_latency_histogram(char *file, char *buf, int print, int logid)
{
	...
	if (revision[1] == 1 && revision[0] == 0) {
		...
		for (i = 0; i < 32; i++, index++) {
			if (i == 31) {
				strcpy(unit[1], "ms");
				ioLatencyHistogramOutput(
				    fdi, index, i * 32, 1, unit[0], unit[1],
				    (unsigned int *)buf, print);
			} else {
				ioLatencyHistogramOutput(
				    fdi, index, i * 32, (i + 1) * 32, unit[0],
		...
		for (i = 1; i < 32; i++, index++) {
			ioLatencyHistogramOutput(fdi, index, i, i + 1, unit[0],
						 unit[1], (unsigned int *)buf,
						 print);
		}
		...
		ioLatencyHistogramOutput(fdi, index, i, 0x7FFFFFFF, unit[0],
					 unit[1], (unsigned int *)buf, print);
	} else {
...
static int mb_lat_stats_log_print(int argc, char **argv, struct command *cmd,
				  struct plugin *plugin)
{
	...
	OPT_ARGS(opts) = {OPT_FLAG("write", 'w', &cfg.write, write), OPT_END()};

	fd = parse_and_open(argc, argv, desc, opts);
	if (fd < 0)
		return fd;

	err = nvme_get_log_simple(fd, cfg.write ? 0xc2 : 0xc1, sizeof(stats),
				  &stats);
	if (!err)
		io_latency_histogram(
		    cfg.write ? f2 : f1, stats, DO_PRINT_FLAG,
				    ...
				    struct plugin *plugin)
{
	...
	// const char *value = "new value of feature (required)";
	// const char *save = "specifies that the controller shall save the
	// attribute";
	__u32 result;
	...
	/*
		...
		if (!err) {
			printf("OP(0x%2X) FID(0x%2X) Clear error log
...
static int mb_set_lat_stats(int argc, char **argv, struct command *command,
			    struct plugin *plugin)
{
	int err, fd;
	const char *desc = ("Enable/Disable Latency Statistics Tracking.\n"
	...
	case None:
		err =
		    nvme_get_features(fd, nsid, fid, sel, cdw11, 0, data_len,
				      buf, NVME_DEFAULT_IOCTL_TIMEOUT, &result);
		if (!err) {
			printf("Latency Statistics Tracking (FID 0x%X) is "
			       "currently (%i).\n",
			       fid, result);
		} else {
	...
	case False:
		...
		if (err > 0) {
			...
			printf("Successfully set enable bit for FID (0x%X) to "
			       "%i.\n",
			       0xe2, option);
		}
		break;
