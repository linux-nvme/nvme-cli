...
int getlogpage_format_type(char *model_name)
{
	...
	const char *boundary_model_name2 =
		"P5920"; // Use INTEL_FORMAT from Raisin P5920.
	if (0 == strncmp(model_name, boundary_model_name1,
			 strlen(boundary_model_name1))) {
		if (strncmp(model_name, boundary_model_name2,
			    strlen(boundary_model_name2)) < 0) {
	...
	*(fw_ver_local + STR_VER_SIZE - 1) = '\0';
	...
	if (compare_fw_version(fw_ver, "0.09.0300") != 0) {
		printf("Maximum temperature in Kelvin since power on			: %u\n",
		       le16_to_cpu(smart->items[TEMPT_SINCE_BOOTUP]
					   .temperature_p.max));
...
{
	const char *desc =
		"Get Memblaze power management ststus\n	(value 0 - 25w, 1 - 20w, 2 - 15w)";
	int err, fd;
	__u32 result;
...
{
	const char *desc =
		"Set Memblaze power management status\n	(value 0 - 25w, 1 - 20w, 2 - 15w)";
				   ...
				   struct plugin *plugin)
{
	...
	OPT_ARGS(opts) = { OPT_LIST("param", 'p', &cfg.param, param),
			   OPT_END() };
...
int io_latency_histogram(char *file, char *buf, int print, int logid)
{
	...
	if (revision[1] == 1 && revision[0] == 0) {
		...
		for (i = 0; i < 32; i++, index++) {
			if (i == 31) {
				strcpy(unit[1], "ms");
				ioLatencyHistogramOutput(fdi, index, i * 32, 1,
							 unit[0], unit[1],
							 (unsigned int *)buf,
							 print);
			} else {
				ioLatencyHistogramOutput(fdi, index, i * 32,
							 (i + 1) * 32, unit[0],
							 unit[1],
							 (unsigned int *)buf,
							 print);
