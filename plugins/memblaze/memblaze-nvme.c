...
static int mb_set_latency_feature(int argc, char **argv, struct command *cmd, struct plugin *plugin) {
    ...
    struct nvme_set_features_args args = {
        ...
        .cdw13     = 0 | (cfg.read_threshold & 0xff) | ((cfg.write_threshold & 0xff) << 8) | ((cfg.de_allocate_trim_threshold & 0xff) << 16),
        .cdw15     = 0,
        .save      = 0,
        .uuidx     = 0,
        .data      = NULL,
...
static int mb_get_latency_stats(int argc, char **argv, struct command *cmd, struct plugin *plugin) {
    ...
    err = nvme_get_log_simple(dev_fd(dev), LID_LATENCY_STATISTICS, sizeof(struct latency_stats), &log);
    if (!err) {
        if (!cfg.raw_binary)
...
static int mb_get_high_latency_log(int argc, char **argv, struct command *cmd, struct plugin *plugin) {
    // Get the configuration
    ...
    err = nvme_get_log_simple(dev_fd(dev), LID_HIGH_LATENCY_LOG, sizeof(struct high_latency_log), &log);
    if (!err) {
        if (!cfg.raw_binary)
    ...
    for (int i = 0; i < duration; i++) {
        ...
        for (int j = 0; j < 3600; j++) {
            ...
            printf("%-8u%-14u%-21u%-22u%-22u%-15u%-22u%-23u%-23u\n",
                   j + 1,
                   entry->read_iops, entry->read_bandwidth, entry->read_iops == 0 ? 0 : entry->read_latency / entry->read_iops, entry->read_latency_max,
                   entry->write_iops, entry->write_bandwidth, entry->write_iops == 0 ? 0 : entry->write_latency / entry->write_iops, entry->write_latency_max);
            usleep(100);
        }
        printf("\n");
    }
...
static int mb_get_performance_stats(int argc, char **argv, struct command *cmd, struct plugin *plugin) {
    // Get the configuration
