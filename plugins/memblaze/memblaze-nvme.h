...
PLUGIN(NAME("memblaze", "Memblaze vendor specific extensions", NVME_VERSION),
	COMMAND_LIST(
        ...
        ENTRY("lat-stats-print-x", "Get Latency Statistics log and show it.", mb_get_latency_stats)
		ENTRY("lat-log-print-x", "Get High Latency log and show it.", mb_get_high_latency_log)
        ENTRY("perf-stats-print-x", "Get Performance Stat log and show it.", mb_get_performance_stats)
	)
);
