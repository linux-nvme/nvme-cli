...
#define WDC_DRIVE_CAP_SMART_LOG_MASK                                           \
	(WDC_DRIVE_CAP_C0_LOG_PAGE | WDC_DRIVE_CAP_C1_LOG_PAGE |               \
	 WDC_DRIVE_CAP_CA_LOG_PAGE | WDC_DRIVE_CAP_D0_LOG_PAGE)
#define WDC_DRIVE_CAP_CLEAR_PCIE_MASK                                          \
	(WDC_DRIVE_CAP_CLEAR_PCIE | WDC_DRIVE_CAP_VUC_CLEAR_PCIE |             \
	 WDC_DRIVE_CAP_VU_FID_CLEAR_PCIE)
#define WDC_DRIVE_CAP_FW_ACTIVATE_HISTORY_MASK                                 \
	(WDC_DRIVE_CAP_FW_ACTIVATE_HISTORY |                                   \
	 WDC_DRIVE_CAP_FW_ACTIVATE_HISTORY_C2)
#define WDC_DRIVE_CAP_CLEAR_FW_ACT_HISTORY_MASK                                \
	(WDC_DRIVE_CAP_CLEAR_FW_ACT_HISTORY |                                  \
	 WDC_DRIVE_CAP_VU_FID_CLEAR_FW_ACT_HISTORY)
#define WDC_DRIVE_CAP_INTERNAL_LOG_MASK                                        \
	(WDC_DRIVE_CAP_INTERNAL_LOG | WDC_DRIVE_CAP_DUI |                      \
...
#define WDC_DE_MANUFACTURING_INFO_PAGE_FILE_NAME                               \
	"manufacturing_info" /* Unique log entry page name. */
...
static __u8 scao_guid[WDC_C2_GUID_LENGTH] = {0xC5, 0xAF, 0x10, 0x28, 0xEA, 0xBF,
					     0xF2, 0xA4, 0x9C, 0x4F, 0x6F, 0x7C,
					     0xC9, 0x14, 0xD5, 0xAF};
...
static int wdc_do_dump(int fd, __u32 opcode, __u32 data_len, __u32 cdw12,
		       char *file, __u32 xfer_size);
...
static int wdc_purge(int argc, char **argv, struct command *command,
		     struct plugin *plugin);
static int wdc_purge_monitor(int argc, char **argv, struct command *command,
			     struct plugin *plugin);
static bool wdc_nvme_check_supported_log_page(nvme_root_t r, int fd,
					      __u8 log_id);
...
static int wdc_clear_assert_dump(int argc, char **argv, struct command *command,
				 struct plugin *plugin);
static int wdc_drive_resize(int argc, char **argv, struct command *command,
			    struct plugin *plugin);
...
static int wdc_log_page_directory(int argc, char **argv,
				  struct command *command,
				  struct plugin *plugin);
...
static int wdc_enc_get_nic_log(int fd, __u8 log_id, __u32 xfer_size,
			       __u32 data_len, FILE *out);
static int wdc_enc_submit_move_data(int fd, char *cmd, int len, int xfer_size,
				    FILE *out, int data_id, int cdw14,
				    int cdw15);
static bool get_dev_mgment_cbs_data(nvme_root_t r, int fd, __u8 log_id,
				    void **cbs_data);
...
struct __attribute__((__packed__)) wdc_ssd_d0_smart_log {
	...
	__le32
	    lifetime_realloc_erase_block_count; /* 0x04 - Lifetime reallocated
						   erase block count      */
	__le32 lifetime_power_on_hours; /* 0x08 - Lifetime power on hours */
	__le32 lifetime_uecc_count;	/* 0x0C - Lifetime UECC count     */
	__le32 lifetime_wrt_amp_factor; /* 0x10 - Lifetime write amplification
					   factor         */
	__le32 trailing_hr_wrt_amp_factor; /* 0x14 - Trailing hour write
					      amplification factor    */
	__le32 reserve_erase_block_count; /* 0x18 - Reserve erase block count */
	__le32 lifetime_program_fail_count;	/* 0x1C - Lifetime program fail
						   count                 */
	__le32 lifetime_block_erase_fail_count; /* 0x20 - Lifetime block erase
						   fail count             */
	__le32 lifetime_die_failure_count; /* 0x24 - Lifetime die failure count
					    */
	__le32 lifetime_link_rate_downgrade_count; /* 0x28 - Lifetime link rate
						      downgrade count */
	__le32 lifetime_clean_shutdown_count; /* 0x2C - Lifetime clean shutdown
						 count on power loss */
	__le32 lifetime_unclean_shutdown_count; /* 0x30 - Lifetime unclean
						   shutdowns on power loss    */
	__le32 current_temp;	  /* 0x34 - Current temperature		    */
	__le32 max_recorded_temp; /* 0x38 - Max recorded temperature */
	__le32 lifetime_retired_block_count; /* 0x3C - Lifetime retired block
						count                */
	__le32 lifetime_read_disturb_realloc_events; /* 0x40 - Lifetime read
							disturb reallocation
							events   */
	...
	__le32 lifetime_thermal_throttle_act;  /* 0x60 - Lifetime thermal
						  throttle activations       */
	__le32 percentage_pe_cycles_remaining; /* 0x64 - Percentage of P/E
						  cycles remaining          */
...
struct __attribute__((__packed__)) wdc_fw_act_history_log_format_c2 {
	...
	struct wdc_fw_act_history_log_entry_c2
	    entry[WDC_MAX_NUM_ACT_HIST_ENTRIES];
	__u8 reserved2[2790];
...
{
	return denominator
		   ? (uint64_t)(((double)numerator / (double)denominator) * 100)
		   : 0;
}
...
static int wdc_get_vendor_id(int fd, uint32_t *vendor_id)
{
	...
	if (ret) {
		fprintf(stderr,
			"ERROR : WDC : nvme_identify_ctrl() failed "
			"0x%x\n",
			ret);
		return -1;
	}
...
static bool wdc_check_device(nvme_root_t r, int fd)
{
	...
	if (ret < 0) {
		/* Use the identify nvme command to get vendor id due to NVMeOF
		 * device. */
	...
	if (read_vendor_id == WDC_NVME_VID ||
		...
		fprintf(stderr,
			"ERROR : WDC: unsupported WDC device, Vendor ID = "
			"0x%x, Device ID = 0x%x\n",
...
static bool wdc_enc_check_model(int fd)
{
	...
	if (strstr(model, WDC_OPENFLEX_MI_DEVICE_MODEL) != NULL)
		...
		fprintf(stderr,
			"ERROR : WDC: unsupported WDC enclosure, Model = %s\n",
			model);
...
static __u64 wdc_get_drive_capabilities(nvme_root_t r, int fd)
{
	 ...
	 * device_id so we need to use only vendor_id*/
	...
	case WDC_NVME_VID:
		...
		case WDC_NVME_SN100_DEV_ID:
			capabilities =
			    (WDC_DRIVE_CAP_CAP_DIAG |
			     WDC_DRIVE_CAP_INTERNAL_LOG |
			     WDC_DRIVE_CAP_C1_LOG_PAGE |
			     WDC_DRIVE_CAP_DRIVE_LOG |
			     WDC_DRIVE_CAP_CRASH_DUMP |
			     WDC_DRIVE_CAP_PFAIL_DUMP | WDC_DRIVE_CAP_PURGE);
			break;
		case WDC_NVME_SN200_DEV_ID:
			...
			if (wdc_nvme_check_supported_log_page(
				r, fd, WDC_NVME_GET_DEVICE_INFO_LOG_OPCODE) ==
			    true)
				capabilities |= WDC_DRIVE_CAP_CA_LOG_PAGE;

			/* verify the 0xC1 log page is supported */
			if (wdc_nvme_check_supported_log_page(
				r, fd, WDC_NVME_ADD_LOG_OPCODE) == true)
	...
	case WDC_NVME_VID_2:
		...
		case WDC_NVME_SN630_DEV_ID_1:
			capabilities =
			    (WDC_DRIVE_CAP_CAP_DIAG |
			     ...
			     WDC_DRIVE_CAP_CLEAR_ASSERT | WDC_DRIVE_CAP_RESIZE |
			     WDC_DRIVE_CAP_CLEAR_PCIE);
			/* verify the 0xCA log page is supported */
			if (wdc_nvme_check_supported_log_page(
				r, fd, WDC_NVME_GET_DEVICE_INFO_LOG_OPCODE) ==
			...
			if (wdc_nvme_check_supported_log_page(
				r, fd, WDC_NVME_GET_VU_SMART_LOG_OPCODE) ==
			    true)
		...
		case WDC_NVME_SN640_DEV_ID_3:
			...
			if (wdc_nvme_check_supported_log_page(
				r, fd, WDC_NVME_GET_EOL_STATUS_LOG_OPCODE) ==
			    true) {
			...
			if (!get_dev_mgment_cbs_data(
				r, fd, WDC_C2_CUSTOMER_ID_ID, (void *)&data)) {
				fprintf(stderr,
					"%s: ERROR : WDC : 0xC2 Log Page entry "
					"ID 0x%x not found\n",
	...
	case WDC_NVME_SNDK_VID:
		...
		case WDC_NVME_SN730A_DEV_ID:
			capabilities =
			    WDC_DRIVE_CAP_DUI | WDC_DRIVE_CAP_NAND_STATS |
			    WDC_DRIVE_CAP_INFO_2 | WDC_DRIVE_CAP_TEMP_STATS |
...
static __u64 wdc_get_enc_drive_capabilities(nvme_root_t r, int fd)
{
	...
	case WDC_NVME_VID:
		...
		if (wdc_nvme_check_supported_log_page(
			r, fd, WDC_LATENCY_MON_OPCODE) == true)
			capabilities |= WDC_DRIVE_CAP_C3_LOG_PAGE;

		/* verify the 0xCA log page is supported */
		if (wdc_nvme_check_supported_log_page(
			r, fd, WDC_NVME_GET_DEVICE_INFO_LOG_OPCODE) == true)
			capabilities |= WDC_DRIVE_CAP_CA_LOG_PAGE;
	...
	case WDC_NVME_VID_2:
		...
		if (wdc_nvme_check_supported_log_page(
			r, fd, WDC_NVME_GET_FW_ACT_HISTORY_LOG_ID) == true)
		...
		/* verify the 0xD0 log page is supported */
		if (wdc_nvme_check_supported_log_page(
			r, fd, WDC_NVME_GET_VU_SMART_LOG_OPCODE) == true)
			capabilities |= WDC_DRIVE_CAP_D0_LOG_PAGE;

		if (!get_dev_mgment_cbs_data(r, fd, WDC_C2_CUSTOMER_ID_ID,
					     (void *)&data)) {
			fprintf(stderr,
				"%s: ERROR : WDC : 0xC2 Log Page entry ID 0x%x "
				"not found\n",
				__func__, WDC_C2_CUSTOMER_ID_ID);
			return -1;
		}
			       ...
			       const char *suffix)
{
	...
	res_len =
	    snprintf(file, len, "%s%.*s%s", orig, ctrl_sn_len, ctrl.sn, suffix);
	if (len <= res_len) {
		fprintf(stderr,
			"ERROR : WDC : cannot format serial number due to data "
...
bool wdc_get_dev_mng_log_entry(
    __u32 log_length, __u32 entry_id, struct wdc_c2_log_page_header *p_log_hdr,
    struct wdc_c2_log_subpage_header **p_p_found_log_entry)
{
	...
	if (*p_p_found_log_entry == NULL) {
		fprintf(stderr, "ERROR : WDC - wdc_get_dev_mng_log_entry: No "
				"ppLogEntry pointer.\n");
		return false;
	}
	...
	*p_p_found_log_entry = NULL;
	...
	if (log_length < sizeof(struct wdc_c2_log_page_header)) {
		fprintf(stderr,
			"ERROR : WDC - wdc_get_dev_mng_log_entry: \
	...
	p_next_log_entry =
	    (struct wdc_c2_log_subpage_header *)((__u8 *)p_log_hdr +
						 current_data_offset);
	remaining_len = log_length - size;
	valid_log = false;

	/* Walk the entire structure. Perform a sanity check to make sure this
	 is a standard version of the structure. This means making sure each
	 entry looks valid. But allow for the data to overflow the allocated
	 buffer (we don't want a false negative because of a FW formatting
	 error) */
	...
	while (remaining_len >= log_entry_hdr_size) {
		...
		if ((log_entry_size == 0) || (log_entry_size > remaining_len)) {
			fprintf(stderr,
				"ERROR : WDC: wdc_get_dev_mng_log_entry: \
			...
			/* The struture is invalid, so any match that was found
			 * is invalid. */
			*p_p_found_log_entry = NULL;
		} else {
			/* Structure must have at least one valid entry to be
			 * considered valid */
			valid_log = true;
				    ...
				    void **cbs_data)
{
	...
	*cbs_data = NULL;
	...
	if (device_id == WDC_NVME_ZN350_DEV_ID ||
	...
	if ((data = (__u8 *)malloc(sizeof(__u8) * WDC_C2_LOG_BUF_LEN)) ==
	    NULL) {
	...
	ret = nvme_get_log(fd, lid, 0xFFFFFFFF, NVME_LOG_LSP_NONE, 0, 0, false,
			   uuid_ix, 0, WDC_C2_LOG_BUF_LEN, data,
			   NVME_DEFAULT_IOCTL_TIMEOUT, NULL);
	if (ret) {
		fprintf(stderr,
			"ERROR : WDC : Unable to get 0x%x Log Page length, ret "
			"= 0x%x\n",
			lid, ret);
		goto end;
	}
	...
	if (le32_to_cpu(hdr_ptr->length) > WDC_C2_LOG_BUF_LEN) {
		/* Log Page buffer too small, free and reallocate the necessary
		 * size */
		free(data);
	...
	ret = nvme_get_log(fd, WDC_NVME_GET_DEV_MGMNT_LOG_PAGE_OPCODE,
			   0xFFFFFFFF, 0, NVME_LOG_LSP_NONE, 0, false, uuid_ix,
			   0, le32_to_cpu(hdr_ptr->length), data,
			   NVME_DEFAULT_IOCTL_TIMEOUT, NULL);

	if (ret) {
		fprintf(stderr,
			"ERROR : WDC : Unable to read C2 Log Page data, ret = "
			"0x%x\n",
			ret);
		goto end;
	}
	...
	found =
	    wdc_get_dev_mng_log_entry(hdr_ptr->length, log_id, hdr_ptr, &sph);
		...
		*cbs_data = (void *)&sph->data;
		...
		if (found) {
			*cbs_data = (void *)&sph->data;
			...
			fprintf(stderr,
				"ERROR : WDC : Unable to find correct version "
				"of page 0x%x, entry id = %d\n",
				lid, log_id);
		}
	}
end:
					      ...
					      __u8 log_id)
{
	...
	if (get_dev_mgment_cbs_data(r, fd, WDC_C2_LOG_PAGES_SUPPORTED_ID,
				    (void *)&cbs_data)) {
		if (cbs_data != NULL) {
		...
		fprintf(stderr,
			"ERROR : WDC : 0xC2 Log Page entry ID 0x%x not found\n",
			WDC_C2_LOG_PAGES_SUPPORTED_ID);

	return found;
}

static bool wdc_nvme_get_dev_status_log_data(nvme_root_t r, int fd,
					     __le32 *ret_data, __u8 log_id)
{
	__u32 *cbs_data = NULL;
...
static __u32 wdc_dump_length(int fd, __u32 opcode, __u32 cdw10, __u32 cdw12,
			     __u32 *dump_length)
{
	int ret;
	...
	if (opcode == WDC_NVME_CAP_DIAG_OPCODE)
		*dump_length = buf[0x04] << 24 | buf[0x05] << 16 |
			       buf[0x06] << 8 | buf[0x07];
	else
...
static __u32 wdc_dump_length_e6(int fd, __u32 opcode, __u32 cdw10, __u32 cdw12,
				struct wdc_e6_log_hdr *dump_hdr)
{
	int ret;
	struct nvme_passthru_cmd admin_cmd;
...
static __u32 wdc_dump_dui_data(int fd, __u32 dataLen, __u32 offset,
			       __u8 *dump_data, bool last_xfer)
{
	int ret;
	struct nvme_passthru_cmd admin_cmd;
...
static __u32 wdc_dump_dui_data_v2(int fd, __u32 dataLen, __u64 offset,
				  __u8 *dump_data, bool last_xfer)
{
	int ret;
	struct nvme_passthru_cmd admin_cmd;
		       ...
		       char *file, __u32 xfer_size)
{
	...
	while (curr_data_offset < data_len) {
		...
		if (ret != 0) {
			nvme_show_status(ret);
			fprintf(stderr,
				"%s: ERROR : WDC : Get chunk %d, size = 0x%x, "
				"offset = 0x%x, addr = 0x%lx\n",
				__func__, i, admin_cmd.data_len,
				curr_data_offset,
				(long unsigned int)admin_cmd.addr);
			break;
		}
		...
		if ((curr_data_offset + xfer_size) <= data_len)
			...
			curr_data_len =
			    data_len - curr_data_offset; /* last transfer */

		curr_data_offset += curr_data_len;
		admin_cmd.addr =
		    (__u64)(uintptr_t)dump_data + (__u64)curr_data_offset;
		admin_cmd.data_len = curr_data_len;
		admin_cmd.cdw10 = curr_data_len >> 2;
...
static int wdc_do_dump_e6(int fd, __u32 opcode, __u32 data_len, __u32 cdw12,
			  char *file, __u32 xfer_size, __u8 *log_hdr)
{
	int ret = 0;
	__u8 *dump_data;
				    ...
				    int data_area)
{
	...
	if (type == WDC_TELEMETRY_TYPE_HOST) {
		...
		err = nvme_get_features_data(
		    fd, WDC_VU_DISABLE_CNTLR_TELEMETRY_OPTION_FEATURE_ID, 0, 4,
		    buf, &result);
		if (err == 0) {
			if (result == 0) {
				/* enabled */
				...
				fprintf(stderr,
					"%s: Controller initiated option "
					"telemetry log page disabled\n",
					__func__);
				err = -EINVAL;
				goto close_fd;
			}
		} else {
			fprintf(stderr, "ERROR : WDC: Get telemetry option "
					"feature failed.");
	...
	if (!hdr || !page_log) {
		fprintf(stderr,
			"%s: Failed to allocate 0x%x bytes for log: %s\n",
	...
	if (ctrl_init)
		err = nvme_get_log_telemetry_ctrl(
		    fd, true, 0, WDC_TELEMETRY_HEADER_LENGTH, hdr);
	else if (host_gen)
		err = nvme_get_log_create_telemetry_host(fd, hdr);
	else
		err = nvme_get_log_telemetry_host(
		    fd, 0, WDC_TELEMETRY_HEADER_LENGTH, hdr);
	...
	if (err != WDC_TELEMETRY_HEADER_LENGTH) {
		fprintf(stderr,
			"%s: Failed to flush header data to file!, err = %d\n",
			__func__, err);
		goto close_output;
	}
	...
	case 1:
		full_size =
		    (le16_to_cpu(hdr->dalb1) * WDC_TELEMETRY_BLOCK_SIZE) +
		    WDC_TELEMETRY_HEADER_LENGTH;
		break;
	case 2:
		full_size =
		    (le16_to_cpu(hdr->dalb2) * WDC_TELEMETRY_BLOCK_SIZE) +
		    WDC_TELEMETRY_HEADER_LENGTH;
		break;
	case 3:
		full_size =
		    (le16_to_cpu(hdr->dalb3) * WDC_TELEMETRY_BLOCK_SIZE) +
		    WDC_TELEMETRY_HEADER_LENGTH;
		break;
	case 4:
		...
		err = nvme_get_features(
		    fd, NVME_NSID_ALL, NVME_FEAT_FID_HOST_BEHAVIOR, 0, 0, 0,
		    len, buf, NVME_DEFAULT_IOCTL_TIMEOUT, &result);
		if (err > 0) {
			nvme_show_status(err);
		} else if (err < 0) {
			perror("get-feature");
			...
			if ((ctrl.lpa & 0x40)) {
				if (((unsigned char *)buf)[1] == 1)
					full_size = (le32_to_cpu(hdr->dalb4) *
						     WDC_TELEMETRY_BLOCK_SIZE) +
						    WDC_TELEMETRY_HEADER_LENGTH;
				else {
					fprintf(stderr,
						"Data area 4 unsupported, Host "
						"Behavior Support ETDAS not "
						"set to 1\n");
					errno = EINVAL;
					err = -1;
				}
				...
				fprintf(stderr,
					"Data area 4 unsupported, bit 6 of Log "
					"Page Attributes not set\n");
				errno = EINVAL;
				err = -1;
			}
		}
	...
	default:
		fprintf(stderr,
			"%s: Invalid data area requested, data area = %d\n",
			__func__, data_area);
		err = -EINVAL;
		goto close_output;
	}
	...
	while (offset < full_size) {
		...
		if (ctrl_init)
			err = nvme_get_log_telemetry_ctrl(fd, true, offset, bs,
							  page_log);
		else
			err = nvme_get_log_telemetry_host(fd, offset, bs,
							  page_log);
		...
		if (err < 0) {
			...
			nvme_show_status(err);
			fprintf(stderr,
				"%s: Failed to acquire full telemetry log!\n",
				__func__);
			break;
		}
		...
		if (err != bs) {
			fprintf(stderr,
				"%s: Failed to flush telemetry data to file!, "
				"err = %d\n",
				__func__, err);
			break;
		}
...
static int wdc_do_cap_diag(nvme_root_t r, int fd, char *file, __u32 xfer_size,
			   int type, int data_area)
{
	int ret = -1;
	...
	if (type == WDC_TELEMETRY_TYPE_NONE) {
		...
		cap_diag_length =
		    (log_hdr->log_size[0] << 24 | log_hdr->log_size[1] << 16 |
		     log_hdr->log_size[2] << 8 | log_hdr->log_size[3]);

		if (cap_diag_length == 0) {
			fprintf(
			    stderr,
			    "INFO : WDC : Capture Diagnostics log is empty\n");
		} else {
			ret = wdc_do_dump_e6(fd, WDC_NVME_CAP_DIAG_OPCODE,
					     cap_diag_length,
					     (WDC_NVME_CAP_DIAG_SUBCMD
					      << WDC_NVME_SUBCMD_SHIFT) |
						 WDC_NVME_CAP_DIAG_CMD,
					     file, xfer_size, (__u8 *)log_hdr);

			fprintf(stderr,
				"INFO : WDC : Capture Diagnostics log, length "
				"= 0x%x\n",
				cap_diag_length);
		}
		...
		ret = wdc_do_cap_telemetry_log(fd, file, xfer_size, type,
					       data_area);
	} else
...
static int wdc_do_cap_dui(int fd, char *file, __u32 xfer_size, int data_area,
			  int verbose, __u64 file_size, __u64 offset)
{
	int ret = 0;
	...
	if (log_hdr == NULL) {
		fprintf(stderr,
			"%s: ERROR : log header malloc failed : status %s, "
			"size 0x%x\n",
	...
	ret = wdc_dump_dui_data(fd, WDC_NVME_CAP_DUI_HEADER_SIZE, 0x00,
				(__u8 *)log_hdr, last_xfer);
	if (ret != 0) {
	...
	if ((log_hdr->hdr_version & 0xFF) == 0x00 ||
		...
		if (verbose) {
			fprintf(stderr,
				"INFO : WDC : Capture V1 Device Unit Info log, "
				"data area = %d\n",
				data_area);
			fprintf(stderr,
				"INFO : WDC : DUI Header Version = 0x%x\n",
				log_hdr->hdr_version);
		}

		if (cap_dui_length == 0) {
			fprintf(stderr, "INFO : WDC : Capture V1 Device Unit "
					"Info log is empty\n");
		} else {
			/* parse log header for all sections up to specified
			 * data area inclusively */
			if (data_area != WDC_NVME_DUI_MAX_DATA_AREA) {
				for (j = 0; j < WDC_NVME_DUI_MAX_SECTION; j++) {
					if (log_hdr->log_section[j]
						    .data_area_id <=
						data_area &&
					    log_hdr->log_section[j]
						    .data_area_id != 0) {
						log_size +=
						    log_hdr->log_section[j]
							.section_size;
			...
			if (dump_data == NULL) {
				fprintf(stderr,
					"%s: ERROR : dump data V1 malloc "
					"failed : status %s, size = 0x%x\n",
			...
			/* write the telemetry and log headers into the
			 * dump_file */
			err = write(output, (void *)log_hdr,
				    WDC_NVME_CAP_DUI_HEADER_SIZE);
			if (err != WDC_NVME_CAP_DUI_HEADER_SIZE) {
				fprintf(stderr,
					"%s:  Failed to flush header data to "
					"file!\n",
					__func__);
				goto free_mem;
			}
			...
			for (; log_size > 0; log_size -= xfer_size) {
				...
				ret = wdc_dump_dui_data(fd, xfer_size,
							curr_data_offset,
							buffer_addr, last_xfer);
				if (ret != 0) {
					fprintf(stderr,
						"%s: ERROR : WDC : Get chunk "
						"%d, size = 0x%" PRIx64
						", offset = 0x%x, addr = %p\n",
						__func__, i, (uint64_t)log_size,
						curr_data_offset, buffer_addr);
					fprintf(stderr,
						"%s: ERROR : WDC : ", __func__);
					nvme_show_status(ret);
					break;
				}

				/* write the dump data into the file */
				err = write(output, (void *)buffer_addr,
					    xfer_size);
				if (err != xfer_size) {
					fprintf(
					    stderr,
					    "%s: ERROR : WDC : Failed to flush "
					    "DUI data to file! chunk %d, err = "
					    "0x%x, xfer_size = 0x%x\n",
		   ...
		   ((log_hdr->hdr_version & 0xFF) ==
		    0x03)) { /* Process Version 2 or 3 header */
		__s64 log_size = 0;
		...
		if (verbose) {
			fprintf(stderr,
				"INFO : WDC : Capture V2 or V3 Device Unit "
				"Info log, data area = %d\n",
				data_area);
			...
			if ((log_hdr->hdr_version & 0xFF) == 0x03)
				fprintf(
				    stderr,
				    "INFO : WDC : DUI Product ID = 0x%x/%c\n",
				    log_hdr_v3->product_id,
				    log_hdr_v3->product_id);
		}

		if (cap_dui_length_v3 == 0) {
			fprintf(stderr, "INFO : WDC : Capture V2 or V3 Device "
					"Unit Info log is empty\n");
		} else {
			/* parse log header for all sections up to specified
			 * data area inclusively */
			if (data_area != WDC_NVME_DUI_MAX_DATA_AREA) {
				for (j = 0; j < WDC_NVME_DUI_MAX_SECTION_V3;
				     j++) {
					if (log_hdr_v3->log_section[j]
						    .data_area_id <=
						data_area &&
					    log_hdr_v3->log_section[j]
						    .data_area_id != 0) {
						log_size +=
						    log_hdr_v3->log_section[j]
							.section_size;
			...
			if (offset >= total_size) {
				fprintf(stderr,
					"%s: INFO : WDC : Offset 0x%" PRIx64
					" exceeds total size 0x%" PRIx64
					", no data retrieved\n",
			...
			dump_data =
			    (__u8 *)malloc(sizeof(__u8) * xfer_size_long);
			if (dump_data == NULL) {
				fprintf(stderr,
					"%s: ERROR : dump data v3 malloc "
					"failed : status %s, size = 0x%" PRIx64
					"\n",
					__func__, strerror(errno),
			...
			if (file_size != 0) {
				/* Write the DUI data based on the passed in
				 * file size */
				if ((offset + file_size) > total_size)
					log_size = min((total_size - offset),
						       file_size);
				else
			...
			for (; log_size > 0; log_size -= xfer_size_long) {
				...
				ret = wdc_dump_dui_data_v2(
				    fd, (__u32)xfer_size_long, curr_data_offset,
				    buffer_addr, last_xfer);
				if (ret != 0) {
					fprintf(stderr,
						"%s: ERROR : WDC : Get chunk "
		...
		cap_dui_length_v4 = le64_to_cpu(log_hdr_v4->log_size_sectors) *
				    WDC_NVME_SN730_SECTOR_SIZE;

		if (verbose) {
			fprintf(stderr,
				"INFO : WDC : Capture V4 Device Unit Info log, "
				"data area = %d\n",
				data_area);
			fprintf(stderr,
				"INFO : WDC : DUI Header Version = 0x%x\n",
			...
			fprintf(stderr,
				"INFO : WDC : DUI log size sectors = 0x%x\n",
		...
		if (cap_dui_length_v4 == 0) {
			fprintf(stderr, "INFO : WDC : Capture V4 Device Unit "
					"Info log is empty\n");
		} else {
			/* parse log header for all sections up to specified
			 * data area inclusively */
			if (data_area != WDC_NVME_DUI_MAX_DATA_AREA) {
				for (j = 0; j < WDC_NVME_DUI_MAX_SECTION; j++) {
					if (log_hdr_v4->log_section[j]
						    .data_area_id <=
						data_area &&
					    log_hdr_v4->log_section[j]
						    .data_area_id != 0) {
						section_size_bytes =
						    ((__s64)log_hdr_v4
							 ->log_section[j]
							 .section_size_sectors *
						     WDC_NVME_SN730_SECTOR_SIZE);
			...
			if (dump_data == NULL) {
				fprintf(stderr,
					"%s: ERROR : dump data V4 malloc "
					"failed : status %s, size = 0x%x\n",
					__func__, strerror(errno),
					(unsigned int)xfer_size_long);
				ret = -1;
				goto out;
			}
			memset(dump_data, 0, sizeof(__u8) * xfer_size_long);
		...
		fprintf(stderr,
			"INFO : WDC : Unsupported header version = 0x%x\n",
			log_hdr->hdr_version);
		goto out;
	}
	...
	if (verbose)
		fprintf(stderr,
			"INFO : WDC : Capture Device Unit Info log, length = "
			"0x%" PRIx64 "\n",
			(uint64_t)total_size);
			...
			struct plugin *plugin)
{
	...
	struct config cfg = {.file = NULL, .xfer_size = 0x10000};

	OPT_ARGS(opts) = {OPT_FILE("output-file", 'o', &cfg.file, file),
...
static int wdc_do_get_sn730_log_len(int fd, uint32_t *len_buf,
				    uint32_t subopcode)
{
	int ret;
				...
				uint32_t subopcode)
{
	...
	if ((output = (uint8_t *)calloc(SN730_LOG_CHUNK_SIZE,
					sizeof(uint8_t))) == NULL) {
				...
				uint32_t subopcode)
{
	...
	while (remaining > 0) {
		...
		ret =
		    wdc_do_get_sn730_log(fd, chunk_buf, curr_offset, subopcode);
		if (!ret) {
			if (remaining >= SN730_LOG_CHUNK_SIZE) {
				memcpy(log_buf +
					   (curr_offset * SN730_LOG_CHUNK_SIZE),
...
static int wdc_do_sn730_get_and_tar(int fd, char *outputName)
{
	...
	wdc_UtilsSnprintf((char *)tarInfo->timeString, MAX_PATH_LEN,
			  "%02u%02u%02u_%02u%02u%02u", tarInfo->timeInfo.year,
			  tarInfo->timeInfo.month, tarInfo->timeInfo.dayOfMonth,
	...
	if (retPtr != NULL)
		wdc_UtilsSnprintf(
		    (char *)tarInfo->bufferFolderPath, MAX_PATH_LEN, "%s%s%s",
		    (char *)tarInfo->currDir, WDC_DE_PATH_SEPARATOR,
		    (char *)tarInfo->bufferFolderName);
	else {
		fprintf(stderr,
			"ERROR : WDC : get current working directory failed\n");
		goto free_buf;
	}
	...
	if (ret) {
		fprintf(stderr,
			"ERROR : WDC : create directory failed, ret = %d, dir "
			"= %s\n",
	...
	core_dump_log_buf =
	    (uint8_t *)calloc(core_dump_log_len, sizeof(uint8_t));
	...
	wdc_UtilsSnprintf(tarInfo->fileName, MAX_PATH_LEN, "%s%s%s_%s.bin",
			  (char *)tarInfo->bufferFolderPath,
			  WDC_DE_PATH_SEPARATOR, "full_log",
			  (char *)tarInfo->timeString);
	wdc_WriteToFile(tarInfo->fileName, (char *)full_log_buf, full_log_len);

	wdc_UtilsSnprintf(tarInfo->fileName, MAX_PATH_LEN, "%s%s%s_%s.bin",
			  (char *)tarInfo->bufferFolderPath,
			  WDC_DE_PATH_SEPARATOR, "key_log",
			  (char *)tarInfo->timeString);
	wdc_WriteToFile(tarInfo->fileName, (char *)key_log_buf, key_log_len);

	wdc_UtilsSnprintf(tarInfo->fileName, MAX_PATH_LEN, "%s%s%s_%s.bin",
			  (char *)tarInfo->bufferFolderPath,
			  WDC_DE_PATH_SEPARATOR, "core_dump_log",
			  (char *)tarInfo->timeString);
	...
	wdc_UtilsSnprintf(tarInfo->fileName, MAX_PATH_LEN, "%s%s%s_%s.bin",
			  (char *)tarInfo->bufferFolderPath,
			  WDC_DE_PATH_SEPARATOR, "extended_log",
			  (char *)tarInfo->timeString);
	...
	wdc_UtilsSnprintf(tarInfo->tarFileName, sizeof(tarInfo->tarFileName),
			  "%s%s", (char *)tarInfo->bufferFolderPath,
			  WDC_DE_TAR_FILE_EXTN);
	wdc_UtilsSnprintf(tarInfo->tarFiles, sizeof(tarInfo->tarFiles),
			  "%s%s%s", (char *)tarInfo->bufferFolderName,
			  WDC_DE_PATH_SEPARATOR, WDC_DE_TAR_FILES);
	wdc_UtilsSnprintf(tarInfo->tarCmd, sizeof(tarInfo->tarCmd), "%s %s %s",
			  WDC_DE_TAR_CMD, (char *)tarInfo->tarFileName,
			  (char *)tarInfo->tarFiles);

	ret = system(tarInfo->tarCmd);

	if (ret)
		fprintf(stderr,
			"ERROR : WDC : Tar of log data failed, ret = %d\n",
			ret);
...
static int wdc_vs_internal_fw_log(int argc, char **argv,
				  struct command *command,
				  struct plugin *plugin)
{
	...
	char *size = "Data retrieval transfer size.";
	char *data_area =
	    "Data area to retrieve up to. Currently only supported on the "
	    "SN340, SN640, SN730, and SN840 devices.";
	char *file_size =
	    "Output file size.  Currently only supported on the SN340 device.";
	...
	if (cfg.file != NULL) {
		 ...
		 * getting the dump data */
		verify_file =
		    open(cfg.file, O_WRONLY | O_CREAT | O_TRUNC, 0666);
		if (verify_file < 0) {
			fprintf(stderr, "ERROR : WDC: open : %s\n",
				strerror(errno));
			return -1;
		...
		wdc_UtilsSnprintf((char *)timeStamp, MAX_PATH_LEN,
				  ...
				  timeInfo.second);
		snprintf(fileSuffix, PATH_MAX, "_internal_fw_log_%s",
			 (char *)timeStamp);

		if (wdc_get_serial_name(fd, f, PATH_MAX, fileSuffix) == -1) {
			fprintf(stderr,
				"ERROR : WDC: failed to generate file name\n");
			return -1;
		}
	}
	...
	if ((capabilities & WDC_DRIVE_CAP_INTERNAL_LOG) ==
		...
		if (cfg.data_area == 0)
			cfg.data_area =
			    3; /* Set the default DA to 3 if not specified */

		if ((cfg.type == NULL) || (!strcmp(cfg.type, "NONE")) ||
			...
			fprintf(stderr, "ERROR : WDC: Invalid type - Must be "
					"NONE, HOST or CONTROLLER\n");
			return -1;
		}

		return wdc_do_cap_diag(r, fd, f, xfer_size, telemetry_type,
				       telemetry_data_area);
	}
	...
	if ((capabilities & WDC_DRIVE_CAP_DUI_DATA) == WDC_DRIVE_CAP_DUI_DATA)
		return wdc_do_cap_dui(fd, f, xfer_size,
				      WDC_NVME_DUI_MAX_DATA_AREA, cfg.verbose,
				      0, 0);
...
static int wdc_do_crash_dump(int fd, char *file, int type)
{
	...
	if (type == WDC_NVME_PFAIL_DUMP_TYPE) {
		...
		cdw12_size = ((WDC_NVME_PF_CRASH_DUMP_SIZE_SUBCMD
			       << WDC_NVME_SUBCMD_SHIFT) |
			      WDC_NVME_PF_CRASH_DUMP_SIZE_CMD);

		cdw12 =
		    (WDC_NVME_PF_CRASH_DUMP_SUBCMD << WDC_NVME_SUBCMD_SHIFT) |
		    WDC_NVME_PF_CRASH_DUMP_CMD;

		cdw12_clear = ((WDC_NVME_CLEAR_PF_CRASH_DUMP_SUBCMD
				<< WDC_NVME_SUBCMD_SHIFT) |
		...
		cdw12_size = ((WDC_NVME_CRASH_DUMP_SIZE_SUBCMD
			       << WDC_NVME_SUBCMD_SHIFT) |
		...
		cdw12_clear = ((WDC_NVME_CLEAR_CRASH_DUMP_SUBCMD
				<< WDC_NVME_SUBCMD_SHIFT) |
			       WDC_NVME_CLEAR_CRASH_DUMP_CMD);
	}

	ret = wdc_dump_length(fd, opcode, cdw10_size, cdw12_size,
			      &crash_dump_length);

	if (ret == -1) {
		if (type == WDC_NVME_PFAIL_DUMP_TYPE)
			fprintf(stderr,
				"INFO : WDC: Pfail dump get size failed\n");
		else
			fprintf(stderr,
				"INFO : WDC: Crash dump get size failed\n");
	...
	if (crash_dump_length == 0) {
		...
		ret = wdc_do_dump(fd, opcode, crash_dump_length, cdw12, file,
				  crash_dump_length);

		if (ret == 0)
			ret = wdc_do_clear_dump(fd, WDC_NVME_CLEAR_DUMP_OPCODE,
						cdw12_clear);
	}
	return ret;
}
...
static int wdc_do_drive_log(int fd, char *file)
{
	...
	ret = wdc_dump_length(
	    fd, WDC_NVME_DRIVE_LOG_SIZE_OPCODE, WDC_NVME_DRIVE_LOG_SIZE_NDT,
	    (WDC_NVME_DRIVE_LOG_SIZE_SUBCMD << WDC_NVME_SUBCMD_SHIFT |
	     WDC_NVME_DRIVE_LOG_SIZE_CMD),
	    &drive_log_length);
	if (ret == -1) {
		return -1;
	}
	...
	admin_cmd.cdw12 =
	    ((WDC_NVME_DRIVE_LOG_SUBCMD << WDC_NVME_SUBCMD_SHIFT) |
	     WDC_NVME_DRIVE_LOG_SIZE_CMD);
	...
	if (ret == 0) {
		ret =
		    wdc_create_log_file(file, drive_log_data, drive_log_length);
	}
	free(drive_log_data);
	return ret;
}
			      ...
			      struct plugin *plugin)
{
	...
	if ((capabilities & WDC_DRIVE_CAP_CRASH_DUMP) == 0) {
		...
		if (ret != 0) {
			fprintf(stderr,
				"ERROR : WDC : failed to read crash dump\n");
		}
	}
	nvme_free_tree(r);
	return ret;
			      ...
			      struct plugin *plugin)
{
	...
	if ((capabilities & WDC_DRIVE_CAP_PFAIL_DUMP) == 0) {
		...
		if (ret != 0) {
			fprintf(
			    stderr,
			    "ERROR : WDC : failed to read pfail crash dump\n");
		}
	}
	nvme_free_tree(r);
	return ret;
...
static void wdc_do_id_ctrl(__u8 *vs, struct json_object *root)
{
	...
	if (root)
		json_object_add_value_string(root, "wdc vsn",
					     strlen(vsn) > 1 ? vsn : "NULL");
	else
		printf("wdc vsn : %s\n", strlen(vsn) > 1 ? vsn : "NULL");
}

static int wdc_id_ctrl(int argc, char **argv, struct command *cmd,
		       struct plugin *plugin)
{
	...
	if ((capabilities & WDC_DRIVE_CAP_PURGE) == 0) {
		...
		if (ret > 0) {
			...
			case WDC_NVME_PURGE_INT_DEV_ERR:
				err_str =
				    "ERROR : WDC : Internal Device Error.\n";
				break;
			default:
				err_str = "ERROR : WDC\n";
			}
...
static void wdc_print_log_normal(struct wdc_ssd_perf_stats *perf)
{
	...
	printf("  Average Read Size                              %20lf\n",
	       safe_div_fp((le64_to_cpu(perf->hr_blks)),
			   (le64_to_cpu(perf->hr_cmds))));
	...
	printf("  Host Read Commands Stalled Percentage          %20" PRIu64
	       "%%\n",
	       (uint64_t)calc_percent((le64_to_cpu(perf->hr_st_cmds)),
				      le64_to_cpu(perf->hr_cmds)));
	...
	printf("  Average Write Size                             %20f\n",
	       safe_div_fp((le64_to_cpu(perf->hw_blks)),
			   (le64_to_cpu(perf->hw_cmds))));
	...
	printf("  Host Write Odd Start Commands Percentage       %20" PRIu64
	       "%%\n",
	       (uint64_t)calc_percent((le64_to_cpu(perf->hw_os_cmds)),
				      (le64_to_cpu(perf->hw_cmds))));
	...
	printf("  Host Write Odd End Commands Percentage         %20" PRIu64
	       "%%\n",
	       (uint64_t)calc_percent((le64_to_cpu(perf->hw_oe_cmds)),
				      (le64_to_cpu((perf->hw_cmds)))));
	...
	printf("  Host Write Commands Stalled Percentage         %20" PRIu64
	       "%%\n",
	       (uint64_t)calc_percent((le64_to_cpu(perf->hw_st_cmds)),
				      (le64_to_cpu(perf->hw_cmds))));
	...
	printf("  Average NAND Read Size                         %20f\n",
	       safe_div_fp((le64_to_cpu(perf->nr_blks)),
			   (le64_to_cpu((perf->nr_cmds)))));
	...
	printf("  Average NAND Write Size                        %20f\n",
	       safe_div_fp((le64_to_cpu(perf->nw_blks)),
			   (le64_to_cpu(perf->nw_cmds))));
...
static void wdc_print_log_json(struct wdc_ssd_perf_stats *perf)
{
	...
	json_object_add_value_int(root, "Host Read Commands",
				  le64_to_cpu(perf->hr_cmds));
	json_object_add_value_int(root, "Host Read Blocks",
				  le64_to_cpu(perf->hr_blks));
	...
	json_object_add_value_int(
	    root, "Host Read Cache Hit Percentage",
	    (uint64_t)calc_percent(le64_to_cpu(perf->hr_ch_cmds),
				   le64_to_cpu(perf->hr_cmds)));
	...
	json_object_add_value_int(root, "Host Read Commands Stalled",
				  le64_to_cpu(perf->hr_st_cmds));
	json_object_add_value_int(
	    root, "Host Read Commands Stalled Percentage",
	    (uint64_t)calc_percent((le64_to_cpu(perf->hr_st_cmds)),
				   le64_to_cpu(perf->hr_cmds)));
	...
	json_object_add_value_int(root, "Host Write Odd Start Commands",
				  le64_to_cpu(perf->hw_os_cmds));
	json_object_add_value_int(
	    root, "Host Write Odd Start Commands Percentage",
	    (uint64_t)calc_percent((le64_to_cpu(perf->hw_os_cmds)),
				   (le64_to_cpu(perf->hw_cmds))));
	json_object_add_value_int(root, "Host Write Odd End Commands",
				  le64_to_cpu(perf->hw_oe_cmds));
	json_object_add_value_int(
	    root, "Host Write Odd End Commands Percentage",
	    (uint64_t)calc_percent((le64_to_cpu(perf->hw_oe_cmds)),
				   (le64_to_cpu((perf->hw_cmds)))));
	json_object_add_value_int(root, "Host Write Commands Stalled",
				  le64_to_cpu(perf->hw_st_cmds));
	json_object_add_value_int(
	    root, "Host Write Commands Stalled Percentage",
	    (uint64_t)calc_percent((le64_to_cpu(perf->hw_st_cmds)),
				   (le64_to_cpu(perf->hw_cmds))));
...
static int wdc_print_log(struct wdc_ssd_perf_stats *perf, int fmt)
{
	if (!perf) {
		fprintf(stderr,
			"ERROR : WDC : Invalid buffer to read perf stats\n");
		return -1;
	}
	switch (fmt) {
	case NORMAL:
...
static int wdc_print_latency_monitor_log_normal(
    int fd, struct wdc_ssd_latency_monitor_log *log_data)
{
	...
	if (!err)
		...
		fprintf(stderr,
			"ERROR : WDC : latency monitor read id ctrl failure, "
			"err = %d\n",
			err);
		return err;
	}
	...
	printf("                                                            "
	       ...
	       "Deallocate/Trim \n");
	for (i = 0; i <= 3; i++) {
		printf("  Active Bucket Counter: Bucket %d    %27d     %27d    "
		       " %27d \n",
		       i, le32_to_cpu(log_data->active_bucket_counter[i][READ]),
		       le32_to_cpu(log_data->active_bucket_counter[i][WRITE]),
		       le32_to_cpu(log_data->active_bucket_counter[i][TRIM]));
	}

	for (i = 0; i <= 3; i++) {
		printf("  Active Measured Latency: Bucket %d  %27d ms  %27d ms "
		       " %27d ms \n",
		       i,
	...
	for (i = 0; i <= 3; i++) {
		...
		for (j = 0; j <= 2; j++) {
			if (le64_to_cpu(
				log_data->active_latency_timestamp[i][j]) == -1)
				printf("                    N/A         ");
			else {
				wdc_convert_ts(
				    le64_to_cpu(
					log_data
					    ->active_latency_timestamp[i][j]),
				    ts_buf);
				printf("%s     ", ts_buf);
			}
		}
	...
	for (i = 0; i <= 3; i++) {
		printf("  Static Bucket Counter: Bucket %d    %27d     %27d    "
		       " %27d \n",
		       i, le32_to_cpu(log_data->static_bucket_counter[i][READ]),
		       le32_to_cpu(log_data->static_bucket_counter[i][WRITE]),
		       le32_to_cpu(log_data->static_bucket_counter[i][TRIM]));
	}

	for (i = 0; i <= 3; i++) {
		printf("  Static Measured Latency: Bucket %d  %27d ms  %27d ms "
		       " %27d ms \n",
		       i,
	...
	for (i = 0; i <= 3; i++) {
		...
		for (j = 0; j <= 2; j++) {
			if (le64_to_cpu(
				log_data->static_latency_timestamp[i][j]) == -1)
				printf("                    N/A         ");
			else {
				wdc_convert_ts(
				    le64_to_cpu(
					log_data
					    ->static_latency_timestamp[i][j]),
				    ts_buf);
				printf("%s     ", ts_buf);
			}
		}
...
static void
wdc_print_latency_monitor_log_json(struct wdc_ssd_latency_monitor_log *log_data)
{
	int i, j;
	char buf[128];
	...
	json_object_add_value_int(root, "Feature Status",
				  log_data->feature_status);
	json_object_add_value_int(
	    root, "Active Bucket Timer",
	    5 * le16_to_cpu(log_data->active_bucket_timer));
	json_object_add_value_int(
	    root, "Active Bucket Timer Threshold",
	    5 * le16_to_cpu(log_data->active_bucket_timer_threshold));
	json_object_add_value_int(
	    root, "Active Threshold A",
	    5 * le16_to_cpu(log_data->active_threshold_a + 1));
	json_object_add_value_int(
	    root, "Active Threshold B",
	    5 * le16_to_cpu(log_data->active_threshold_b + 1));
	json_object_add_value_int(
	    root, "Active Threshold C",
	    5 * le16_to_cpu(log_data->active_threshold_c + 1));
	json_object_add_value_int(
	    root, "Active Threshold D",
	    5 * le16_to_cpu(log_data->active_threshold_d + 1));
	json_object_add_value_int(root, "Active Latency Config",
				  le16_to_cpu(log_data->active_latency_config));
	json_object_add_value_int(root, "Active Lantency Minimum Window",
				  100 * log_data->active_latency_min_window);
	json_object_add_value_int(
	    root, "Active Latency Stamp Units",
	    le16_to_cpu(log_data->active_latency_stamp_units));
	json_object_add_value_int(
	    root, "Static Latency Stamp Units",
	    le16_to_cpu(log_data->static_latency_stamp_units));
	json_object_add_value_int(
	    root, "Debug Log Trigger Enable",
	    le16_to_cpu(log_data->debug_log_trigger_enable));

	for (i = 0; i <= 3; i++) {
		for (j = 0; j <= 2; j++) {
			sprintf(buf, "Active Bucket Counter: Bucket %d %s", i,
				operation[j]);
			json_object_add_value_int(
			    root, buf,
	...
	for (i = 0; i <= 3; i++) {
		for (j = 0; j <= 2; j++) {
			sprintf(buf, "Active Measured Latency: Bucket %d %s", i,
				operation[j]);
			json_object_add_value_int(
			    root, buf,
			    le16_to_cpu(
	...
	for (i = 0; i <= 3; i++) {
		for (j = 0; j <= 2; j++) {
			sprintf(buf, "Active Latency Time Stamp: Bucket %d %s",
				i, operation[j]);
			json_object_add_value_int(
			    root, buf,
			    le64_to_cpu(
	...
	for (i = 0; i <= 3; i++) {
		for (j = 0; j <= 2; j++) {
			sprintf(buf, "Static Bucket Counter: Bucket %d %s", i,
				operation[j]);
			json_object_add_value_int(
			    root, buf,
	...
	for (i = 0; i <= 3; i++) {
		for (j = 0; j <= 2; j++) {
			sprintf(buf, "Static Measured Latency: Bucket %d %s", i,
				operation[j]);
			json_object_add_value_int(
			    root, buf,
			    le16_to_cpu(
	...
	for (i = 0; i <= 3; i++) {
		for (j = 0; j <= 2; j++) {
			sprintf(buf, "Static Latency Time Stamp: Bucket %d %s",
				i, operation[j]);
			json_object_add_value_int(
			    root, buf,
			    le64_to_cpu(
...
static void wdc_print_fb_ca_log_normal(struct wdc_ssd_ca_perf_stats *perf)
{
	...
	printf("  NAND Bytes Written                             %20" PRIu64
	       "%20" PRIu64 "\n",
	       le64_to_cpu(perf->nand_bytes_wr_hi),
	       le64_to_cpu(perf->nand_bytes_wr_lo));
	printf("  NAND Bytes Read                                %20" PRIu64
	       "%20" PRIu64 "\n",
...
static void wdc_print_fb_ca_log_json(struct wdc_ssd_ca_perf_stats *perf)
{
	...
	json_object_add_value_int(root, "NAND Bytes Written Lo",
				  le64_to_cpu(perf->nand_bytes_wr_lo));
	json_object_add_value_int(root, "NAND Bytes Read Hi",
				  le64_to_cpu(perf->nand_bytes_rd_hi));
	json_object_add_value_int(root, "NAND Bytes Read Lo",
				  le64_to_cpu(perf->nand_bytes_rd_lo));
	...
	json_object_add_value_int(root, "Uncorrectable Read Count",
				  le64_to_cpu(perf->uncorr_read_count));
	json_object_add_value_int(root, "Soft ECC Error Count",
				  le64_to_cpu(perf->ecc_error_count));
	json_object_add_value_int(root,
				  "SSD End to End Detected Correction Count",
				  le32_to_cpu(perf->ssd_detect_count));
	json_object_add_value_int(root,
				  "SSD End to End Corrected Correction Count",
	...
	json_object_add_value_int(root, "Refresh Count",
				  le64_to_cpu(perf->refresh_count));
	...
	converted = le64_to_cpu(perf->user_erase_fail);
	json_object_add_value_int(root,
				  "User Data Erase Fail Count (Normalized)",
				  converted & 0xFFFF);
	...
	json_object_add_value_int(root, "PCIe Correctable Error",
				  le64_to_cpu(perf->pcie_corr_error));
	json_object_add_value_int(root, "Incomplete Shutdown Counte",
				  le32_to_cpu(perf->incomplete_shutdown_count));
	json_object_add_value_int(root, "Percent Free Blocks",
				  perf->percent_free_blocks);
	json_print_object(root, NULL);
	printf("\n");
	json_free_object(root);
...
{
	struct wdc_bd_ca_log_format *bd_data =
	    (struct wdc_bd_ca_log_format *)data;
	__u64 *raw;
	...
	if (bd_data->field_id == 0x00) {
		...
		printf(
		    "Additional Smart Log for NVME device:%s namespace-id:%x\n",
		    devicename, WDC_DE_GLOBAL_NSID);
		printf("key                               normalized raw\n");
		printf("program_fail_count              : %3" PRIu8
		       "%%       %" PRIu64 "\n",
	...
	if (bd_data->field_id == 0x01) {
		raw = (__u64 *)bd_data->raw_value;
		printf("erase_fail_count                : %3" PRIu8
		       "%%       %" PRIu64 "\n",
		       bd_data->normalized_value,
		       le64_to_cpu(*raw & 0x00FFFFFFFFFFFFFF));
	} else {
	...
	if (bd_data->field_id == 0x02) {
		...
		printf("wear_leveling                   : %3" PRIu8
		       "%%       min: %" PRIu16 ", max: %" PRIu16
		       ", avg: %" PRIu16 "\n",
		       bd_data->normalized_value, le16_to_cpu(*word_raw1),
		       le16_to_cpu(*word_raw2), le16_to_cpu(*word_raw3));
	} else {
		goto invalid_id;
	}
	bd_data++;
	if (bd_data->field_id == 0x03) {
		raw = (__u64 *)bd_data->raw_value;
		printf("end_to_end_error_detection_count: %3" PRIu8
		       "%%       %" PRIu64 "\n",
		       bd_data->normalized_value,
		       le64_to_cpu(*raw & 0x00FFFFFFFFFFFFFF));
	} else {
	...
	if (bd_data->field_id == 0x04) {
		raw = (__u64 *)bd_data->raw_value;
		printf("crc_error_count                 : %3" PRIu8
		       "%%       %" PRIu64 "\n",
		       bd_data->normalized_value,
		       le64_to_cpu(*raw & 0x00FFFFFFFFFFFFFF));
	} else {
	...
	if (bd_data->field_id == 0x05) {
		raw = (__u64 *)bd_data->raw_value;
		printf("timed_workload_media_wear       : %3" PRIu8
		       "%%       %-.3f%%\n",
		       bd_data->normalized_value,
	...
	if (bd_data->field_id == 0x06) {
		raw = (__u64 *)bd_data->raw_value;
		printf("timed_workload_host_reads       : %3" PRIu8
		       "%%       %" PRIu64 "%%\n",
		       bd_data->normalized_value,
		       le64_to_cpu(*raw & 0x00FFFFFFFFFFFFFF));
	} else {
	...
	if (bd_data->field_id == 0x07) {
		raw = (__u64 *)bd_data->raw_value;
		printf("timed_workload_timer            : %3" PRIu8
		       "%%       %" PRIu64 "\n",
		       bd_data->normalized_value,
		       le64_to_cpu(*raw & 0x00FFFFFFFFFFFFFF));
	} else {
	...
	if (bd_data->field_id == 0x08) {
		...
		printf("thermal_throttle_status         : %3" PRIu8
		       "%%       %" PRIu16 "%%, cnt: %" PRIu16 "\n",
	...
	if (bd_data->field_id == 0x09) {
		raw = (__u64 *)bd_data->raw_value;
		printf("retry_buffer_overflow_count     : %3" PRIu8
		       "%%       %" PRIu64 "\n",
		       bd_data->normalized_value,
		       le64_to_cpu(*raw & 0x00FFFFFFFFFFFFFF));
	} else {
	...
	if (bd_data->field_id == 0x0A) {
		raw = (__u64 *)bd_data->raw_value;
		printf("pll_lock_loss_count             : %3" PRIu8
		       "%%       %" PRIu64 "\n",
		       bd_data->normalized_value,
		       le64_to_cpu(*raw & 0x00FFFFFFFFFFFFFF));
	} else {
	...
	if (bd_data->field_id == 0x0B) {
		raw = (__u64 *)bd_data->raw_value;
		printf("nand_bytes_written              : %3" PRIu8
		       "%%       sectors: %.f\n",
		       bd_data->normalized_value,
	...
	if (bd_data->field_id == 0x0C) {
		raw = (__u64 *)bd_data->raw_value;
		printf("host_bytes_written              : %3" PRIu8
		       "%%       sectors: %.f\n",
		       bd_data->normalized_value,
		       safe_div_fp((*raw & 0x00FFFFFFFFFFFFFF), 0xFFFF));
		raw = (__u64 *)bd_data->raw_value;
...
static void wdc_print_bd_ca_log_json(void *data)
{
	...
	if (bd_data->field_id == 0x03) {
		raw = (__u64 *)bd_data->raw_value;
		json_object_add_value_int(
		    root, "end_to_end_error_detection_count",
		    le64_to_cpu(*raw & 0x00FFFFFFFFFFFFFF));
	} else {
		goto invalid_id;
	}
	...
	if (bd_data->field_id == 0x05) {
		...
		json_object_add_value_float(
		    root, "timed_workload_media_wear",
		    safe_div_fp((*raw & 0x00FFFFFFFFFFFFFF), 1024.0));
	} else {
		goto invalid_id;
	}
	...
	if (bd_data->field_id == 0x06) {
		raw = (__u64 *)bd_data->raw_value;
		json_object_add_value_int(
		    root, "timed_workload_host_reads",
	...
	if (bd_data->field_id == 0x09) {
		raw = (__u64 *)bd_data->raw_value;
		json_object_add_value_int(
		    root, "retry_buffer_overflow_count",
		    le64_to_cpu(*raw & 0x00FFFFFFFFFFFFFF));
	} else {
		goto invalid_id;
	}
...
static void wdc_print_d0_log_normal(struct wdc_ssd_d0_smart_log *perf)
{
	...
	printf(
	    "  Lifetime Read Disturb Reallocation Events	 %20" PRIu32
	    "\n",
...
static void wdc_print_d0_log_json(struct wdc_ssd_d0_smart_log *perf)
{
	...
	root = json_create_object();
	json_object_add_value_int(
	    root, "Lifetime Reallocated Erase Block Count",
	...
	json_object_add_value_int(
	    root, "Trailing Hour Write Amplification Factor",
	...
	json_object_add_value_int(
	    root, "Lifetime Retired Block Count",
	    le32_to_cpu(perf->lifetime_retired_block_count));
	json_object_add_value_int(
	    root, "Lifetime Read Disturb Reallocation Events",
	...
	json_object_add_value_int(root, "Lifetime User Reads",
				  le64_to_cpu(perf->lifetime_user_reads));
	json_object_add_value_int(
	    root, "Lifetime Thermal Throttle Activations",
	    le32_to_cpu(perf->lifetime_thermal_throttle_act));
	json_object_add_value_int(
	    root, "Percentage of P/E Cycles Remaining",
						...
						__u32 cust_id, __u32 vendor_id)
{
	...
	if (data[0] == WDC_NVME_GET_FW_ACT_HISTORY_C2_LOG_ID) {
		...
		if (cust_id == WDC_CUSTOMER_ID_0x1005 ||
			...
			printf(
			    "           Power on Hour       Power Cycle     "
			    "Previous    New                               \n");
			printf(
			    "  Entry      hh:mm:ss             Count        "
			    "Firmware    Firmware    Slot   Action  Result \n");
			printf(
			    "  -----  -----------------  -----------------  "
			    "---------   ---------   -----  ------  -------\n");
		} else {
		...
		struct wdc_fw_act_history_log_format_c2 *fw_act_history_entry =
		    (struct wdc_fw_act_history_log_format_c2 *)(data);

		if (num_entries == WDC_MAX_NUM_ACT_HIST_ENTRIES) {
			/* find lowest/oldest entry */
			for (i = 0; i < num_entries; i++) {
				j = (i + 1 == WDC_MAX_NUM_ACT_HIST_ENTRIES)
					? 0
					: i + 1;
				if (le16_to_cpu(fw_act_history_entry->entry[i]
						    .fw_act_hist_entries) >
				    le16_to_cpu(fw_act_history_entry->entry[j]
						    .fw_act_hist_entries)) {
					oldestEntryIdx = j;
					break;
				}
		...
		for (i = 0; i < num_entries; i++) {
			...
			memcpy(previous_fw,
			       (char *)&(fw_act_history_entry->entry[entryIdx]
					     .previous_fw_version),
			       8);
			if (strlen(
				(char *)&(fw_act_history_entry->entry[entryIdx]
				...
				memcpy(new_fw,
				       (char *)&(
					   fw_act_history_entry->entry[entryIdx]
					       .current_fw_version),
				       8);
			else
			...
			if (cust_id == WDC_CUSTOMER_ID_0x1005) {
				...
				sprintf((char *)time_str, "%04d:%02d:%02d",
					(int)(le64_to_cpu(fw_act_history_entry
							      ->entry[entryIdx]
							      .timestamp) /
					      3600),
					(int)((le64_to_cpu(fw_act_history_entry
			...
			printf("     ");
			printf("%2" PRIu8 "",
			       (uint8_t)fw_act_history_entry->entry[entryIdx]
				   .slot_number);
			printf("   ");
			...
			printf("  ");
			if (le16_to_cpu(
				fw_act_history_entry->entry[entryIdx].result) ==
			    0)
				printf("pass");
			else
				printf("fail #%d",
		...
		printf("  Firmware Activate History Log \n");
		printf("         Power on Hour   Power Cycle           "
		       "Previous    New                               \n");
		printf("  Entry    hh:mm:ss      Count                 "
		       "Firmware    Firmware    Slot   Action  Result \n");
		printf("  -----  --------------  --------------------  "
		       "----------  ----------  -----  ------  -------\n");
		...
		struct wdc_fw_act_history_log_entry *fw_act_history_entry =
		    (struct wdc_fw_act_history_log_entry
			 *)(data + sizeof(struct wdc_fw_act_history_log_hdr));

		if (num_entries == WDC_MAX_NUM_ACT_HIST_ENTRIES) {
			/* find lowest/oldest entry */
			for (i = 0; i < num_entries; i++) {
				if (le32_to_cpu(
					fw_act_history_entry[i].entry_num) >
				    le32_to_cpu(fw_act_history_entry[i + 1]
						    .entry_num)) {
					oldestEntryIdx = i + 1;
					break;
				}
		...
		for (i = 0; i < num_entries; i++) {
			...
			memcpy(previous_fw,
			       (char *)&(fw_act_history_entry[entryIdx]
					     .previous_fw_version),
			       8);
			if (strlen((char *)&(fw_act_history_entry[entryIdx]
						 .new_fw_version)) > 1)
				memcpy(new_fw,
				       (char *)&(fw_act_history_entry[entryIdx]
						     .new_fw_version),
			...
			printf("%5" PRIu32 "",
			       (uint32_t)le32_to_cpu(
				   fw_act_history_entry[entryIdx].entry_num));
			printf("      ");
			printf("%04d:%02d:%02d",
			       (int)(le64_to_cpu(fw_act_history_entry[entryIdx]
						     .power_on_seconds) /
				     3600),
			       (int)((le64_to_cpu(fw_act_history_entry[entryIdx]
						      .power_on_seconds) %
			...
			printf(
			    "%16" PRIu32 "",
			    (uint32_t)le32_to_cpu(fw_act_history_entry[entryIdx]
						      .power_cycle_count));
			printf("     ");
			printf("%s", (char *)previous_fw);
			printf("    ");
			...
			printf("     ");
			printf("%2" PRIu8 "",
			       (uint8_t)fw_act_history_entry[entryIdx]
				   .slot_number);
			printf("  ");
			wdc_get_commit_action_bin(
			...
			printf("   ");
			if (le16_to_cpu(
				fw_act_history_entry[entryIdx].result) == 0)
				printf("pass");
			else
				printf(
				    "fail #%d",
				    (uint16_t)le16_to_cpu(
					fw_act_history_entry[entryIdx].result));
					      ...
					      __u32 cust_id, __u32 vendor_id)
{
	...
	if (data[0] == WDC_NVME_GET_FW_ACT_HISTORY_C2_LOG_ID) {
		...
		for (i = 0; i < num_entries; i++) {
			...
			json_object_add_value_int(
			    root, "Entry",
			    le16_to_cpu(fw_act_history_entry->entry[entryIdx]
					    .fw_act_hist_entries));
			...
			if (cust_id == WDC_CUSTOMER_ID_0x1005) {
				...
				uint64_t timestamp =
				    (0x0000FFFFFFFFFFFF &
					 ...
					 fw_act_history_entry->entry[entryIdx]
					     .timestamp));
				json_object_add_value_int(root, "Timestamp",
							  timestamp);
			}

			json_object_add_value_int(
			    root, "Power Cycle Count",
			    le64_to_cpu(fw_act_history_entry->entry[entryIdx]
					    .power_cycle_count));
			...
			wdc_get_commit_action_bin(
				...
				.commit_action_type,
			    (char *)&commit_action_bin);
			json_object_add_value_string(root, "Action",
						     commit_action_bin);

			if (le16_to_cpu(
				fw_act_history_entry->entry[entryIdx].result) ==
			    0)
				json_object_add_value_string(root, "Result",
							     "pass");
			else {
				sprintf((char *)fail_str, "fail #%d",
					(int)(le16_to_cpu(fw_act_history_entry
							      ->entry[entryIdx]
							      .result)));
				json_object_add_value_string(root, "Result",
							     fail_str);
		...
		for (i = 0; i < num_entries; i++) {
			...
			sprintf(
			    ...
			    (int)(le64_to_cpu(fw_act_history_entry[entryIdx]
						  .power_on_seconds) %
				  60));
			json_object_add_value_string(root, "Power on Hour",
						     time_str);

			json_object_add_value_int(
			    root, "Power Cycle Count",
			    le32_to_cpu(fw_act_history_entry[entryIdx]
					    .power_cycle_count));
			json_object_add_value_string(root, "Previous Firmware",
						     previous_fw);
			json_object_add_value_string(root, "New Firmware",
...
static void wdc_print_smart_cloud_attr_C0_normal(void *data)
{
	...
	printf("  Bad user nand blocks Raw			: %" PRIu64
	       "\n",
	       (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_BUNBR] &
				     0x0000FFFFFFFFFFFF));
	printf("  Bad user nand blocks Normalized		: %d\n",
	       (uint16_t)le16_to_cpu(*(uint16_t *)&log_data[SCAO_BUNBN]));
	printf("  Bad system nand blocks Raw			: %" PRIu64
	       "\n",
	       (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_BSNBR] &
				     0x0000FFFFFFFFFFFF));
	...
	printf("  Refresh counts				: %" PRIu64
	       "\n",
	       (uint64_t)(le64_to_cpu(*(uint64_t *)&log_data[SCAO_RFSC]) &
			  0x00FFFFFFFFFFFFFF));
	...
	if (smart_log_ver > 2) {
		...
		printf("  Point Version Field				: "
		       "%" PRIu16 "\n",
		       (uint16_t)log_data[SCAO_PVF]);
		printf("  Minor Version Field				: "
		       "%" PRIu16 "\n",
...
static void wdc_print_smart_cloud_attr_C0_json(void *data)
{
	...
	json_object_add_value_uint64(
	    root, "Physical media units written hi",
	    (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_PMUW + 8] &
				  0xFFFFFFFFFFFFFFFF));
	json_object_add_value_uint64(
	    root, "Physical media units written lo",
	    (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_PMUW] &
				  0xFFFFFFFFFFFFFFFF));
	json_object_add_value_uint64(
	    root, "Physical media units read hi",
	    (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_PMUR + 8] &
				  0xFFFFFFFFFFFFFFFF));
	json_object_add_value_uint64(
	    root, "Physical media units read lo",
	    (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_PMUR] &
				  0xFFFFFFFFFFFFFFFF));
	json_object_add_value_uint64(
	    root, "Bad user nand blocks - Raw",
	    (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_BUNBR] &
				  0x0000FFFFFFFFFFFF));
	json_object_add_value_uint(
	    root, "Bad user nand blocks - Normalized",
	    (uint16_t)le16_to_cpu(*(uint16_t *)&log_data[SCAO_BUNBN]));
	json_object_add_value_uint64(
	    root, "Bad system nand blocks - Raw",
	    (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_BSNBR] &
				  0x0000FFFFFFFFFFFF));
	json_object_add_value_uint(
	    root, "Bad system nand blocks - Normalized",
	    (uint16_t)le16_to_cpu(*(uint16_t *)&log_data[SCAO_BSNBN]));
	json_object_add_value_uint64(
	    root, "XOR recovery count",
	    (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_XRC]));
	json_object_add_value_uint64(
	    root, "Uncorrectable read error count",
	    (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_UREC]));
	json_object_add_value_uint64(
	    root, "Soft ecc error count",
	    (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_SEEC]));
	json_object_add_value_uint(
	    root, "End to end corrected errors",
	    (uint32_t)le32_to_cpu(*(uint32_t *)&log_data[SCAO_EECE]));
	json_object_add_value_uint(
	    root, "End to end detected errors",
	    (uint32_t)le32_to_cpu(*(uint32_t *)&log_data[SCAO_EEDC]));
	...
	json_object_add_value_uint64(
	    ...
	    (uint64_t)(le64_to_cpu(*(uint64_t *)&log_data[SCAO_RFSC]) &
		       0x00FFFFFFFFFFFFFF));
	json_object_add_value_uint(
	    root, "Max User data erase counts",
	    (uint32_t)le32_to_cpu(*(uint32_t *)&log_data[SCAO_MXUDEC]));
	json_object_add_value_uint(
	    root, "Min User data erase counts",
	    (uint32_t)le32_to_cpu(*(uint32_t *)&log_data[SCAO_MNUDEC]));
	...
	json_object_add_value_uint(root, "Current throttling status",
				   (__u8)log_data[SCAO_CTS]);
	json_object_add_value_uint64(
	    root, "PCIe correctable error count",
	    (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_PCEC]));
	json_object_add_value_uint(
	    root, "Incomplete shutdowns",
	    (uint32_t)le32_to_cpu(*(uint32_t *)&log_data[SCAO_ICS]));
	json_object_add_value_uint(root, "Percent free blocks",
				   (__u8)log_data[SCAO_PFB]);
	json_object_add_value_uint(
	    root, "Capacitor health",
	    (uint16_t)le16_to_cpu(*(uint16_t *)&log_data[SCAO_CPH]));
	json_object_add_value_uint64(
	    root, "Unaligned I/O",
	    (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_UIO]));
	json_object_add_value_uint64(
	    root, "Security Version Number",
	    (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_SVN]));
	json_object_add_value_uint64(
	    root, "NUSE - Namespace utilization",
	    (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_NUSE]));
	...
	if (smart_log_ver > 2) {
		...
		json_object_add_value_uint(root, "NVMe Errata Version",
					   (__u8)log_data[SCAO_NEV]);
		json_object_add_value_uint64(
		    root, "PCIe Link Retraining Count",
		    (uint64_t)le64_to_cpu(*(uint64_t *)&log_data[SCAO_PLRC]));
	}
	json_print_object(root, NULL);
	printf("\n");
...
static int wdc_print_c0_cloud_attr_log(void *data, int fmt)
{
	if (!data) {
		fprintf(stderr,
			"ERROR : WDC : Invalid buffer to read 0xC0 log\n");
		return -1;
	}
	switch (fmt) {
	case NORMAL:
			       ...
			       int uuid_index, __u32 namespace_id)
{
	...
	case WDC_NVME_SN840_DEV_ID_1:
		...
		if ((*cust_id == WDC_CUSTOMER_ID_0x1004) ||
			...
			if (uuid_index == 0) {
				...
				ret = nvme_get_log(
				    fd, WDC_NVME_GET_EOL_STATUS_LOG_OPCODE,
				    namespace_id, 0, NVME_LOG_LSP_NONE, 0,
				    false, uuid_index, 0,
				...
				if (ret == 0) {
					...
					if (ret == 0) {
						...
						/* parse the data */
						wdc_print_c0_cloud_attr_log(
						    data, fmt);
					}
				} else {
					fprintf(stderr,
						"ERROR : WDC : Unable to read "
						"C0 Log Page data\n");
					ret = -1;
				}
				...
				ret = nvme_get_log(
				    fd, WDC_NVME_GET_EOL_STATUS_LOG_OPCODE,
				    NVME_NSID_ALL, 0, NVME_LOG_LSP_NONE, 0,
				    false, uuid_index, 0,
				...
				fprintf(stderr,
					"ERROR : WDC : Unknown uuid index\n");
				ret = -1;
			}
		} else {
			if ((data = (__u8 *)malloc(
				 sizeof(__u8) * WDC_NVME_EOL_STATUS_LOG_LEN)) ==
			    NULL) {
				fprintf(stderr, "ERROR : WDC : malloc : %s\n",
					strerror(errno));
				return -1;
			...
			if (ret == 0) {
				...
				fprintf(stderr, "ERROR : WDC : Unable to read "
						"C0 Log Page data\n");
				ret = -1;
			}
	...
	case WDC_NVME_ZN350_DEV_ID:
	case WDC_NVME_ZN350_DEV_ID_1:
		if ((data = (__u8 *)malloc(sizeof(__u8) *
					   WDC_NVME_SMART_CLOUD_ATTR_LEN)) ==
		    NULL) {
			fprintf(stderr, "ERROR : WDC : malloc : %s\n",
				strerror(errno));
		...
		/* Get the 0xC0 log data */
		ret = nvme_get_log_simple(
		    fd, WDC_NVME_GET_SMART_CLOUD_ATTR_LOG_OPCODE,
		    WDC_NVME_SMART_CLOUD_ATTR_LEN, data);
		...
		if (ret == 0) {
			...
			fprintf(
			    stderr,
			    "ERROR : WDC : Unable to read C0 Log Page data\n");
			ret = -1;
		}
...
static int wdc_print_latency_monitor_log(
    int fd, struct wdc_ssd_latency_monitor_log *log_data, int fmt)
{
	if (!log_data) {
...
{
	if (!data) {
		fprintf(stderr, "ERROR : WDC : Invalid buffer to read fw "
				"activate history entries\n");
		return -1;
	}
...
static int wdc_get_ca_log_page(nvme_root_t r, int fd, char *format)
{
	...
	/* verify the 0xCA log page is supported */
	if (wdc_nvme_check_supported_log_page(
		r, fd, WDC_NVME_GET_DEVICE_INFO_LOG_OPCODE) == false) {
	...
	case WDC_NVME_SN200_DEV_ID:
		...
		if (*cust_id == WDC_CUSTOMER_ID_0x1005) {
			...
			ret = nvme_get_log_simple(
			    fd, WDC_NVME_GET_DEVICE_INFO_LOG_OPCODE,
			...
			fprintf(stderr,
				"ERROR : WDC : Unsupported Customer id, id = "
				"0x%x\n",
				*cust_id);
			return -1;
		}
	...
	case WDC_NVME_SN840_DEV_ID_1:
		...
		} else if ((*cust_id == WDC_CUSTOMER_ID_GN) ||
			   (*cust_id == WDC_CUSTOMER_ID_GD) ||
			   (*cust_id == WDC_CUSTOMER_ID_BD)) {

			if ((data = (__u8 *)malloc(sizeof(__u8) *
						   WDC_BD_CA_LOG_BUF_LEN)) ==
			    NULL) {
				fprintf(stderr, "ERROR : WDC : malloc : %s\n",
					strerror(errno));
				return -1;
	...
	default:
		...
		fprintf(stderr, "ERROR : WDC : Log page 0xCA not supported for "
				"this device\n");
		return -1;
		break;
	}
...
static int wdc_get_c1_log_page(nvme_root_t r, int fd, char *format,
			       uint8_t interval)
{
	int ret = 0;
	int fmt = -1;
	...
	if ((data = (__u8 *)malloc(sizeof(__u8) * WDC_ADD_LOG_BUF_LEN)) ==
	    NULL) {
		fprintf(stderr, "ERROR : WDC : malloc : %s\n", strerror(errno));
		return -1;
	}
	...
	if (ret == 0) {
		...
		total_subpages = l->num_subpages +
				 WDC_NVME_GET_STAT_PERF_INTERVAL_LIFETIME - 1;
		for (i = 0, p = data + skip_cnt; i < total_subpages;
		     i++, p += skip_cnt) {
			sph = (struct wdc_log_page_subpage_header *)p;
			if (sph->spcode == WDC_GET_LOG_PAGE_SSD_PERFORMANCE) {
				if (sph->pcset == interval) {
					perf = (struct wdc_ssd_perf_stats *)(p +
									     4);
		...
		if (ret) {
			fprintf(
			    stderr,
			    "ERROR : WDC : Unable to read data from buffer\n");
		}
	}
	free(data);
	return ret;
...
static int wdc_get_c3_log_page(nvme_root_t r, int fd, char *format)
{
	...
	if ((data = (__u8 *)malloc(sizeof(__u8) *
				   WDC_LATENCY_MON_LOG_BUF_LEN)) == NULL) {
		fprintf(stderr, "ERROR : WDC : malloc : %s\n", strerror(errno));
		return -1;
	}
	...
	if (ret == 0) {
		...
		if (log_data->log_page_version != WDC_LATENCY_MON_VERSION) {
			fprintf(
			    stderr,
			    "ERROR : WDC : invalid latency monitor version\n");
			ret = -1;
			goto out;
		}
		...
		for (i = 0; i < 16; i++) {
			if (wdc_lat_mon_guid[i] != log_data->log_page_guid[i]) {
				fprintf(stderr, "ERROR : WDC : Unknown GUID in "
						"C3 Log Page data\n");
				int j;
				fprintf(stderr,
					"ERROR : WDC : Expected GUID:  0x");
				for (j = 0; j < 16; j++) {
					fprintf(stderr, "%x",
						wdc_lat_mon_guid[j]);
				}
		...
		fprintf(stderr,
			"ERROR : WDC : Unable to read C3 data from buffer\n");
	}
...
static int wdc_get_d0_log_page(nvme_root_t r, int fd, char *format)
{
	...
	/* verify the 0xD0 log page is supported */
	if (wdc_nvme_check_supported_log_page(
		r, fd, WDC_NVME_GET_VU_SMART_LOG_OPCODE) == false) {
		fprintf(stderr, "ERROR : WDC : 0xD0 Log Page not supported\n");
		return -1;
	}

	if ((data = (__u8 *)malloc(sizeof(__u8) * WDC_NVME_VU_SMART_LOG_LEN)) ==
	    NULL) {
		fprintf(stderr, "ERROR : WDC : malloc : %s\n", strerror(errno));
		return -1;
	}
	...
	if (ret == 0) {
		...
		fprintf(stderr,
			"ERROR : WDC : Unable to read D0 Log Page data\n");
		ret = -1;
	}
				...
				struct plugin *plugin)
{
	...
	const char *log_page_mask =
	    "Log Page Mask, comma separated list: 0xC0, 0xC1, 0xCA, 0xD0";
	...
	OPT_ARGS(opts) = {
	    ...
	    OPT_BYTE("log-page-version", 'l', &cfg.log_page_version,
		     log_page_version),
	...
	if (cfg.log_page_version == 0) {
		...
		fprintf(stderr, "ERROR : WDC: unsupported log page version for "
				"this command\n");
		ret = -1;
		goto out;
	}

	num = argconfig_parse_comma_sep_array(cfg.log_page_mask, log_page_list,
					      16);
	...
	if (((capabilities & WDC_DRIVE_CAP_C0_LOG_PAGE) ==
	     WDC_DRIVE_CAP_C0_LOG_PAGE) &&
	    (page_mask & WDC_C0_PAGE_MASK)) {
		...
		if (ret)
			fprintf(stderr,
				"ERROR : WDC : Failure reading the C0 Log "
				"Page, ret = %d\n",
				ret);
	}
	if (((capabilities & (WDC_DRIVE_CAP_CA_LOG_PAGE)) ==
		...
		if (ret)
			fprintf(stderr,
				"ERROR : WDC : Failure reading the CA Log "
				"Page, ret = %d\n",
				ret);
	}
	if (((capabilities & WDC_DRIVE_CAP_C1_LOG_PAGE) ==
	     WDC_DRIVE_CAP_C1_LOG_PAGE) &&
	    (page_mask & WDC_C1_PAGE_MASK)) {
		/* Get the C1 Log Page */
		ret =
		    wdc_get_c1_log_page(r, fd, cfg.output_format, cfg.interval);
		if (ret)
			fprintf(stderr,
				"ERROR : WDC : Failure reading the C1 Log "
				"Page, ret = %d\n",
				ret);
	}
	if (((capabilities & WDC_DRIVE_CAP_D0_LOG_PAGE) ==
		...
		if (ret)
			fprintf(stderr,
				"ERROR : WDC : Failure reading the D0 Log "
				"Page, ret = %d\n",
				ret);
	}
...
static int wdc_get_latency_monitor_log(int argc, char **argv,
				       struct command *command,
				       struct plugin *plugin)
{
	...
	if (ret)
		fprintf(
		    stderr,
		    "ERROR : WDC : Failure reading the C3 Log Page, ret = %d\n",
		    ret);
...
static int wdc_do_clear_pcie_correctable_errors(int fd)
{
	...
	admin_cmd.cdw12 =
	    ((WDC_NVME_CLEAR_PCIE_CORR_SUBCMD << WDC_NVME_SUBCMD_SHIFT) |
			    ...
			    struct plugin *plugin)
{
	...
	r = nvme_scan(NULL);
	capabilities = wdc_get_drive_capabilities(r, fd);
	if ((capabilities & WDC_DRIVE_CAP_DRIVE_STATUS) !=
	    WDC_DRIVE_CAP_DRIVE_STATUS) {
		fprintf(stderr,
			"ERROR : WDC: unsupported device for this command\n");
		ret = -1;
	...
	if (wdc_nvme_check_supported_log_page(
		r, fd, WDC_NVME_GET_DEV_MGMNT_LOG_PAGE_OPCODE) == false) {
	...
	if (!wdc_nvme_get_dev_status_log_data(
		r, fd, &thermal_status, WDC_C2_THERMAL_THROTTLE_STATUS_ID))
		fprintf(stderr,
			"ERROR : WDC : Get Thermal Throttling Status Failed\n");
	...
	if (!wdc_nvme_get_dev_status_log_data(r, fd, &format_corrupt_reason,
		...
		fprintf(stderr,
			"ERROR : WDC : Get Format Corrupt Reason Failed\n");
	...
	if (system_eol_state == WDC_EOL_STATUS_NORMAL &&
	    user_eol_state == WDC_EOL_STATUS_NORMAL)
		...
		printf("  Drive Life Status:			Unknown : "
		       "0x%08x/0x%08x\n",
		       le32_to_cpu(user_eol_state),
		       le32_to_cpu(system_eol_state));
	...
	if (assert_status == WDC_ASSERT_DUMP_PRESENT)
		...
		printf("  Assert Dump Status:			Unknown : "
		       "0x%08x\n",
	...
	if (format_corrupt_reason == WDC_FORMAT_NOT_CORRUPT)
		...
		printf(
		    "  Format Corrupt Reason:	        Unknown : 0x%08x\n",
				 ...
				 struct plugin *plugin)
{
	...
	r = nvme_scan(NULL);
	capabilities = wdc_get_drive_capabilities(r, fd);
	if ((capabilities & WDC_DRIVE_CAP_CLEAR_ASSERT) !=
	...
	if (assert_status == WDC_ASSERT_DUMP_PRESENT) {
		...
		admin_cmd.cdw12 = ((WDC_NVME_CLEAR_ASSERT_DUMP_SUBCMD
				    << WDC_NVME_SUBCMD_SHIFT) |
...
static int wdc_get_fw_act_history(nvme_root_t r, int fd, char *format)
{
	...
	if (wdc_nvme_check_supported_log_page(
		r, fd, WDC_NVME_GET_FW_ACT_HISTORY_LOG_ID) == false) {
	...
	if ((data = (__u8 *)malloc(sizeof(__u8) *
				   WDC_FW_ACT_HISTORY_LOG_BUF_LEN)) == NULL) {
		fprintf(stderr, "ERROR : WDC : malloc : %s\n", strerror(errno));
		return -1;
	}
	...
	if (ret == 0) {
		/* parse the data */
		fw_act_history_hdr =
		    (struct wdc_fw_act_history_log_hdr *)(data);

		if ((fw_act_history_hdr->num_entries > 0) &&
		    (fw_act_history_hdr->num_entries <=
		     WDC_MAX_NUM_ACT_HIST_ENTRIES))
			ret = wdc_print_fw_act_history_log(
			    data, fw_act_history_hdr->num_entries, fmt, 0, 0);
			...
			fprintf(stderr,
				"ERROR : WDC : Invalid number entries found in "
				"FW Activate History Log Page - %d\n",
				fw_act_history_hdr->num_entries);
			ret = -1;
		}
	} else {
		fprintf(stderr, "ERROR : WDC : Unable to read FW Activate "
				"History Log Page data\n");
		ret = -1;
	}
...
static int wdc_get_fw_act_history_C2(nvme_root_t r, int fd, char *format)
{
	...
	if ((data = (__u8 *)malloc(
		 sizeof(__u8) * WDC_FW_ACT_HISTORY_C2_LOG_BUF_LEN)) == NULL) {
		fprintf(stderr, "ERROR : WDC : malloc : %s\n", strerror(errno));
		return -1;
	}
	...
	if (ret == 0) {
		...
		if (tot_entries > 0) {
			...
			num_entries =
				...
				: WDC_MAX_NUM_ACT_HIST_ENTRIES;
			ret = wdc_print_fw_act_history_log(
			    data, num_entries, fmt, *cust_id, vendor_id);
		} else {
			fprintf(stderr, "INFO : WDC : No FW Activate History "
					"entries found.\n");
			ret = 0;
...
static int wdc_vs_fw_activate_history(int argc, char **argv,
				      struct command *command,
				      struct plugin *plugin)
{
	int fd;
	...
	if (capabilities & WDC_DRIVE_CAP_FW_ACTIVATE_HISTORY) {
		...
		/* check for the GUID in the 0xC0 log page to determine which
		 * log page to use to */
		...
		if (c0GuidMatch) {
			ret =
			    wdc_get_fw_act_history_C2(r, fd, cfg.output_format);
		} else {
	...
	if (ret)
		fprintf(stderr,
			"ERROR : WDC : Failure reading the FW Activate "
			"History, ret = %d\n",
			ret);
out:
	nvme_free_tree(r);
...
static int wdc_do_clear_fw_activate_history_vuc(int fd)
{
	...
	admin_cmd.cdw12 =
	    ((WDC_NVME_CLEAR_FW_ACT_HIST_SUBCMD << WDC_NVME_SUBCMD_SHIFT) |
...
static int wdc_do_clear_fw_activate_history_fid(int fd)
{
	...
	ret = nvme_set_features_simple(fd, WDC_NVME_CLEAR_FW_ACT_HIST_VU_FID, 0,
				       value, false, &result);
...
static int wdc_clear_fw_activate_history(int argc, char **argv,
					 struct command *command,
					 struct plugin *plugin)
{
...
static int wdc_vs_telemetry_controller_option(int argc, char **argv,
					      struct command *command,
					      struct plugin *plugin)
{
	char *desc =
	    "Disable/Enable Controller Option of the Telemetry Log Page.";
	...
	char *status =
	    "Displays the current state of the controller initiated log page.";
	int fd;
	int ret = -1;
	nvme_root_t r;
	__u64 capabilities = 0;
	...
	OPT_ARGS(opts) = {OPT_FLAG("disable", 'd', &cfg.disable, disable),
	...
	r = nvme_scan(NULL);
	capabilities = wdc_get_drive_capabilities(r, fd);
	if ((capabilities & WDC_DRVIE_CAP_DISABLE_CTLR_TELE_LOG) !=
	    WDC_DRVIE_CAP_DISABLE_CTLR_TELE_LOG) {
		fprintf(stderr,
			"ERROR : WDC: unsupported device for this command\n");
		ret = -1;
	...
	if (cfg.disable) {
		ret = nvme_set_features_simple(
		    fd, WDC_VU_DISABLE_CNTLR_TELEMETRY_OPTION_FEATURE_ID, 0, 1,
		    false, &result);
		...
		if (cfg.enable) {
			ret = nvme_set_features_simple(
			    fd,
			    WDC_VU_DISABLE_CNTLR_TELEMETRY_OPTION_FEATURE_ID, 0,
			    0, false, &result);
		} else if (cfg.status) {
			ret = nvme_get_features_simple(
			    fd,
			    WDC_VU_DISABLE_CNTLR_TELEMETRY_OPTION_FEATURE_ID, 0,
			    &result);
			if (ret == 0) {
				if (result)
					fprintf(stderr,
						"Controller Option Telemetry "
						"Log Page State: Disabled\n");
				else
					fprintf(stderr,
						"Controller Option Telemetry "
						"Log Page State: Enabled\n");
			} else {
				nvme_show_status(ret);
			}
		} else {
			fprintf(stderr, "ERROR : WDC: unsupported option for "
					"this command\n");
			fprintf(stderr,
				"Please provide an option, -d, -e or -s\n");
			ret = -1;
			goto out;
		}
	}
...
static int wdc_de_VU_read_size(int fd, __u32 fileId, __u16 spiDestn,
			       __u32 *logSize)
{
...
static int wdc_de_VU_read_buffer(int fd, __u32 fileId, __u16 spiDestn,
				 __u32 offsetInDwords, __u8 *dataBuffer,
				 __u32 *bufferSize)
{
	int ret = WDC_STATUS_FAILURE;
	struct nvme_passthru_cmd cmd;
...
static int wdc_get_log_dir_max_entries(int fd, __u32 *maxNumOfEntries)
{
	...
	if (WDC_STATUS_SUCCESS !=
	    (ret =
		 wdc_de_VU_read_size(fd, 0, 5, (__u32 *)&headerPayloadSize))) {
		fprintf(stderr,
			"ERROR : WDC : %s: Failed to get headerPayloadSize "
			"from file directory 0x%x\n",
			__func__, ret);
		goto end;
	}

	fileIdOffsetsBufferSize = WDC_DE_FILE_HEADER_SIZE +
				  (headerPayloadSize * WDC_DE_FILE_OFFSET_SIZE);
	fileIdOffsetsBuffer = (__u8 *)calloc(1, fileIdOffsetsBufferSize);

	/* 2.Read to get file offsets */
	if (WDC_STATUS_SUCCESS !=
	    (ret = wdc_de_VU_read_buffer(fd, 0, 5, 0, fileIdOffsetsBuffer,
					 &fileIdOffsetsBufferSize))) {
		fprintf(stderr,
			"ERROR : WDC : %s: Failed to get fileIdOffsets from "
			"file directory 0x%x\n",
			__func__, ret);
		goto end;
	}
	...
	for (fileNum = 0;
	     fileNum < (headerPayloadSize - WDC_DE_FILE_HEADER_SIZE) /
			   WDC_DE_FILE_OFFSET_SIZE;
	     fileNum++) {
		fileOffset =
		    (fileIdOffsetsBuffer[WDC_DE_FILE_HEADER_SIZE +
					 (fileNum * WDC_DE_FILE_OFFSET_SIZE)]
		     << 8) +
		    fileIdOffsetsBuffer[WDC_DE_FILE_HEADER_SIZE +
					(fileNum * WDC_DE_FILE_OFFSET_SIZE) +
					1];
...
static WDC_DRIVE_ESSENTIAL_TYPE wdc_get_essential_type(__u8 fileName[])
{
	...
	} else if (wdc_UtilsStrCompare((char *)fileName,
				       WDC_DE_EVENT_LOG_FILE_NAME) == 0) {
		essentialType = WDC_DE_TYPE_EVENTLOG;
	} else if (wdc_UtilsStrCompare(
		       (char *)fileName,
		       WDC_DE_MANUFACTURING_INFO_PAGE_FILE_NAME) == 0) {
...
static int wdc_fetch_log_directory(int fd, PWDC_DE_VU_LOG_DIRECTORY directory)
{
	...
	if (WDC_STATUS_SUCCESS != ret) {
		fprintf(stderr,
			"ERROR : WDC : %s: Failed to get filesystem directory "
			"size, ret = %d\n",
			__func__, ret);
		goto end;
	}
	...
	ret = wdc_de_VU_read_buffer(fd, 0, 5, 0, fileDirectory,
				    &fileDirectorySize);
	if (WDC_STATUS_SUCCESS != ret) {
		fprintf(stderr,
			"ERROR : WDC : %s: Failed to get filesystem directory, "
	...
	for (fileNum = 0; fileNum < (headerSize - WDC_DE_FILE_HEADER_SIZE) /
					WDC_DE_FILE_OFFSET_SIZE;
	     fileNum++) {
		if (entryId >= directory->maxNumLogEntries)
			break;

		startIdx = WDC_DE_FILE_HEADER_SIZE +
			   (fileNum * WDC_DE_FILE_OFFSET_SIZE);
		memcpy(&fileOffsetTemp, fileDirectory + startIdx,
		...
		memset(&directory->logEntry[entryId], 0,
		       sizeof(WDC_DRIVE_ESSENTIALS));
		memcpy(&directory->logEntry[entryId].metaData, fileOffset,
		       sizeof(WDC_DE_VU_FILE_META_DATA));
		directory->logEntry[entryId]
		    .metaData.fileName[WDC_DE_FILE_NAME_SIZE - 1] = '\0';
		wdc_UtilsDeleteCharFromString(
		    (char *)directory->logEntry[entryId].metaData.fileName,
		    WDC_DE_FILE_NAME_SIZE, ' ');
		if (0 == directory->logEntry[entryId].metaData.fileID)
			continue;

		directory->logEntry[entryId].essentialType =
		    wdc_get_essential_type(
			directory->logEntry[entryId].metaData.fileName);
		/*fprintf(stderr, "WDC : %s: NVMe VU Log Entry %d, fileName =
		   %s, fileSize = 0x%lx, fileId = 0x%x\n",
			__func__, entryId,
		   directory->logEntry[entryId].metaData.fileName, (long
		   unsigned int)directory->logEntry[entryId].metaData.fileSize,
		   directory->logEntry[entryId].metaData.fileID);
		 */
		entryId++;
	}
...
static int wdc_fetch_log_file_from_device(int fd, __u32 fileId, __u16 spiDestn,
					  __u64 fileSize, __u8 *dataBuffer)
{
	int ret = WDC_STATUS_FAILURE;
	...
	if ((fileSize >= maximumTransferLength) || (fileSize > 0xFFFFFFFF)) {
		...
		if (maximumTransferLength <
		    WDC_DE_VU_READ_BUFFER_STANDARD_OFFSET)
			chunckSize = maximumTransferLength;

		buffSize = chunckSize;
		for (offsetIdx = 0; (offsetIdx * chunckSize) < fileSize;
		     offsetIdx++) {
			if (((offsetIdx * chunckSize) + buffSize) > fileSize)
				buffSize = (__u32)(fileSize -
						   (offsetIdx * chunckSize));
			/* Limitation in VU read buffer - offsetIdx and
			 * bufferSize are not greater than u32 */
			ret = wdc_de_VU_read_buffer(
			    fd, fileId, spiDestn,
			    (__u32)((offsetIdx * chunckSize) / sizeof(__u32)),
			    dataBuffer + (offsetIdx * chunckSize), &buffSize);
			if (ret != WDC_STATUS_SUCCESS) {
				fprintf(
				    stderr,
				    "ERROR : WDC : %s: wdc_de_VU_read_buffer "
				    "failed with ret = %d, fileId = 0x%x, "
				    "fileSize = 0x%lx\n",
				    __func__, ret, fileId,
		...
		if (ret != WDC_STATUS_SUCCESS) {
			fprintf(
			    stderr,
			    "ERROR : WDC : %s: wdc_de_VU_read_buffer failed "
			    "with ret = %d, fileId = 0x%x, fileSize = 0x%lx\n",
...
static int wdc_de_get_dump_trace(int fd, char *filePath, __u16 binFileNameLen,
				 char *binFileName)
{
	int ret = WDC_STATUS_FAILURE;
	...
	do {
		/* Get dumptrace size */
		ret = wdc_de_VU_read_size(fd, 0, WDC_DE_DUMPTRACE_DESTINATION,
					  &dumptraceSize);
		if (ret != WDC_STATUS_SUCCESS) {
			fprintf(stderr,
				"ERROR : WDC : %s: wdc_de_VU_read_size failed "
				"with ret = %d\n",
				__func__, ret);
			break;
		}
		...
		if (dumptraceSize < 4) {
			...
			fprintf(stderr,
				"ERROR : WDC : %s: wdc_de_VU_read_size failed, "
				"read size is less than 4 bytes, dumptraceSize "
				"= 0x%x\n",
		...
		maxTransferLen =
			...
			: WDC_DE_READ_MAX_TRANSFER_SIZE;

		/* Comment from  FW Team:
		 * The max non - block transfer size is 0xFFFF (16 bits allowed
		 * as the block size).Use 0x8000 to keep it on a word -
		 * boundary. max_xfer = int(pow(2, id_data['MDTS'])) * 4096 # 4k
		 * page size as reported in pcie capabiltiies
		 */
		chunkSize = dumptraceSize < maxTransferLen ? dumptraceSize
							   : maxTransferLen;
		chunks = (dumptraceSize / maxTransferLen) +
			 ((dumptraceSize % maxTransferLen) ? 1 : 0);

		readBuffer = (unsigned char *)calloc(dumptraceSize,
						     sizeof(unsigned char));
		readBufferLen = chunkSize;
		lastPktReadBufferLen = (dumptraceSize % maxTransferLen)
					   ? (dumptraceSize % maxTransferLen)
					   : chunkSize;

		if (readBuffer == NULL) {
			fprintf(stderr,
				"ERROR : WDC : %s: readBuffer calloc failed\n",
				__func__);
		...
		for (i = 0; i < chunks; i++) {
			...
			/* Last loop call, Assign readBufferLen to read only
			 * left over bytes */
			if (i == (chunks - 1)) {
			...
			ret = wdc_de_VU_read_buffer(
			    fd, 0, WDC_DE_DUMPTRACE_DESTINATION, 0,
	...
	if (ret == WDC_STATUS_SUCCESS) {
		ret = wdc_WriteToFile(binFileName, (char *)readBuffer,
				      dumptraceSize);
		if (ret != WDC_STATUS_SUCCESS)
			fprintf(stderr,
				"ERROR : WDC : %s: wdc_WriteToFile failed, ret "
				"= %d\n",
				__func__, ret);
	} else {
		fprintf(stderr,
			"ERROR : WDC : %s: Read Buffer Loop failed, ret = %d\n",
			__func__, ret);
	}
...
static int wdc_do_drive_essentials(nvme_root_t r, int fd, char *dir, char *key)
{
	...
	if (wdc_get_serial_and_fw_rev(fd, (char *)idSerialNo,
				      (char *)idFwRev)) {
		fprintf(stderr,
			"ERROR : WDC : get serial # and fw revision failed\n");
		return -1;
	} else {
		fprintf(stderr,
			"Get Drive Essentials Data for device serial #: %s and "
			"fw revision: %s\n",
			idSerialNo, idFwRev);
	}
	...
	wdc_UtilsSnprintf((char *)timeString, MAX_PATH_LEN,
			  "%02u%02u%02u_%02u%02u%02u", timeInfo.year,
			  timeInfo.month, timeInfo.dayOfMonth, timeInfo.hour,
	...
	wdc_UtilsSnprintf((char *)firmwareRevision, WDC_NVME_FIRMWARE_REV_LEN,
			  (char *)idFwRev);
	/* Remove any space form FirmwareRevision */
	wdc_UtilsDeleteCharFromString((char *)firmwareRevision,
				      WDC_NVME_FIRMWARE_REV_LEN, ' ');

	wdc_UtilsSnprintf((char *)bufferFolderName, MAX_PATH_LEN, "%s_%s_%s_%s",
			  "DRIVE_ESSENTIALS", (char *)serialNo,
			  (char *)firmwareRevision, (char *)timeString);

	if (dir != NULL) {
		wdc_UtilsSnprintf((char *)bufferFolderPath, MAX_PATH_LEN,
				  "%s%s%s", (char *)dir, WDC_DE_PATH_SEPARATOR,
				  (char *)bufferFolderName);
	} else {
		retPtr = getcwd((char *)currDir, MAX_PATH_LEN);
		if (retPtr != NULL)
			wdc_UtilsSnprintf(
			    (char *)bufferFolderPath, MAX_PATH_LEN, "%s%s%s",
			    (char *)currDir, WDC_DE_PATH_SEPARATOR,
			    (char *)bufferFolderName);
		else {
			fprintf(stderr, "ERROR : WDC : get current working "
					"directory failed\n");
			return -1;
		}
	}
	...
	if (ret != 0) {
		...
		fprintf(stderr,
			"Store Drive Essentials bin files in directory: %s\n",
			bufferFolderPath);
	}
	...
	if (ret) {
		fprintf(stderr,
			"ERROR : WDC : nvme_identify_ctrl() failed, ret = %d\n",
			ret);
		return -1;
	} else {
		wdc_UtilsSnprintf(fileName, MAX_PATH_LEN, "%s%s%s_%s_%s.bin",
				  (char *)bufferFolderPath,
				  WDC_DE_PATH_SEPARATOR, "IdentifyController",
				  (char *)serialNo, (char *)timeString);
		wdc_WriteToFile(fileName, (char *)&ctrl,
	...
	if (ret) {
		fprintf(stderr,
			"ERROR : WDC : nvme_identify_ns() failed, ret = %d\n",
			ret);
	} else {
		wdc_UtilsSnprintf(fileName, MAX_PATH_LEN, "%s%s%s_%s_%s.bin",
				  (char *)bufferFolderPath,
				  WDC_DE_PATH_SEPARATOR, "IdentifyNamespace",
				  (char *)serialNo, (char *)timeString);
		wdc_WriteToFile(fileName, (char *)&ns,
	...
	if (ret) {
		fprintf(stderr,
			"ERROR : WDC : nvme_error_log() failed, ret = %d\n",
			ret);
	} else {
		wdc_UtilsSnprintf(fileName, MAX_PATH_LEN, "%s%s%s_%s_%s.bin",
				  (char *)bufferFolderPath,
				  WDC_DE_PATH_SEPARATOR, "ErrorLog",
				  (char *)serialNo, (char *)timeString);
	...
	if (ret) {
		fprintf(stderr,
			"ERROR : WDC : nvme_smart_log() failed, ret = %d\n",
			ret);
	} else {
		wdc_UtilsSnprintf(fileName, MAX_PATH_LEN, "%s%s%s_%s_%s.bin",
				  (char *)bufferFolderPath,
				  WDC_DE_PATH_SEPARATOR, "SmartLog",
				  (char *)serialNo, (char *)timeString);
		wdc_WriteToFile(fileName, (char *)&smart_log,
	...
	if (ret) {
		fprintf(stderr,
			"ERROR : WDC : nvme_fw_log() failed, ret = %d\n", ret);
	} else {
		wdc_UtilsSnprintf(fileName, MAX_PATH_LEN, "%s%s%s_%s_%s.bin",
				  (char *)bufferFolderPath,
				  WDC_DE_PATH_SEPARATOR, "FwSLotLog",
				  (char *)serialNo, (char *)timeString);
		wdc_WriteToFile(fileName, (char *)&fw_log,
				sizeof(struct nvme_firmware_slot));
	}

	/* Get VU log pages  */
	/* define inputs for vendor unique log pages */
	vuLogInput = (PWDC_NVME_DE_VU_LOGPAGES)calloc(
	    1, sizeof(WDC_NVME_DE_VU_LOGPAGES));
	vuLogInput->numOfVULogPages =
	    sizeof(deVULogPagesList) / sizeof(deVULogPagesList[0]);
	...
	for (vuLogIdx = 0; vuLogIdx < vuLogInput->numOfVULogPages; vuLogIdx++) {
		...
		ret = nvme_get_log_simple(fd,
					  deVULogPagesList[vuLogIdx].logPageId,
					  dataBufferSize, dataBuffer);
		if (ret) {
			fprintf(stderr,
				"ERROR : WDC : nvme_get_log() for log page "
				"0x%x failed, ret = %d\n",
				deVULogPagesList[vuLogIdx].logPageId, ret);
		} else {
			wdc_UtilsDeleteCharFromString(
			    (char *)deVULogPagesList[vuLogIdx].logPageIdStr, 4,
			    ' ');
			wdc_UtilsSnprintf(
			    fileName, MAX_PATH_LEN, "%s%s%s_%s_%s_%s.bin",
			    (char *)bufferFolderPath, WDC_DE_PATH_SEPARATOR,
			    "LogPage",
			    (char *)&deVULogPagesList[vuLogIdx].logPageIdStr,
			    (char *)serialNo, (char *)timeString);
			wdc_WriteToFile(fileName, (char *)dataBuffer,
					dataBufferSize);
		}
	...
	/* Get NVMe Features (0x01, 0x02, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
	 * 0x0A, 0x0B, 0x0C) */
	for (listIdx = 1;
	     listIdx < (sizeof(deFeatureIdList) / sizeof(deFeatureIdList[0]));
	     listIdx++) {
		memset(featureIdBuff, 0, sizeof(featureIdBuff));
		/* skipping  LbaRangeType as it is an optional nvme command and
		 * not supported */
		if (deFeatureIdList[listIdx].featureId == FID_LBA_RANGE_TYPE)
			continue;
		ret = nvme_get_features_data(
		    fd, deFeatureIdList[listIdx].featureId, WDC_DE_GLOBAL_NSID,
		    sizeof(featureIdBuff), &featureIdBuff, &result);

		if (ret) {
			fprintf(stderr,
				"ERROR : WDC : nvme_get_feature id 0x%x "
				"failed, ret = %d\n",
				deFeatureIdList[listIdx].featureId, ret);
		} else {
			wdc_UtilsSnprintf(
			    fileName, MAX_PATH_LEN, "%s%s%s0x%x_%s_%s_%s.bin",
			    (char *)bufferFolderPath, WDC_DE_PATH_SEPARATOR,
			    "FEATURE_ID_", deFeatureIdList[listIdx].featureId,
			    deFeatureIdList[listIdx].featureName, serialNo,
			    timeString);
			wdc_WriteToFile(fileName, (char *)featureIdBuff,
	...
	if (ret == WDC_STATUS_SUCCESS) {
		...
		deEssentialsList.logEntry = (WDC_DRIVE_ESSENTIALS *)calloc(
		    1, sizeof(WDC_DRIVE_ESSENTIALS) * maxNumOfVUFiles);
		...
		if (ret == WDC_STATUS_SUCCESS) {
			...
			for (listIdx = 0;
			     listIdx < deEssentialsList.numOfValidLogEntries;
			     listIdx++) {
				if (0 == deEssentialsList.logEntry[listIdx]
					     .metaData.fileSize) {
					fprintf(
					    stderr,
					    "ERROR : WDC : File Size for %s is "
					    "0\n",
					    deEssentialsList.logEntry[listIdx]
						.metaData.fileName);
					ret = WDC_STATUS_FILE_SIZE_ZERO;
				} else {
					/* Fetch Log File Data */
					dataBuffer = (char *)calloc(
					    1, (size_t)deEssentialsList
						   .logEntry[listIdx]
						   .metaData.fileSize);
					ret = wdc_fetch_log_file_from_device(
					    fd,
					    deEssentialsList.logEntry[listIdx]
						.metaData.fileID,
					    WDC_DE_DESTN_SPI,
			...
			fprintf(
			    stderr,
			    "WDC : wdc_fetch_log_directory failed, ret = %d\n",
			    ret);
		}
		...
		fprintf(stderr,
			"WDC : wdc_get_log_dir_max_entries failed, ret = %d\n",
			ret);
	}
	...
	wdc_UtilsSnprintf(fileName, MAX_PATH_LEN, "%s%s%s_%s_%s.bin",
			  (char *)bufferFolderPath, WDC_DE_PATH_SEPARATOR,
			  "dumptrace", serialNo, timeString);
	if (WDC_STATUS_SUCCESS !=
	    (ret = wdc_de_get_dump_trace(fd, (char *)bufferFolderPath, 0,
					 fileName))) {
		fprintf(
		    stderr,
		    "ERROR : WDC : wdc_de_get_dump_trace failed, ret = %d\n",
		    ret);
	}

	/* Tar the Drive Essentials directory */
	wdc_UtilsSnprintf(tarFileName, sizeof(tarFileName), "%s%s",
			  (char *)bufferFolderPath, WDC_DE_TAR_FILE_EXTN);
	if (dir != NULL) {
		...
		wdc_UtilsSnprintf(tarFiles, sizeof(tarFiles), "%s%s%s",
				  (char *)bufferFolderName,
				  WDC_DE_PATH_SEPARATOR, WDC_DE_TAR_FILES);
	}
	wdc_UtilsSnprintf(tarCmd, sizeof(tarCmd), "%s %s %s", WDC_DE_TAR_CMD,
			  (char *)tarFileName, (char *)tarFiles);

	ret = system(tarCmd);

	if (ret) {
		fprintf(stderr,
			"ERROR : WDC : Tar of Drive Essentials data failed, "
			"ret = %d\n",
			ret);
	}
				...
				struct plugin *plugin)
{
	...
	r = nvme_scan(NULL);
	capabilities = wdc_get_drive_capabilities(r, fd);
	if ((capabilities & WDC_DRIVE_CAP_DRIVE_ESSENTIALS) !=
	    WDC_DRIVE_CAP_DRIVE_ESSENTIALS) {
		fprintf(stderr,
			"ERROR : WDC: unsupported device for this command\n");
		nvme_free_tree(r);
		return -1;
...
static int wdc_do_drive_resize(int fd, uint64_t new_size)
{
	...
	admin_cmd.cdw12 =
	    ((WDC_NVME_DRIVE_RESIZE_SUBCMD << WDC_NVME_SUBCMD_SHIFT) |
...
static int wdc_do_drive_info(int fd, __u32 *result)
{
	...
	admin_cmd.cdw12 =
	    ((WDC_NVME_DRIVE_INFO_SUBCMD << WDC_NVME_SUBCMD_SHIFT) |
			    ...
			    struct plugin *plugin)
{
	...
	OPT_ARGS(opts) = {OPT_UINT("size", 's', &cfg.size, size), OPT_END()};
				...
				struct plugin *plugin)
{
	...
	const char *op_option =
	    "The over provisioning option to set for namespace.";
	nvme_root_t r;
	uint64_t capabilities = 0;
	int fd, ret;
	...
	OPT_ARGS(opts) = {
	    OPT_UINT("namespace-id", 'n', &cfg.namespace_id, namespace_id),
	    OPT_UINT("op-option", 'o', &cfg.op_option, op_option), OPT_END()};

	fd = parse_and_open(argc, argv, desc, opts);
	if (fd < 0)
		return fd;

	if ((cfg.op_option != 0x1) && (cfg.op_option != 0x2) &&
	    (cfg.op_option != 0x3) && (cfg.op_option != 0xF)) {
		fprintf(stderr,
			"ERROR : WDC: unsupported OP option parameter\n");
		return -1;
	}
	...
	wdc_check_device(r, fd);
	capabilities = wdc_get_drive_capabilities(r, fd);
	if ((capabilities & WDC_DRIVE_CAP_NS_RESIZE) ==
	    WDC_DRIVE_CAP_NS_RESIZE) {
		ret = wdc_do_namespace_resize(fd, cfg.namespace_id,
					      cfg.op_option);

		if (ret != 0)
			printf("ERROR : WDC: Namespace Resize of namespace id "
			       "0x%x, op option 0x%x failed\n",
				 ...
				 struct plugin *plugin)
{
	...
	OPT_ARGS(opts) = {OPT_UINT("log-id", 'i', &cfg.log_id, log_id),
			  OPT_FILE("file", 'o', &cfg.file, fname), OPT_END()};
	...
	if (cfg.log_id != NVME_LOG_LID_TELEMETRY_HOST &&
		...
		fprintf(stderr, "ERROR : WDC: Invalid Log ID. It must be 7 "
				"(Host) or 8 (Controller)\n");
		ret = -1;
		goto close_fd;
	}
	...
	if (cfg.file != NULL) {
		...
		if (cfg.log_id == NVME_LOG_LID_TELEMETRY_CTRL)
			snprintf(fileSuffix, PATH_MAX,
				 "_error_reason_identifier_ctlr_%s",
				 (char *)timeStamp);
		else
			snprintf(fileSuffix, PATH_MAX,
				 "_error_reason_identifier_host_%s",
				 (char *)timeStamp);
	...
	capabilities = wdc_get_drive_capabilities(r, fd);
	if ((capabilities & WDC_DRIVE_CAP_REASON_ID) ==
	    WDC_DRIVE_CAP_REASON_ID) {
...
static const char *nvme_log_id_to_string(__u8 log_id)
{
	...
	case WDC_LOG_ID_C0:
		return "WDC Vendor Unique Log ID";
	case WDC_LOG_ID_C1:
		return "WDC Vendor Unique Log ID";
	case WDC_LOG_ID_C2:
		return "WDC Vendor Unique Log ID";
				  ...
				  struct plugin *plugin)
{
	...
	if ((capabilities & WDC_DRIVE_CAP_LOG_PAGE_DIR) == 0) {
		...
		/* verify the 0xC2 Device Manageability log page is supported */
		if (wdc_nvme_check_supported_log_page(
			r, fd, WDC_NVME_GET_DEV_MGMNT_LOG_PAGE_OPCODE) ==
		    false) {
			fprintf(
			    stderr,
			    "%s: ERROR : WDC : 0xC2 Log Page not supported\n",
			    __func__);
			ret = -1;
			goto out;
		}
		...
		if (get_dev_mgment_cbs_data(r, fd,
			...
			if (cbs_data != NULL) {
				...
				if (!strcmp(cfg.output_format, "normal")) {
					for (i = 0;
					     i < le32_to_cpu(cbs_data->length);
					     i++) {
						printf("0x%x  - %s\n",
						       cbs_data->data[i],
						       nvme_log_id_to_string(
							   cbs_data->data[i]));
					}
				} else if (!strcmp(cfg.output_format,
						   "binary")) {
					d((__u8 *)cbs_data->data,
					  le32_to_cpu(cbs_data->length), 16, 1);
...
static int wdc_get_drive_reason_id(int fd, char *drive_reason_id, size_t len)
{
	...
	res_len = snprintf(drive_reason_id, len, "%s_%s_%s", ctrl.sn, ctrl.mn,
			   reason_id_str);
	if (len <= res_len) {
		fprintf(stderr,
			"ERROR : WDC : cannot format serial number due to data "
...
static int wdc_save_reason_id(int fd, __u8 *rsn_ident, int size)
{
	 ...
	 */
	ret = wdc_create_log_file(reason_id_file, rsn_ident,
				  WDC_REASON_ID_ENTRY_LEN);
	free(reason_id_file);
...
static int wdc_dump_telemetry_hdr(int fd, int log_id,
				  struct nvme_telemetry_log *log_hdr)
{
	...
	if (log_id == NVME_LOG_LID_TELEMETRY_HOST)
		...
		ret = nvme_get_log_telemetry_ctrl(fd, false, 0, 512,
						  (void *)log_hdr);
	...
	if (ret < 0)
		...
		nvme_show_status(ret);
		fprintf(stderr,
			"%s: ERROR : Failed to acquire telemetry header, ret = "
			"%d!\n",
			__func__, ret);
	}
...
static int wdc_do_get_reason_id(int fd, char *file, int log_id)
{
	...
	if (log_hdr == NULL) {
		fprintf(stderr,
			"%s: ERROR : malloc failed, size : 0x%x, status : %s\n",
	...
	if (ret != 0) {
		fprintf(stderr,
			"%s: ERROR : get telemetry header failed, ret  : %d\n",
			__func__, ret);
		ret = -1;
		goto out;
	}
	...
	ret = wdc_create_log_file(file, (__u8 *)log_hdr->rsnident,
				  reason_id_size);
...
static void wdc_print_nand_stats_normal(__u16 version, void *data)
{
	...
	struct wdc_nand_stats_V3 *nand_stats_v3 =
	    (struct wdc_nand_stats_V3 *)(data);
	__u64 temp_raw;
	__u16 temp_norm;
	__u64 *temp_ptr = NULL;
	...
	case 3:
		...
		printf(
		    "  Estimate of Incomplete Trim Data		 %" PRIu64 "\n",
		...
		temp_raw = ((*temp_ptr & 0xFFFFFFFFFFFF0000) >> 16);
		printf(
		    "  Bad System Nand Block Count - Normalized	 %" PRIu16 "\n",
		    le16_to_cpu(temp_norm));
		printf("  Bad System Nand Block Count - Raw	         "
		       "%" PRIu64 "\n",
		       le64_to_cpu(temp_raw));
...
static void wdc_print_nand_stats_json(__u16 version, void *data)
{
	...
	case 0:
		...
		json_object_add_value_float(
		    root, "NAND Writes TLC (Bytes)",
		    int128_to_double(nand_stats->nand_write_tlc));
		json_object_add_value_float(
		    root, "NAND Writes SLC (Bytes)",
		    int128_to_double(nand_stats->nand_write_slc));
		json_object_add_value_uint(
		    root, "NAND Program Failures",
		    le32_to_cpu(nand_stats->nand_prog_failure));
		json_object_add_value_uint(
		    root, "NAND Erase Failures",
		    le32_to_cpu(nand_stats->nand_erase_failure));
		json_object_add_value_uint(
		    root, "Bad Block Count",
		    le32_to_cpu(nand_stats->bad_block_count));
		json_object_add_value_uint64(
		    root, "NAND XOR/RAID Recovery Trigger Events",
		    le64_to_cpu(nand_stats->nand_rec_trigger_event));
		json_object_add_value_uint64(
		    root, "E2E Error Counter",
		    le64_to_cpu(nand_stats->e2e_error_counter));
		json_object_add_value_uint64(
		    root, "Number Successful NS Resizing Events",
		    le64_to_cpu(nand_stats->successful_ns_resize_event));
	...
	case 3:
		...
		json_object_add_value_uint(root,
					   "Bad NAND Blocks Count - Normalized",
					   le16_to_cpu(temp_norm));
		json_object_add_value_uint64(
		    root, "Bad NAND Blocks Count - Raw", le64_to_cpu(temp_raw));
		json_object_add_value_uint64(
		    root, "NAND XOR Recovery count",
		    le64_to_cpu(nand_stats_v3->xor_recovery_count));
		json_object_add_value_uint64(
		    root, "UECC Read Error count",
		    le64_to_cpu(nand_stats_v3->uecc_read_error_count));
		json_object_add_value_uint64(
		    root, "SSD End to End corrected errors",
		    le64_to_cpu(nand_stats_v3->ssd_correction_counts[0]));
		json_object_add_value_uint(
		    root, "SSD End to End detected errors",
		    le32_to_cpu(nand_stats_v3->ssd_correction_counts[8]));
		json_object_add_value_uint(
		    root, "SSD End to End uncorrected E2E errors",
		    le32_to_cpu(nand_stats_v3->ssd_correction_counts[12]));
		json_object_add_value_uint(root, "System data % life-used",
					   nand_stats_v3->percent_life_used);
		json_object_add_value_uint64(
		    root, "User Data Erase Counts - SLC Min",
		    le64_to_cpu(nand_stats_v3->user_data_erase_counts[0]));
		json_object_add_value_uint64(
		    root, "User Data Erase Counts - SLC Max",
		    le64_to_cpu(nand_stats_v3->user_data_erase_counts[1]));
		json_object_add_value_uint64(
		    root, "User Data Erase Counts - TLC Min",
		    le64_to_cpu(nand_stats_v3->user_data_erase_counts[2]));
		json_object_add_value_uint64(
		    root, "User Data Erase Counts - TLC Max",
		    le64_to_cpu(nand_stats_v3->user_data_erase_counts[3]));
		temp_ptr = (__u64 *)nand_stats_v3->program_fail_count;
		temp_norm = (__u16)(*temp_ptr & 0x000000000000FFFF);
		temp_raw = ((*temp_ptr & 0xFFFFFFFFFFFF0000) >> 16);
		json_object_add_value_uint(root,
					   "Program Fail Count - Normalized",
					   le16_to_cpu(temp_norm));
		...
		json_object_add_value_uint(
		    ...
		    le16_to_cpu(nand_stats_v3->correctable_error_count));
		json_object_add_value_uint(
		    root, "% Free Blocks (User)",
		    nand_stats_v3->percent_free_blocks_user);
		json_object_add_value_uint64(
		    root, "Security Version Number",
		    le64_to_cpu(nand_stats_v3->security_version_number));
		json_object_add_value_uint(
		    root, "% Free Blocks (System)",
		    nand_stats_v3->percent_free_blocks_system);
		json_object_add_value_float(
		    root, "Data Set Management Commands",
		    int128_to_double(nand_stats_v3->trim_completions));
		json_object_add_value_uint64(
		    root, "Estimate of Incomplete Trim Data",
		    le64_to_cpu(nand_stats_v3->trim_completions[16]));
		json_object_add_value_uint(root, "%% of completed trim",
					   nand_stats_v3->trim_completions[24]);
		json_object_add_value_uint(root,
					   "Background Back-Pressure-Guage",
					   nand_stats_v3->back_pressure_guage);
		json_object_add_value_uint64(
		    root, "Soft ECC Error Count",
		    le64_to_cpu(nand_stats_v3->soft_ecc_error_count));
		...
		temp_raw = ((*temp_ptr & 0xFFFFFFFFFFFF0000) >> 16);
		json_object_add_value_uint(
		    root, "Bad System Nand Block Count - Normalized",
		    le16_to_cpu(temp_norm));
		json_object_add_value_uint64(
		    root, "Bad System Nand Block Count - Raw",
		    le64_to_cpu(temp_raw));
		json_object_add_value_float(
		    root, "Endurance Estimate",
		    int128_to_double(nand_stats_v3->endurance_estimate));
		json_object_add_value_uint(
		    root, "Thermal Throttling Status",
		    nand_stats_v3->thermal_throttling_st_ct[0]);
		json_object_add_value_uint(
		    root, "Thermal Throttling Count",
		    nand_stats_v3->thermal_throttling_st_ct[1]);
		json_object_add_value_uint64(
		    root, "Unaligned I/O",
		    le64_to_cpu(nand_stats_v3->unaligned_IO));
		json_object_add_value_float(
		    root, "Physical Media Units Read",
		    int128_to_double(nand_stats_v3->physical_media_units));
		json_object_add_value_uint(
		    root, "log page version",
		    le16_to_cpu(nand_stats_v3->log_page_version));
...
static void wdc_print_pcie_stats_json(struct wdc_vs_pcie_stats *pcie_stats)
{
	...
	json_object_add_value_uint64(
	    root, "Unsupported Request Error Counter",
	    le64_to_cpu(pcie_stats->unsupportedRequestErrorCount));
	json_object_add_value_uint64(
	    root, "ECRC Error Status Counter",
	    le64_to_cpu(pcie_stats->ecrcErrorStatusCount));
	json_object_add_value_uint64(
	    root, "Malformed TLP Status Counter",
	    le64_to_cpu(pcie_stats->malformedTlpStatusCount));

	json_object_add_value_uint64(
	    root, "Receiver Overflow Status Counter",
	    le64_to_cpu(pcie_stats->receiverOverflowStatusCount));
	json_object_add_value_uint64(
	    root, "Unexpected Completion Status Counter",
	    le64_to_cpu(pcie_stats->unexpectedCmpltnStatusCount));
	json_object_add_value_uint64(
	    root, "Complete Abort Status Counter",
	    le64_to_cpu(pcie_stats->completeAbortStatusCount));
	json_object_add_value_uint64(
	    root, "Completion Timeout Status Counter",
	    le64_to_cpu(pcie_stats->cmpltnTimoutStatusCount));
	json_object_add_value_uint64(
	    root, "Flow Control Error Status Counter",
	    le64_to_cpu(pcie_stats->flowControlErrorStatusCount));
	json_object_add_value_uint64(
	    root, "Poisoned TLP Status Counter",
	    le64_to_cpu(pcie_stats->poisonedTlpStatusCount));
	json_object_add_value_uint64(
	    root, "Dlink Protocol Error Status Counter",
	    le64_to_cpu(pcie_stats->dLinkPrtclErrorStatusCount));
	json_object_add_value_uint64(
	    root, "Advisory Non Fatal Error Status Counter",
	    le64_to_cpu(pcie_stats->advsryNFatalErrStatusCount));
	json_object_add_value_uint64(
	    root, "Replay Timer TO Status Counter",
	    le64_to_cpu(pcie_stats->replayTimerToStatusCount));
	json_object_add_value_uint64(
	    root, "Replay Number Rollover Status Counter",
	    le64_to_cpu(pcie_stats->replayNumRolloverStCount));
	json_object_add_value_uint64(
	    root, "Bad DLLP Status Counter",
	    le64_to_cpu(pcie_stats->badDllpStatusCount));
	json_object_add_value_uint64(
	    root, "Bad TLP Status Counter",
	    le64_to_cpu(pcie_stats->badTlpStatusCount));
	json_object_add_value_uint64(
	    root, "Receiver Error Status Counter",
	    le64_to_cpu(pcie_stats->receiverErrStatusCount));
...
static int wdc_do_vs_nand_stats(int fd, char *format)
{
	...
	if ((output = (uint8_t *)calloc(WDC_NVME_NAND_STATS_SIZE,
					sizeof(uint8_t))) == NULL) {
		fprintf(stderr, "ERROR : WDC : calloc : %s\n", strerror(errno));
		ret = -1;
		goto out;
	...
	if (ret) {
		fprintf(stderr,
			"ERROR : WDC : %s : Failed to retreive NAND stats\n",
			__func__);
		goto out;
	} else {
		fmt = validate_output_format(format);
		if (fmt < 0) {
			fprintf(stderr,
				"ERROR : WDC : invalid output format\n");
			ret = fmt;
			goto out;
		}
			     ...
			     struct plugin *plugin)
{
	...
	if ((capabilities & WDC_DRIVE_CAP_NAND_STATS) == 0) {
		...
		if (ret)
			fprintf(stderr,
				"ERROR : WDC : Failure reading NAND "
				"statistics, ret = %d\n",
				ret);
	}
...
static int wdc_do_vs_pcie_stats(int fd, struct wdc_vs_pcie_stats *pcieStatsPtr)
{
	int ret;
	struct nvme_passthru_cmd admin_cmd;
			     ...
			     struct plugin *plugin)
{
	...
	if ((capabilities & WDC_DRIVE_CAP_PCIE_STATS) == 0) {
		...
		if (ret)
			fprintf(stderr,
				"ERROR : WDC : Failure reading PCIE "
				"statistics, ret = 0x%x\n",
				ret);
		else {
			/* parse the data */
			     ...
			     struct plugin *plugin)
{
	...
	if ((capabilities & WDC_DRIVE_CAP_INFO) == WDC_DRIVE_CAP_INFO) {
		...
		if (!ret) {
			size =
			    (__u16)((cpu_to_le32(result) & 0xffff0000) >> 16);
			...
			if (fmt == NORMAL) {
				...
				json_object_add_value_string(
				    root, "Drive HW Revison", rev_str);

				json_object_add_value_int(root, "FTL Unit Size",
							  le16_to_cpu(size));
				wdc_StrFormat(formatter, sizeof(formatter),
					      &ctrl.sn[0], sizeof(ctrl.sn));
				json_object_add_value_string(
				    root, "Customer SN", formatter);
	...
	} else if ((capabilities & WDC_DRIVE_CAP_INFO_2) ==
		   WDC_DRIVE_CAP_INFO_2) {
		...
		if (fmt == NORMAL) {
			...
			json_object_add_value_string(root, "Drive HW Revison",
						     rev_str);
			wdc_StrFormat(formatter, sizeof(formatter), &ctrl.sn[0],
				      14);
			json_object_add_value_string(root, "Customer SN",
						     formatter);
				    ...
				    struct plugin *plugin)
{
	...
	wdc_check_device(r, fd);
	capabilities = wdc_get_drive_capabilities(r, fd);
	if ((capabilities & WDC_DRIVE_CAP_TEMP_STATS) !=
	    WDC_DRIVE_CAP_TEMP_STATS) {
		fprintf(stderr,
			"ERROR : WDC: unsupported device for this command\n");
		nvme_free_tree(r);
		return -1;
	...
	temperature =
	    ((smart_log.temperature[1] << 8) | smart_log.temperature[0]) - 273;

	/* retrieve HCTM Thermal Management Temperatures */
	nvme_get_features_simple(fd, 0x10, 0, &hctm_tmt);
	temp_tmt1 =
	    ((hctm_tmt >> 16) & 0xffff) ? ((hctm_tmt >> 16) & 0xffff) - 273 : 0;
	...
	if (fmt == NORMAL) {
		...
		json_object_add_value_int(root, "Current Composite Temperature",
					  le32_to_cpu(temperature));
		json_object_add_value_int(root, "WCTEMP",
					  le16_to_cpu(id_ctrl.wctemp - 273));
		json_object_add_value_int(root, "CCTEMP",
					  le16_to_cpu(id_ctrl.cctemp - 273));
		json_object_add_value_int(root, "DITT support", 0);
		json_object_add_value_int(root, "HCTM support",
					  le16_to_cpu(id_ctrl.hctma));

		json_object_add_value_int(root, "HCTM Light (TMT1)",
					  le16_to_cpu(temp_tmt1));
		json_object_add_value_int(
		    root, "TMT1 Transition Counter",
		    le32_to_cpu(smart_log.thm_temp1_trans_count));
		json_object_add_value_int(
		    root, "TMT1 Total Time",
		    le32_to_cpu(smart_log.thm_temp1_total_time));

		json_object_add_value_int(root, "HCTM Light (TMT2)",
					  le16_to_cpu(temp_tmt2));
		json_object_add_value_int(
		    root, "TMT2 Transition Counter",
		    le32_to_cpu(smart_log.thm_temp2_trans_count));
		json_object_add_value_int(
		    root, "TMT2 Total Time",
		    le32_to_cpu(smart_log.thm_temp2_total_time));
		json_object_add_value_int(root, "Thermal Shutdown Threshold",
					  95);
...
static int wdc_capabilities(int argc, char **argv, struct command *command,
			    struct plugin *plugin)
{
	...
	printf("cap-diag                      : %s\n",
	       capabilities & WDC_DRIVE_CAP_CAP_DIAG ? "Supported"
						     : "Not Supported");
	printf("drive-log                     : %s\n",
	       capabilities & WDC_DRIVE_CAP_DRIVE_LOG ? "Supported"
						      : "Not Supported");
	printf("get-crash-dump                : %s\n",
	       capabilities & WDC_DRIVE_CAP_CRASH_DUMP ? "Supported"
						       : "Not Supported");
	printf("get-pfail-dump                : %s\n",
	       capabilities & WDC_DRIVE_CAP_PFAIL_DUMP ? "Supported"
						       : "Not Supported");
	printf("id-ctrl                       : Supported\n");
	printf("purge                         : %s\n",
	       capabilities & WDC_DRIVE_CAP_PURGE ? "Supported"
						  : "Not Supported");
	printf("purge-monitor                 : %s\n",
	       capabilities & WDC_DRIVE_CAP_PURGE ? "Supported"
						  : "Not Supported");
	printf("vs-internal-log               : %s\n",
	       capabilities & WDC_DRIVE_CAP_INTERNAL_LOG_MASK
		   ? "Supported"
		   : "Not Supported");
	printf("vs-nand-stats                 : %s\n",
	       capabilities & WDC_DRIVE_CAP_NAND_STATS ? "Supported"
						       : "Not Supported");
	printf("vs-smart-add-log              : %s\n",
	       capabilities & WDC_DRIVE_CAP_SMART_LOG_MASK ? "Supported"
							   : "Not Supported");
	printf("--C0 Log Page                 : %s\n",
	       capabilities & WDC_DRIVE_CAP_C0_LOG_PAGE ? "Supported"
							: "Not Supported");
	printf("--C1 Log Page                 : %s\n",
	       capabilities & WDC_DRIVE_CAP_C1_LOG_PAGE ? "Supported"
							: "Not Supported");
	printf("--C3 Log Page                 : %s\n",
	       capabilities & WDC_DRIVE_CAP_C3_LOG_PAGE ? "Supported"
							: "Not Supported");
	printf("--CA Log Page                 : %s\n",
	       capabilities & WDC_DRIVE_CAP_CA_LOG_PAGE ? "Supported"
							: "Not Supported");
	printf("--D0 Log Page                 : %s\n",
	       capabilities & WDC_DRIVE_CAP_D0_LOG_PAGE ? "Supported"
							: "Not Supported");
	printf("clear-pcie-correctable-errors : %s\n",
	       capabilities & WDC_DRIVE_CAP_CLEAR_PCIE_MASK ? "Supported"
							    : "Not Supported");
	printf("drive-essentials              : %s\n",
	       capabilities & WDC_DRIVE_CAP_DRIVE_ESSENTIALS ? "Supported"
							     : "Not Supported");
	printf("get-drive-status              : %s\n",
	       capabilities & WDC_DRIVE_CAP_DRIVE_STATUS ? "Supported"
							 : "Not Supported");
	printf("clear-assert-dump             : %s\n",
	       capabilities & WDC_DRIVE_CAP_CLEAR_ASSERT ? "Supported"
							 : "Not Supported");
	printf("drive-resize                  : %s\n",
	       capabilities & WDC_DRIVE_CAP_RESIZE ? "Supported"
						   : "Not Supported");
	printf("vs-fw-activate-history        : %s\n",
	       capabilities & WDC_DRIVE_CAP_FW_ACTIVATE_HISTORY_MASK
		   ? "Supported"
		   : "Not Supported");
	printf("clear-fw-activate-history     : %s\n",
	       capabilities & WDC_DRIVE_CAP_CLEAR_FW_ACT_HISTORY_MASK
		   ? "Supported"
		   : "Not Supported");
	printf("vs-telemetry-controller-option: %s\n",
	       capabilities & WDC_DRVIE_CAP_DISABLE_CTLR_TELE_LOG
		   ? "Supported"
		   : "Not Supported");
	printf("vs-error-reason-identifier    : %s\n",
	       capabilities & WDC_DRIVE_CAP_REASON_ID ? "Supported"
						      : "Not Supported");
	printf("log-page-directory            : %s\n",
	       capabilities & WDC_DRIVE_CAP_LOG_PAGE_DIR ? "Supported"
							 : "Not Supported");
	printf("namespace-resize              : %s\n",
	       capabilities & WDC_DRIVE_CAP_NS_RESIZE ? "Supported"
						      : "Not Supported");
	printf("vs-drive-info                 : %s\n",
	       capabilities & (WDC_DRIVE_CAP_INFO | WDC_DRIVE_CAP_INFO_2)
		   ? "Supported"
		   : "Not Supported");
	printf("vs-temperature-stats          : %s\n",
	       capabilities & WDC_DRIVE_CAP_TEMP_STATS ? "Supported"
						       : "Not Supported");
	printf("cloud-SSD-plugin-version      : %s\n",
	       capabilities & WDC_DRIVE_CAP_CLOUD_SSD_VERSION
		   ? "Supported"
		   : "Not Supported");
	printf("vs-pcie-stats                 : %s\n",
	       capabilities & WDC_DRIVE_CAP_PCIE_STATS ? "Supported"
						       : "Not Supported");
					...
					struct plugin *plugin)
{
	...
	if ((capabilities & WDC_DRIVE_CAP_CLOUD_SSD_VERSION) ==
	    WDC_DRIVE_CAP_CLOUD_SSD_VERSION) {
		/* print command and supported status */
			   ...
			   struct plugin *plugin)
{
	...
	OPT_ARGS(opts) = {OPT_FILE("output-file", 'o', &cfg.file, file),
			  OPT_UINT("transfer-size", 's', &cfg.xfer_size, size),
			  OPT_UINT("log-id", 'l', &cfg.log_id, log), OPT_END()};
	...
	if (cfg.log_id > 0xff) {
		fprintf(stderr,
			"Invalid log identifier: %d. Valid 0xd1, 0xd2, 0xd3, "
			"0xd4, 0xe2, 0xe4\n",
			cfg.log_id);
		goto closed_fd;
	}
	...
	if (cfg.xfer_size != 0) {
		...
		if (!wdc_check_power_of_2(cfg.xfer_size)) {
			fprintf(
			    stderr,
			    "%s: ERROR : xfer-size (%d) must be a power of 2\n",
	...
	if (cfg.log_id) {
		xfer_size =
		    (xfer_size) ? xfer_size : WDC_NVME_ENC_LOG_SIZE_CHUNK;
		...
		if (cfg.log_id == WDC_ENC_NIC_CRASH_DUMP_ID_SLOT_1 ||
			...
			err = wdc_enc_get_nic_log(fd, cfg.log_id, xfer_size,
						  WDC_NVME_ENC_NIC_LOG_SIZE,
						  output_fd);
		} else {
			fprintf(stderr, "args - sz:%x logid:%x of:%s\n",
				xfer_size, cfg.log_id, cfg.file);
			err = wdc_enc_submit_move_data(fd, NULL, 0, xfer_size,
						       output_fd, cfg.log_id, 0,
						       0);
		}
		...
		if (err == WDC_RESULT_NOT_AVAILABLE) {
			...
			fprintf(stderr,
				"ERROR:0x%x Failed to collect log-id:%x \n",
				err, cfg.log_id);
		}
	}
closed_fd:
...
static int wdc_enc_submit_move_data(int fd, char *cmd, int len, int xfer_size,
				    FILE *out, int log_id, int cdw14, int cdw15)
{
	...
	struct nvme_passthru_cmd nvme_cmd = {
	    ...
	    .data_len = ((len + sizeof(uint32_t) - 1) / sizeof(uint32_t)) *
			sizeof(uint32_t),
	    .cdw10 = len,
	    .cdw12 = log_id,
	    .cdw13 = 0,
	...
	srand(time.tv_nsec);
	handle = random(); /* Handle to associate send request with receive
			      request */
	...
	printf("NVME_ADMIN_COMMAND:\n"
	       "opcode: 0x%02x, flags: 0x%02x, rsvd: 0x%04x, nsid: 0x%08x, "
	       "cdw2: 0x%08x, cdw3: 0x%08x, "
	       "metadata_len: 0x%08x, data_len: 0x%08x, cdw10: 0x%08x, cdw11: "
	       "0x%08x, cdw12: 0x%08x, "
	       "cdw13: 0x%08x, cdw14: 0x%08x, cdw15: 0x%08x, timeout_ms: "
	       "0x%08x, result: 0x%08x, "
	       "metadata: %s, "
	       "data: %s\n",
	       nvme_cmd.opcode, nvme_cmd.flags, nvme_cmd.rsvd1, nvme_cmd.nsid,
	       nvme_cmd.cdw2, nvme_cmd.cdw3, nvme_cmd.metadata_len,
	       nvme_cmd.data_len, nvme_cmd.cdw10, nvme_cmd.cdw11,
	       nvme_cmd.cdw12, nvme_cmd.cdw13, nvme_cmd.cdw14, nvme_cmd.cdw15,
	...
	if (err == NVME_SC_INTERNAL) {
		...
		do {
			...
			nvme_cmd.result = 0; /* returned result !=0 indicates
						more data available */
			err = nvme_submit_admin_passthru(fd, &nvme_cmd, NULL);
			if (err != 0) {
				more = 0;
				fprintf(stderr,
					"%s: ERROR : WDC : NVMe Rcv Mgmt ",
					__func__);
				nvme_show_status(err);
			} else {
				more = nvme_cmd.result & WDC_RESULT_MORE_DATA;
				response_size =
				    nvme_cmd.result & ~WDC_RESULT_MORE_DATA;
				fwrite(buf, response_size, 1, out);
				offset += response_size;
				if (more &&
				    (response_size & (sizeof(uint32_t) - 1))) {
					fprintf(stderr,
						"%s: ERROR : WDC : NVMe Rcv "
						"Mgmt response size:x%x not LW "
						"aligned\n",
			       ...
			       __u32 data_len, FILE *out)
{
	...
	while (curr_data_offset < data_len) {
		...
		fprintf(
		    stderr,
		    "nsid 0x%08x addr 0x%08llx, data_len 0x%08x, cdw10 0x%08x, "
		    "cdw11 0x%08x, cdw12 0x%08x, cdw13 0x%08x, cdw14 0x%08x \n",
		    admin_cmd.nsid, admin_cmd.addr, admin_cmd.data_len,
		    admin_cmd.cdw10, admin_cmd.cdw11, admin_cmd.cdw12,
		    admin_cmd.cdw13, admin_cmd.cdw14);
#endif
		ret = nvme_submit_admin_passthru(fd, &admin_cmd, NULL);
		if (ret != 0) {
			nvme_show_status(ret);
