...
static int wdc_get_serial_name(struct libnvme_transport_handle *hdl, char *file, size_t len, const char *suffix);
static int wdc_create_log_file(const char *file, const __u8 *drive_log_data,
			       __u32 drive_log_length);
static int wdc_do_clear_dump(struct libnvme_transport_handle *hdl, __u8 opcode, __u32 cdw12);
static int wdc_do_dump(struct libnvme_transport_handle *hdl, __u32 opcode, __u32 data_len, __u32 cdw12,
		       const char *file, __u32 xfer_size);
static int wdc_do_crash_dump(struct libnvme_transport_handle *hdl, char *file, int type);
static int wdc_crash_dump(struct libnvme_transport_handle *hdl, const char *file, int type);
static int wdc_get_crash_dump(int argc, char **argv, struct command *acmd,
			      struct plugin *plugin);
static int wdc_do_drive_log(struct libnvme_transport_handle *hdl, const char *file);
...
static int wdc_clear_pcie_correctable_errors(int argc, char **argv, struct command *acmd,
					     struct plugin *plugin);
static int wdc_do_drive_essentials(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl, char *dir, char *key);
...
static int wdc_do_drive_resize(struct libnvme_transport_handle *hdl, uint64_t new_size);
static int wdc_namespace_resize(int argc, char **argv, struct command *acmd,
				struct plugin *plugin);
static int wdc_do_namespace_resize(struct libnvme_transport_handle *hdl, __u32 nsid, __u32 op_option);
static int wdc_reason_identifier(int argc, char **argv, struct command *acmd,
				 struct plugin *plugin);
static int wdc_do_get_reason_id(struct libnvme_transport_handle *hdl, const char *file, int log_id);
static int wdc_save_reason_id(struct libnvme_transport_handle *hdl, __u8 *rsn_ident,  int size);
static int wdc_clear_reason_id(struct libnvme_transport_handle *hdl);
static int wdc_log_page_directory(int argc, char **argv, struct command *acmd,
				  struct plugin *plugin);
static int wdc_do_drive_info(struct libnvme_transport_handle *hdl, __u32 *result);
...
static int wdc_vs_temperature_stats(int argc, char **argv, struct command *acmd,
				    struct plugin *plugin);
static __u64 wdc_get_enc_drive_capabilities(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl);
static int wdc_enc_get_nic_log(struct libnvme_transport_handle *hdl, __u8 log_id, __u32 xfer_size, __u32 data_len,
			       FILE *out);
static int wdc_enc_submit_move_data(struct libnvme_transport_handle *hdl, char *cmd, int len, int xfer_size,
				    FILE *out, int data_id, int cdw14, int cdw15);
static bool get_dev_mgment_cbs_data(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl, __u8 log_id,
				    void **cbs_data);
static __u32 wdc_get_fw_cust_id(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl);
...
static bool get_dev_mgment_data(struct libnvme_global_ctx *ctx,
		struct libnvme_transport_handle *hdl,
		void **data);
...
static bool wdc_check_device(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl)
{
	int ret;
	bool supported;
...
static bool wdc_enc_check_model(struct libnvme_transport_handle *hdl)
{
	int ret;
	bool supported;
...
static __u64 wdc_get_drive_capabilities(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl)
{
	int ret;
	uint32_t read_device_id = -1, read_vendor_id = -1;
...
static __u64 wdc_get_enc_drive_capabilities(struct libnvme_global_ctx *ctx,
					    struct libnvme_transport_handle *hdl)
{
	int ret;
			       ...
			       __u32 drive_log_length)
{
	...
	fd = open(file, O_NVME_WRONLY | O_CREAT | O_TRUNC, 0666);
	if (fd < 0) {
...
static bool get_dev_mgmt_log_page_data(struct libnvme_transport_handle *hdl, void **log_data,
				       __u8 uuid_ix)
{
...
static bool get_dev_mgmt_log_page_lid_data(struct libnvme_transport_handle *hdl,
	void **cbs_data,
	__u8 lid,
	__u8 log_id,
	__u8 uuid_ix)
...
static bool get_dev_mgment_data(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl,
				void **data)
{
	bool found = false;
...
static bool get_dev_mgment_cbs_data(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl,
...
static int wdc_get_supported_log_pages(struct libnvme_transport_handle *hdl,
...
static bool wdc_nvme_get_dev_status_log_data(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl, __le32 *ret_data,
					     __u8 log_id)
{
	__u32 *cbs_data = NULL;
...
static __u32 wdc_dump_length(struct libnvme_transport_handle *link, __u32 opcode, __u32 cdw10, __u32 cdw12, __u32 *dump_length)
{
	int ret;
...
static __u32 wdc_dump_length_e6(struct libnvme_transport_handle *hdl, __u32 opcode, __u32 cdw10, __u32 cdw12, struct wdc_e6_log_hdr *dump_hdr)
{
	int ret;
	struct libnvme_passthru_cmd admin_cmd;
...
static __u32 wdc_dump_dui_data(struct libnvme_transport_handle *hdl, __u32 dataLen, __u32 offset, __u8 *dump_data, bool last_xfer)
{
	int ret;
	struct libnvme_passthru_cmd admin_cmd;
...
static __u32 wdc_dump_dui_data_v2(struct libnvme_transport_handle *hdl, __u32 dataLen, __u64 offset, __u8 *dump_data, bool last_xfer)
{
	int ret;
	struct libnvme_passthru_cmd admin_cmd;
...
static int wdc_do_dump(struct libnvme_transport_handle *hdl, __u32 opcode, __u32 data_len,
...
static int wdc_do_dump_e6(struct libnvme_transport_handle *hdl, __u32 opcode, __u32 data_len,
...
static int wdc_do_cap_telemetry_log(struct libnvme_global_ctx *ctx,
		...
		__u32 bs, int type, int data_area)
{
	...
	output = open(file, O_NVME_WRONLY | O_CREAT | O_TRUNC, 0666);
	if (output < 0) {
...
static int wdc_do_cap_diag(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl, char *file,
...
static int wdc_do_cap_dui_v1(struct libnvme_transport_handle *hdl, char *file, __u32 xfer_size, int data_area, int verbose,
...
static int wdc_do_cap_dui_v2_v3(struct libnvme_transport_handle *hdl, char *file, __u32 xfer_size, int data_area, int verbose,
...
static int wdc_do_cap_dui_v4(struct libnvme_transport_handle *hdl, char *file, __u32 xfer_size, int data_area, int verbose,
			     struct wdc_dui_log_hdr *log_hdr, __s64 *total_size, __u64 file_size,
			     __u64 offset)
{
	__s64 log_size = 0;
...
static int wdc_do_cap_dui(struct libnvme_transport_handle *hdl, char *file, __u32 xfer_size, int data_area, int verbose,
...
static int wdc_do_get_sn730_log_len(struct libnvme_transport_handle *hdl, uint32_t *len_buf, uint32_t subopcode)
{
	int ret;
...
static int wdc_do_get_sn730_log(struct libnvme_transport_handle *hdl, void *log_buf, uint32_t offset, uint32_t subopcode)
{
	int ret;
	uint8_t *output = NULL;
	struct libnvme_passthru_cmd admin_cmd;
...
static int get_sn730_log_chunks(struct libnvme_transport_handle *hdl, uint8_t *log_buf, uint32_t log_len, uint32_t subopcode)
{
	int ret = 0;
...
static int dump_internal_logs(struct libnvme_transport_handle *hdl, const char *dir_name, int verbose)
{
	...
	output = open(file_path, O_NVME_WRONLY | O_CREAT | O_TRUNC, 0666);
	if (output < 0) {
				  ...
				  struct plugin *plugin)
{
	...
	if (!wdc_is_sn861(device_id)) {
		if (cfg.file) {
			...
			verify_file = open(cfg.file, O_NVME_WRONLY | O_CREAT | O_TRUNC, 0666);
			if (verify_file < 0) {
				fprintf(stderr, "ERROR: WDC: open: %s\n", libnvme_strerror(errno));
				goto out;
			}
...
static int wdc_print_latency_monitor_log_normal(struct libnvme_transport_handle *hdl,
...
static void wdc_print_bd_ca_log_normal(struct libnvme_transport_handle *hdl, void *data)
{
	...
	while (valid_id) {
		...
		case 0x0:
			printf("Additional Smart Log for NVME device:%s namespace-id:%x\n",
					libnvme_transport_handle_get_name(hdl), WDC_DE_GLOBAL_NSID);
...
static int nvme_get_print_ocp_cloud_smart_log(struct libnvme_transport_handle *hdl,
		int uuid_index,
		__u32 namespace_id,
		int fmt)
{
...
static int nvme_get_print_c0_eol_log(struct libnvme_transport_handle *hdl,
				int uuid_index,
				__u32 namespace_id,
				int fmt)
{
...
static int nvme_get_ext_smart_cloud_log(struct libnvme_transport_handle *hdl, __u8 **data, int uuid_index, __u32 namespace_id)
{
	struct libnvme_passthru_cmd cmd;
	__u8 *log_ptr = NULL;
	int ret, i;
...
static int nvme_get_hw_rev_log(struct libnvme_transport_handle *hdl, __u8 **data, int uuid_index, __u32 namespace_id)
{
...
static int wdc_get_c0_log_page_sn(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl, int uuid_index, char *format,
...
static int wdc_print_latency_monitor_log(struct libnvme_transport_handle *hdl,
					 struct wdc_ssd_latency_monitor_log *log_data,
					 int fmt)
{
	if (!log_data) {
...
static int wdc_get_c1_log_page(struct libnvme_global_ctx *ctx,
...
static int wdc_get_c3_log_page(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl, char *format)
{
	struct wdc_ssd_latency_monitor_log *log_data;
	nvme_print_flags_t fmt;
	__u8 *data;
...
static int wdc_get_ocp_c1_log_page(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl, char *format)
{
...
static int wdc_get_ocp_c4_log_page(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl, char *format)
{
...
static int wdc_get_ocp_c5_log_page(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl, char *format)
{
...
static int wdc_get_d0_log_page(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl, char *format)
{
...
static int wdc_do_clear_pcie_correctable_errors(struct libnvme_transport_handle *hdl)
{
	int ret;
	struct libnvme_passthru_cmd admin_cmd;
...
static int wdc_do_clear_pcie_correctable_errors_vuc(struct libnvme_transport_handle *hdl)
{
	int ret;
	struct libnvme_passthru_cmd admin_cmd;
...
static int wdc_do_clear_pcie_correctable_errors_fid(struct libnvme_transport_handle *hdl)
{
	int ret;
	__u64 result;
...
static int wdc_get_fw_act_history(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl,
				  char *format)
{
...
static int wdc_get_fw_act_history_C2(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl,
				     char *format)
{
...
static int wdc_do_clear_fw_activate_history_vuc(struct libnvme_transport_handle *hdl)
{
	int ret = -1;
	struct libnvme_passthru_cmd admin_cmd;
...
static int wdc_do_clear_fw_activate_history_fid(struct libnvme_transport_handle *hdl)
{
	int ret = -1;
	__u64 result;
...
static int wdc_de_VU_read_size(struct libnvme_transport_handle *hdl, __u32 fileId, __u16 spiDestn, __u32 *logSize)
{
...
static int wdc_de_VU_read_buffer(struct libnvme_transport_handle *hdl, __u32 fileId, __u16 spiDestn,
...
static int wdc_get_log_dir_max_entries(struct libnvme_transport_handle *hdl, __u32 *maxNumOfEntries)
{
	int ret = WDC_STATUS_FAILURE;
...
static int wdc_fetch_log_directory(struct libnvme_transport_handle *hdl, struct WDC_DE_VU_LOG_DIRECTORY *directory)
{
	int ret = WDC_STATUS_FAILURE;
...
static int wdc_fetch_log_file_from_device(struct libnvme_transport_handle *hdl, __u32 fileId,
...
static int wdc_de_get_dump_trace(struct libnvme_transport_handle *hdl, const char *filePath, __u16 binFileNameLen,
...
int wdc_fetch_vu_file_directory(struct libnvme_transport_handle *hdl,
...
int wdc_read_debug_directory(struct libnvme_transport_handle *hdl, __s8 *bufferFolderPath, __u8 *serialNo,
			     __u8 *timeString)
{
...
static int wdc_do_drive_essentials(struct libnvme_global_ctx *ctx, struct libnvme_transport_handle *hdl,
				   char *dir, char *key)
{
	int ret = 0;
	void *retPtr;
...
static int wdc_clear_reason_id(struct libnvme_transport_handle *hdl)
{
	...
	verify_file = open(reason_id_file, O_NVME_WRONLY | O_CREAT | O_TRUNC, 0666);
	if (verify_file < 0) {
		ret = -1;
		goto free;
	}
...
static int wdc_dump_telemetry_hdr(struct libnvme_transport_handle *hdl, int log_id, struct nvme_telemetry_log *log_hdr)
{
	int ret = 0;
...
static int wdc_do_vs_nand_stats_sn810_2(struct libnvme_transport_handle *hdl, char *format)
{
	nvme_print_flags_t fmt;
	uint8_t *data = NULL;
	int ret;
...
static int wdc_do_vs_nand_stats(struct libnvme_transport_handle *hdl, char *format)
{
	nvme_print_flags_t fmt;
	uint8_t *output = NULL;
	__u16 version = 0;
...
static int wdc_do_vs_pcie_stats(struct libnvme_transport_handle *hdl,
...
static int wdc_enc_submit_move_data(struct libnvme_transport_handle *hdl, char *cmd, int len,
...
static int wdc_enc_get_nic_log(struct libnvme_transport_handle *hdl, __u8 log_id, __u32 xfer_size, __u32 data_len, FILE *out)
{
	__u8 *dump_data;
	__u32 curr_data_offset, curr_data_len;
	int i, ret = -1;
