		...
		__u32 bs, int type, int data_area)
{
	...
	output = open(file, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0666);
	if (output < 0) {
...
static int dump_internal_logs(struct libnvme_transport_handle *hdl, const char *dir_name, int verbose)
{
	...
	output = open(file_path, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0666);
	if (output < 0) {
				  ...
				  struct plugin *plugin)
{
	...
	if (!wdc_is_sn861(device_id)) {
		if (cfg.file) {
			...
			verify_file = open(cfg.file, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0666);
			if (verify_file < 0) {
				fprintf(stderr, "ERROR: WDC: open: %s\n", libnvme_strerror(errno));
				goto out;
			}
...
static int wdc_clear_reason_id(struct libnvme_transport_handle *hdl)
{
	...
	verify_file = open(reason_id_file, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0666);
	if (verify_file < 0) {
		ret = -1;
		goto free;
	}
