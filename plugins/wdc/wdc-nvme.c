...
#define WDC_BD_CA_LOG_BUF_LEN                                                  \
	0xA0 /* Added 4 padding bytes to resolve build warning messages */
...
static __u8 wdc_lat_mon_guid[WDC_C3_GUID_LENGTH] = { 0x92, 0x7a, 0xc0, 0x8c,
						     0xd0, 0x84, 0x6c, 0x9c,
						     0x70, 0x43, 0xe6, 0xd4,
						     0x58, 0x5e, 0xd4, 0x85 };
...
static __u8 scao_guid[WDC_C2_GUID_LENGTH] = { 0xC5, 0xAF, 0x10, 0x28,
					      0xEA, 0xBF, 0xF2, 0xA4,
					      0x9C, 0x4F, 0x6F, 0x7C,
					      0xC9, 0x14, 0xD5, 0xAF };
...
{
	return denominator ?
		       (uint64_t)(((double)numerator / (double)denominator) *
				  100) :
			     0;
}
			       ...
			       const char *suffix)
{
	...
	res_len = snprintf(file, len, "%s%.*s%s", orig, ctrl_sn_len, ctrl.sn,
			   suffix);
	if (len <= res_len) {
		fprintf(stderr,
...
bool wdc_get_dev_mng_log_entry(
	__u32 log_length, __u32 entry_id,
	...
	struct wdc_c2_log_subpage_header **p_p_found_log_entry)
{
	...
	if (*p_p_found_log_entry == NULL) {
		fprintf(stderr,
			"ERROR : WDC - wdc_get_dev_mng_log_entry: No ppLogEntry pointer.\n");
		return false;
	}
				    ...
				    void **cbs_data)
{
	...
	*cbs_data = NULL;
	...
	if (ret) {
		fprintf(stderr,
			"ERROR : WDC : Unable to get 0x%x Log Page length, ret = 0x%x\n",
			lid, ret);
		goto end;
	}
	...
	if (ret) {
		fprintf(stderr,
			"ERROR : WDC : Unable to read 0x%x Log Page data, ret = 0x%x\n",
			lid, ret);
		goto end;
	}
		...
		*cbs_data = (void *)&sph->data;
		...
		if (found) {
			*cbs_data = (void *)&sph->data;
			...
			fprintf(stderr,
				"ERROR : WDC : Unable to find correct version of page 0x%x, entry id = %d\n",
				lid, log_id);
		}
	}
end:
					      ...
					      __u8 log_id)
{
	...
	if (get_dev_mgment_cbs_data(r, fd, WDC_C2_LOG_PAGES_SUPPORTED_ID,
		...
		if (cbs_data != NULL) {
			...
			if (!found) {
				fprintf(stderr,
					"ERROR : WDC : Log Page 0x%x not supported\n",
					log_id);
		       ...
		       char *file, __u32 xfer_size)
{
	...
	while (curr_data_offset < data_len) {
		...
		if (ret != 0) {
			nvme_show_status(ret);
			fprintf(stderr,
				"%s: ERROR : WDC : Get chunk %d, size = 0x%x, offset = 0x%x, addr = 0x%lx\n",
				    ...
				    int data_area)
{
	...
	if (type == WDC_TELEMETRY_TYPE_HOST) {
		...
		err = nvme_get_features_data(
			fd, WDC_VU_DISABLE_CNTLR_TELEMETRY_OPTION_FEATURE_ID, 0,
			4, buf, &result);
		if (err == 0) {
			if (result == 0) {
				/* enabled */
				...
				fprintf(stderr,
					"%s: Controller initiated option telemetry log page disabled\n",
					__func__);
				err = -EINVAL;
				goto close_fd;
			}
			...
			fprintf(stderr,
				"ERROR : WDC: Get telemetry option feature failed.");
	...
	while (offset < full_size) {
		...
		if (err != bs) {
			fprintf(stderr,
				"%s: Failed to flush telemetry data to file!, err = %d\n",
				__func__, err);
			break;
		}
		err = 0;
			   ...
			   int type, int data_area)
{
	...
	if (type == WDC_TELEMETRY_TYPE_NONE) {
		...
		cap_diag_length =
			(log_hdr->log_size[0] << 24 |
			 log_hdr->log_size[1] << 16 |
		...
		if (cap_diag_length == 0) {
			...
			fprintf(stderr,
				"INFO : WDC : Capture Diagnostics log, length = 0x%x\n",
				cap_diag_length);
		}
			  ...
			  int verbose, __u64 file_size, __u64 offset)
{
	...
	if ((log_hdr->hdr_version & 0xFF) == 0x00 ||
		...
		if (verbose) {
			fprintf(stderr,
				"INFO : WDC : Capture V1 Device Unit Info log, data area = %d\n",
				data_area);
			fprintf(stderr,
		...
		if (cap_dui_length == 0) {
			fprintf(stderr,
				"INFO : WDC : Capture V1 Device Unit Info log is empty\n");
		} else {
			...
			if (dump_data == NULL) {
				fprintf(stderr,
					"%s: ERROR : dump data V1 malloc failed : status %s, size = 0x%x\n",
			...
			if (err != WDC_NVME_CAP_DUI_HEADER_SIZE) {
				fprintf(stderr,
					"%s:  Failed to flush header data to file!\n",
					__func__);
				goto free_mem;
			}
			...
			for (; log_size > 0; log_size -= xfer_size) {
				...
				if (ret != 0) {
					fprintf(stderr,
						"%s: ERROR : WDC : Get chunk %d, size = 0x%" PRIx64
						", offset = 0x%x, addr = %p\n",
		...
		if (cap_dui_length_v3 == 0) {
			fprintf(stderr,
				"INFO : WDC : Capture V2 or V3 Device Unit Info log is empty\n");
		} else {
			/* parse log header for all sections up to specified data area inclusively */
			if (data_area != WDC_NVME_DUI_MAX_DATA_AREA) {
		...
		if (verbose) {
			fprintf(stderr,
				"INFO : WDC : Capture V4 Device Unit Info log, data area = %d\n",
				data_area);
			fprintf(stderr,
				"INFO : WDC : DUI Header Version = 0x%x\n",
		...
		if (cap_dui_length_v4 == 0) {
			fprintf(stderr,
				"INFO : WDC : Capture V4 Device Unit Info log is empty\n");
		} else {
			/* parse log header for all sections up to specified data area inclusively */
			if (data_area != WDC_NVME_DUI_MAX_DATA_AREA) {
				for (j = 0; j < WDC_NVME_DUI_MAX_SECTION; j++) {
					if (log_hdr_v4->log_section[j]
						...
						if (verbose)
							fprintf(stderr,
								"%s: Data area ID %d : section size 0x%x sectors, section size 0x%" PRIx64
								" bytes, total size = 0x%" PRIx64
								"\n",
								__func__,
								log_hdr_v4
									->log_section
			...
			if (dump_data == NULL) {
				fprintf(stderr,
					"%s: ERROR : dump data V4 malloc failed : status %s, size = 0x%x\n",
					__func__, strerror(errno),
	...
	if (verbose)
		fprintf(stderr,
			"INFO : WDC : Capture Device Unit Info log, length = 0x%" PRIx64
			"\n",
			(uint64_t)total_size);
...
static int wdc_do_sn730_get_and_tar(int fd, char *outputName)
{
	...
	if (ret) {
		fprintf(stderr,
			"ERROR : WDC : create directory failed, ret = %d, dir = %s\n",
				  ...
				  struct plugin *plugin)
{
	...
	char *type =
		"Telemetry type - NONE, HOST, or CONTROLLER. Currently only supported on the SN640 and SN840 devices.";
	...
	OPT_ARGS(
		opts) = { OPT_FILE("output-file", 'o', &cfg.file, file),
			  OPT_UINT("transfer-size", 's', &cfg.xfer_size, size),
...
static int wdc_do_crash_dump(int fd, char *file, int type)
{
	...
	if (type == WDC_NVME_PFAIL_DUMP_TYPE) {
		...
		cdw12 = (WDC_NVME_PF_CRASH_DUMP_SUBCMD
			 << WDC_NVME_SUBCMD_SHIFT) |
	...
	int fd, struct wdc_ssd_latency_monitor_log *log_data)
{
	...
	if (!err)
		...
		fprintf(stderr,
			"ERROR : WDC : latency monitor read id ctrl failure, err = %d\n",
			err);
		return err;
	}
	...
	for (i = 0; i <= 3; i++) {
		...
		for (j = 0; j <= 2; j++) {
			if (le64_to_cpu(
				...
				wdc_convert_ts(
					le64_to_cpu(
						log_data->active_latency_timestamp
							[i][j]),
					ts_buf);
				printf("%s     ", ts_buf);
			}
	...
	for (i = 0; i <= 3; i++) {
		...
		for (j = 0; j <= 2; j++) {
			if (le64_to_cpu(
				...
				wdc_convert_ts(
					le64_to_cpu(
						log_data->static_latency_timestamp
							[i][j]),
					ts_buf);
				printf("%s     ", ts_buf);
			}
	...
	for (i = 0; i <= 3; i++) {
		for (j = 0; j <= 2; j++) {
			...
			json_object_add_value_int(
				root, buf,
				le32_to_cpu(
					log_data->active_bucket_counter[i][j]));
		}
	}
	for (i = 0; i <= 3; i++) {
		for (j = 0; j <= 2; j++) {
			...
			json_object_add_value_int(
				root, buf,
				le16_to_cpu(
					log_data->active_measured_latency[i]
									 [j]));
		}
	}
	for (i = 0; i <= 3; i++) {
		for (j = 0; j <= 2; j++) {
			...
			json_object_add_value_int(
				root, buf,
				le64_to_cpu(
					log_data->active_latency_timestamp[i]
									  [j]));
		}
	}
	for (i = 0; i <= 3; i++) {
		for (j = 0; j <= 2; j++) {
			...
			json_object_add_value_int(
				root, buf,
				le32_to_cpu(
					log_data->static_bucket_counter[i][j]));
		}
	}
	for (i = 0; i <= 3; i++) {
		for (j = 0; j <= 2; j++) {
			...
			json_object_add_value_int(
				root, buf,
				le16_to_cpu(
					log_data->static_measured_latency[i]
									 [j]));
		}
	}
	for (i = 0; i <= 3; i++) {
		for (j = 0; j <= 2; j++) {
			...
			json_object_add_value_int(
				root, buf,
				le64_to_cpu(
					log_data->static_latency_timestamp[i]
									  [j]));
		}
	}
...
static void wdc_print_bd_ca_log_json(void *data)
{
	...
	if (bd_data->field_id == 0x03) {
		raw = (__u64 *)&bd_data->raw_value[1];
		json_object_add_value_int(
			root, "end_to_end_error_detection_count normalized",
			bd_data->normalized_value);
		json_object_add_value_int(
			root, "end_to_end_error_detection_count raw",
			le64_to_cpu(*raw & 0x00FFFFFFFFFFFFFF));
	} else {
		goto invalid_id;
	}
	...
	if (bd_data->field_id == 0x05) {
		raw = (__u64 *)&bd_data->raw_value[1];
		json_object_add_value_int(
			root, "timed_workload_media_wear normalized",
			bd_data->normalized_value);
		json_object_add_value_float(
			root, "timed_workload_media_wear raw",
			safe_div_fp((*raw & 0x00FFFFFFFFFFFFFF), 1024.0));
	} else {
		goto invalid_id;
	}
	...
	if (bd_data->field_id == 0x06) {
		raw = (__u64 *)&bd_data->raw_value[1];
		json_object_add_value_int(
			root, "timed_workload_host_reads normalized",
			bd_data->normalized_value);
	...
	if (bd_data->field_id == 0x07) {
		raw = (__u64 *)&bd_data->raw_value[1];
		json_object_add_value_int(root,
					  "timed_workload_timer normalized",
					  bd_data->normalized_value);
	...
	if (bd_data->field_id == 0x09) {
		raw = (__u64 *)&bd_data->raw_value[1];
		json_object_add_value_int(
			root, "retry_buffer_overflow_count normalized",
			bd_data->normalized_value);
		json_object_add_value_int(
			root, "retry_buffer_overflow_count raw",
			le64_to_cpu(*raw & 0x00FFFFFFFFFFFFFF));
	} else {
		goto invalid_id;
	}
	...
	if (bd_data->field_id == 0x0A) {
		raw = (__u64 *)&bd_data->raw_value[1];
		json_object_add_value_int(root,
					  "pll_lock_loss_count normalized",
					  bd_data->normalized_value);
...
static void wdc_print_d0_log_normal(struct wdc_ssd_d0_smart_log *perf)
{
	...
	printf("  Lifetime Read Disturb Reallocation Events	 %20" PRIu32
	       ...
	       (uint32_t)le32_to_cpu(
		       perf->lifetime_read_disturb_realloc_events));
	...
	if (data[0] == WDC_NVME_GET_FW_ACT_HISTORY_C2_LOG_ID) {
		...
		for (i = 0; i < num_entries; i++) {
			...
			if (cust_id == WDC_CUSTOMER_ID_0x1005) {
				...
				uint64_t timestamp =
					(0x0000FFFFFFFFFFFF &
					 le64_to_cpu(fw_act_history_entry
							     ->entry[entryIdx]
							     .timestamp));
				memset((void *)time_str, 0, 9);
				sprintf((char *)time_str, "%04d:%02d:%02d",
			...
			printf("%2" PRIu8 "",
			       (uint8_t)fw_act_history_entry->entry[entryIdx]
				       .slot_number);
			printf("   ");
			wdc_get_commit_action_bin(fw_act_history_entry
							  ->entry[entryIdx]
							  .commit_action_type,
						  (char *)&commit_action_bin);
			printf("  %s", (char *)commit_action_bin);
			printf("  ");
			if (le16_to_cpu(fw_act_history_entry->entry[entryIdx]
						.result) == 0)
				printf("pass");
			else
				printf("fail #%d",
		...
		struct wdc_fw_act_history_log_entry *fw_act_history_entry =
			(struct wdc_fw_act_history_log_entry
				 *)(data +
				    sizeof(struct wdc_fw_act_history_log_hdr));
		...
		for (i = 0; i < num_entries; i++) {
			...
			printf("%2" PRIu8 "",
			       (uint8_t)fw_act_history_entry[entryIdx]
				       .slot_number);
			printf("  ");
			wdc_get_commit_action_bin(fw_act_history_entry[entryIdx]
							  .commit_action_type,
						  (char *)&commit_action_bin);
			printf("  %s", (char *)commit_action_bin);
			printf("   ");
			       ...
			       int uuid_index, __u32 namespace_id)
{
	...
	case WDC_NVME_SN840_DEV_ID_1:
		...
		if ((*cust_id == WDC_CUSTOMER_ID_0x1004) ||
				...
				if (ret == 0) {
					...
					for (i = 0; i < 16; i++) {
						if (scao_guid[i] !=
						    data[SCAO_LPG + i]) {
							fprintf(stderr,
								"ERROR : WDC : Unknown GUID in C0 Log Page data\n");
							int j;
							fprintf(stderr,
			...
			if ((data = (__u8 *)malloc(
				     sizeof(__u8) *
				     WDC_NVME_EOL_STATUS_LOG_LEN)) == NULL) {
				fprintf(stderr, "ERROR : WDC : malloc : %s\n",
					strerror(errno));
				return -1;
			}
					...
					__u32 cust_id, __u32 vendor_id)
{
	if (!data) {
		fprintf(stderr,
			"ERROR : WDC : Invalid buffer to read fw activate history entries\n");
		return -1;
	}
...
static int wdc_get_ca_log_page(nvme_root_t r, int fd, char *format)
{
	...
	case WDC_NVME_SN200_DEV_ID:
		...
		if (cust_id == WDC_CUSTOMER_ID_0x1005) {
			...
			if (ret == 0) {
				...
				fprintf(stderr,
					"ERROR : WDC : Unable to read CA Log Page data\n");
				ret = -1;
			}
		} else {
			fprintf(stderr,
				"ERROR : WDC : Unsupported Customer id, id = 0x%x\n",
				cust_id);
			return -1;
		}
		break;
	...
	default:
		...
		fprintf(stderr,
			"ERROR : WDC : Log page 0xCA not supported for this device\n");
		return -1;
		break;
	}
	...
	if (ret == 0) {
		...
		for (i = 0; i < 16; i++) {
			if (wdc_lat_mon_guid[i] != log_data->log_page_guid[i]) {
				fprintf(stderr,
					"ERROR : WDC : Unknown GUID in C3 Log Page data\n");
				int j;
				fprintf(stderr,
					"ERROR : WDC : Expected GUID:  0x");
				for (j = 0; j < 16; j++) {
				...
				struct plugin *plugin)
{
	...
	OPT_ARGS(opts) = { OPT_UINT("interval", 'i', &cfg.interval, interval),
			   OPT_FMT("output-format", 'o', &cfg.output_format,
				   "Output Format: normal|json"),
			   OPT_BYTE("log-page-version", 'l',
				    &cfg.log_page_version, log_page_version),
			   OPT_LIST("log-page-mask", 'p', &cfg.log_page_mask,
				    log_page_mask),
	...
	if (cfg.log_page_version == 0) {
		...
		fprintf(stderr,
			"ERROR : WDC: unsupported log page version for this command\n");
		ret = -1;
		goto out;
	}
	...
	if (((capabilities & (WDC_DRIVE_CAP_CA_LOG_PAGE)) ==
		...
		if (ret)
			fprintf(stderr,
				"ERROR : WDC : Failure reading the CA Log Page, ret = %d\n",
				ret);
	}
	if (((capabilities & WDC_DRIVE_CAP_C1_LOG_PAGE) ==
	    ...
	    (page_mask & WDC_C1_PAGE_MASK)) {
		/* Get the C1 Log Page */
		ret = wdc_get_c1_log_page(r, fd, cfg.output_format,
					  cfg.interval);
		if (ret)
			fprintf(stderr,
				"ERROR : WDC : Failure reading the C1 Log Page, ret = %d\n",
				ret);
	}
	if (((capabilities & WDC_DRIVE_CAP_D0_LOG_PAGE) ==
		...
		if (ret)
			fprintf(stderr,
				"ERROR : WDC : Failure reading the D0 Log Page, ret = %d\n",
				ret);
	}
	...
	if (ret == 0) {
		...
		if ((fw_act_history_hdr->num_entries > 0) &&
			...
			ret = wdc_print_fw_act_history_log(
				data, fw_act_history_hdr->num_entries, fmt, 0,
				0);
		else if (fw_act_history_hdr->num_entries == 0) {
			fprintf(stderr,
				"INFO : WDC : No FW Activate History entries found.\n");
			ret = 0;
		} else {
			fprintf(stderr,
		...
		fprintf(stderr,
			"ERROR : WDC : Unable to read FW Activate History Log Page data\n");
		ret = -1;
	}
...
static int wdc_get_fw_act_history_C2(nvme_root_t r, int fd, char *format)
{
	...
	if ((data = (__u8 *)malloc(sizeof(__u8) *
				   WDC_FW_ACT_HISTORY_C2_LOG_BUF_LEN)) ==
	    NULL) {
		fprintf(stderr, "ERROR : WDC : malloc : %s\n", strerror(errno));
		return -1;
	}
				      ...
				      struct plugin *plugin)
{
	...
	if (ret)
		fprintf(stderr,
			"ERROR : WDC : Failure reading the FW Activate History, ret = %d\n",
			ret);
out:
	nvme_free_tree(r);
	return ret;
	...
	if (cfg.disable) {
		...
		if (cfg.enable) {
			...
			if (ret == 0) {
				if (result)
					fprintf(stderr,
						"Controller Option Telemetry Log Page State: Disabled\n");
				else
					fprintf(stderr,
						"Controller Option Telemetry Log Page State: Enabled\n");
			} else {
				nvme_show_status(ret);
			}
		} else {
			fprintf(stderr,
				"ERROR : WDC: unsupported option for this command\n");
			fprintf(stderr,
...
static int wdc_get_log_dir_max_entries(int fd, __u32 *maxNumOfEntries)
{
	...
	if (WDC_STATUS_SUCCESS !=
	    (ret = wdc_de_VU_read_size(fd, 0, 5,
				       (__u32 *)&headerPayloadSize))) {
		fprintf(stderr,
	...
	if ((fileSize >= maximumTransferLength) || (fileSize > 0xFFFFFFFF)) {
		...
		for (offsetIdx = 0; (offsetIdx * chunckSize) < fileSize;
			...
			ret = wdc_de_VU_read_buffer(
				fd, fileId, spiDestn,
				(__u32)((offsetIdx * chunckSize) /
					sizeof(__u32)),
				dataBuffer + (offsetIdx * chunckSize),
				&buffSize);
			if (ret != WDC_STATUS_SUCCESS) {
				fprintf(stderr,
					"ERROR : WDC : %s: wdc_de_VU_read_buffer failed with ret = %d, fileId = 0x%x, fileSize = 0x%lx\n",
					__func__, ret, fileId,
				 ...
				 char *binFileName)
{
	...
	do {
		...
		if (ret != WDC_STATUS_SUCCESS) {
			fprintf(stderr,
				"ERROR : WDC : %s: wdc_de_VU_read_size failed with ret = %d\n",
				__func__, ret);
			break;
		}
		...
		if (dumptraceSize < 4) {
			...
			fprintf(stderr,
				"ERROR : WDC : %s: wdc_de_VU_read_size failed, read size is less than 4 bytes, dumptraceSize = 0x%x\n",
	...
	if (ret == WDC_STATUS_SUCCESS) {
		...
		if (ret != WDC_STATUS_SUCCESS)
			fprintf(stderr,
				"ERROR : WDC : %s: wdc_WriteToFile failed, ret = %d\n",
				__func__, ret);
	} else {
		fprintf(stderr,
...
static int wdc_do_drive_essentials(nvme_root_t r, int fd, char *dir, char *key)
{
	...
	if (wdc_get_serial_and_fw_rev(fd, (char *)idSerialNo,
		...
		fprintf(stderr,
			"Get Drive Essentials Data for device serial #: %s and fw revision: %s\n",
			idSerialNo, idFwRev);
	}
	...
	if (dir != NULL) {
		...
		if (retPtr != NULL)
			wdc_UtilsSnprintf((char *)bufferFolderPath,
					  MAX_PATH_LEN, "%s%s%s",
					  (char *)currDir,
					  WDC_DE_PATH_SEPARATOR,
					  (char *)bufferFolderName);
	...
	for (vuLogIdx = 0; vuLogIdx < vuLogInput->numOfVULogPages; vuLogIdx++) {
		...
		if (ret) {
			fprintf(stderr,
				"ERROR : WDC : nvme_get_log() for log page 0x%x failed, ret = %d\n",
	...
	for (listIdx = 1;
		...
		if (ret) {
			fprintf(stderr,
				"ERROR : WDC : nvme_get_feature id 0x%x failed, ret = %d\n",
				deFeatureIdList[listIdx].featureId, ret);
		} else {
			wdc_UtilsSnprintf(fileName, MAX_PATH_LEN,
					  "%s%s%s0x%x_%s_%s_%s.bin",
					  (char *)bufferFolderPath,
					  WDC_DE_PATH_SEPARATOR, "FEATURE_ID_",
					  deFeatureIdList[listIdx].featureId,
					  deFeatureIdList[listIdx].featureName,
					  serialNo, timeString);
		...
		if (ret == WDC_STATUS_SUCCESS) {
			...
			for (listIdx = 0;
				...
				if (0 == deEssentialsList.logEntry[listIdx]
						 .metaData.fileSize) {
					fprintf(stderr,
						"ERROR : WDC : File Size for %s is 0\n",
						deEssentialsList
							.logEntry[listIdx]
							.metaData.fileName);
					...
					if (ret == WDC_STATUS_SUCCESS) {
						...
						fprintf(stderr,
							"ERROR : WDC : wdc_fetch_log_file_from_device: %s failed, ret = %d\n",
							deEssentialsList
								.logEntry[listIdx]
								.metaData
								.fileName,
	...
	if (ret) {
		fprintf(stderr,
			"ERROR : WDC : Tar of Drive Essentials data failed, ret = %d\n",
			ret);
	}
				...
				struct plugin *plugin)
{
	...
	OPT_ARGS(opts) = { OPT_STRING("dir-name", 'd', "DIRECTORY",
				      &cfg.dirName, dirName),
			   OPT_END() };
				 ...
				 struct plugin *plugin)
{
	...
	if (cfg.log_id != NVME_LOG_LID_TELEMETRY_HOST &&
		...
		fprintf(stderr,
			"ERROR : WDC: Invalid Log ID. It must be 7 (Host) or 8 (Controller)\n");
		ret = -1;
		goto close_fd;
	}
				  ...
				  struct nvme_telemetry_log *log_hdr)
{
	...
	if (ret < 0)
		...
		nvme_show_status(ret);
		fprintf(stderr,
			"%s: ERROR : Failed to acquire telemetry header, ret = %d!\n",
			__func__, ret);
	}
			     ...
			     struct plugin *plugin)
{
	...
	if ((capabilities & WDC_DRIVE_CAP_NAND_STATS) == 0) {
		...
		if (ret)
			fprintf(stderr,
				"ERROR : WDC : Failure reading NAND statistics, ret = %d\n",
				ret);
	}
			     ...
			     struct plugin *plugin)
{
	...
	if ((capabilities & WDC_DRIVE_CAP_PCIE_STATS) == 0) {
		...
		if (ret)
			fprintf(stderr,
				"ERROR : WDC : Failure reading PCIE statistics, ret = 0x%x\n",
				ret);
		else {
			/* parse the data */
			switch (fmt) {
			     ...
			     struct plugin *plugin)
{
	...
	if ((capabilities & WDC_DRIVE_CAP_INFO) == WDC_DRIVE_CAP_INFO) {
		...
		if (!ret) {
			size = (__u16)((cpu_to_le32(result) & 0xffff0000) >>
				       16);
				    ...
				    struct plugin *plugin)
{
	...
	temperature =
		((smart_log.temperature[1] << 8) | smart_log.temperature[0]) -
		273;

	/* retrieve HCTM Thermal Management Temperatures */
	nvme_get_features_simple(fd, 0x10, 0, &hctm_tmt);
	temp_tmt1 = ((hctm_tmt >> 16) & 0xffff) ?
			    ((hctm_tmt >> 16) & 0xffff) - 273 :
				  0;
				    ...
				    FILE *out, int log_id, int cdw14, int cdw15)
{
	...
	srand(time.tv_nsec);
	handle =
		random(); /* Handle to associate send request with receive request */
	...
	if (err == NVME_SC_INTERNAL) {
		...
		do {
			...
			nvme_cmd.result =
				0; /* returned result !=0 indicates more data available */
			err = nvme_submit_admin_passthru(fd, &nvme_cmd, NULL);
			if (err != 0) {
				more = 0;
				fprintf(stderr,
				...
				if (more &&
					...
					fprintf(stderr,
						"%s: ERROR : WDC : NVMe Rcv Mgmt response size:x%x not LW aligned\n",
