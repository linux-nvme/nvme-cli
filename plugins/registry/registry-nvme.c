...
{
	printf("%s\n", device);
	libnvmf_registry_attr_for_each(device, print_attr, NULL);
}
			  ...
			  struct plugin *plugin)
{
	...
	return libnvmf_registry_device_for_each(print_device, NULL);
}
			      ...
			      struct plugin *plugin)
{
	...
	ret = libnvmf_registry_retrieve(cfg.device, cfg.attr, &value);
	if (ret == -ENOENT) {
			    ...
			    struct plugin *plugin)
{
	...
	ret = libnvmf_registry_update(cfg.device, cfg.attr, cfg.value);
	if (ret)
