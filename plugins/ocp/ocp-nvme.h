...
PLUGIN(NAME("ocp", "OCP cloud SSD extensions", OCP_PLUGIN_VERSION),
	COMMAND_LIST(
		...
		ENTRY("clear-pcie-correctable-errors", "Clear PCIe correctable error counters",
		      clear_pcie_correctable_error_counters)
		ENTRY("device-capability-log", "Get Device capabilities Requirements Log Page",
		      ocp_device_capabilities_log)
		ENTRY("eol-plp-failure-mode", "Define EOL or PLP circuitry failure mode.",
		      eol_plp_failure_mode)
		ENTRY("error-recovery-log", "Retrieve Error Recovery Log Page",
		      ocp_error_recovery_log)
		ENTRY("fw-activate-history", "Get firmware activation history log",
		      fw_activation_history_log)
		ENTRY("get-dssd-async-event-config", "Get DSSD Async Event Config",
		      get_dssd_async_event_config)
		ENTRY("get-dssd-power-state-feature", "Get DSSD Power State feature",
		      get_dssd_power_state_feature)
		ENTRY("get-enable-ieee1667-silo", "return set of enable IEEE1667 silo",
		      get_enable_ieee1667_silo)
		ENTRY("get-error-injection", "Return set of error injection",
		      get_error_injection)
		ENTRY("get-plp-health-check-interval", "Get PLP Health Check Interval",
		      get_plp_health_check_interval)
		ENTRY("hardware-component-log", "retrieve hardware component log",
		      hwcomp_log)
		ENTRY("internal-log", "Retrieve and save internal device telemetry log",
		      ocp_telemetry_log)
		ENTRY("latency-monitor-log", "Get Latency Monitor Log Page",
		      ocp_latency_monitor_log)
		ENTRY("set-dssd-async-event-config", "Set DSSD Async Event Config",
		      set_dssd_async_event_config)
		ENTRY("set-dssd-power-state-feature", "Set DSSD Power State feature",
		      set_dssd_power_state_feature)
		ENTRY("set-enable-ieee1667-silo", "enable IEEE1667 silo",
		      set_enable_ieee1667_silo)
		ENTRY("set-error-injection", "Inject error conditions",
		      set_error_injection)
		ENTRY("set-latency-monitor-feature", "Set Latency Monitor feature",
		      ocp_set_latency_monitor_feature)
		ENTRY("set-plp-health-check-interval", "Set PLP Health Check Interval",
		      set_plp_health_check_interval)
		ENTRY("set-telemetry-profile", "Set Telemetry Profile Feature",
		      ocp_set_telemetry_profile_feature)
		ENTRY("smart-add-log", "Retrieve extended SMART Information",
		      smart_add_log)
		ENTRY("tcg-configuration-log", "Retrieve TCG Configuration Log Page",
		      ocp_tcg_configuration_log)
		ENTRY("telemetry-string-log", "Retrieve Telemetry string Log Page",
		      ocp_telemetry_str_log_format)
		ENTRY("unsupported-reqs-log", "Get Unsupported Requirements Log Page",
		      ocp_unsupported_requirements_log)
	)
);
