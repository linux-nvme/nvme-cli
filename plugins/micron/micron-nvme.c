...
static void WriteData(__u8 *data, __u32 len, const char *dir, const char *file,
		      const char *msg)
{
...
static int ZipAndRemoveDir(char *strDirName, char *strFileName)
{
	...
	if (strstr(strFileName, ".tar.gz") || strstr(strFileName, ".tgz")) {
		...
		sprintf(strBuffer, "zip -r \"%s\" \"%s\" >temp.txt 2>&1",
	...
	if (strchr(strFilePath, '/') != NULL) {
		...
		if (fileName != NULL) {
			...
			while (strFilePath[nIndex] != '\0') {
				if ('\\' == strFilePath[nIndex] &&
				    '\\' == strFilePath[nIndex + 1]) {
					goto exit_status;
				}
				nIndex++;
			}
			...
			length =
			    (int)strlen(strFilePath) - (int)strlen(fileName);

			if (fileName == strFilePath) {
				length = 1;
			}

			if ((fileLocation = (char *)malloc(length + 1)) ==
			    NULL) {
				goto exit_status;
			}
			...
			if (':' == fileLocation[length - 1]) {
				if ((strTemp = (char *)malloc(length + 2)) ==
				    NULL) {
					goto exit_status;
				}
...
static int GetLogPageSize(int nFD, unsigned char ucLogID, int *nLogSize)
{
	...
	if (ucLogID == 0xC1 || ucLogID == 0xC2 || ucLogID == 0xC4) {
		err =
		    nvme_get_log_simple(nFD, ucLogID, CommonChunkSize, pTmpBuf);
		if (err == 0) {
			pLogHeader = (LogPageHeader_t *)pTmpBuf;
			LogPageHeader_t *pLogHeader1 =
			    (LogPageHeader_t *)pLogHeader;
			*nLogSize =
			    (int)(pLogHeader1->numDwordsInEntireLogPage) * 4;
			if (pLogHeader1->logPageHeaderFormatVersion == 0) {
				printf("Unsupported log page format version %d "
				       "of log page : 0x%X\n",
				       ucLogID, err);
				*nLogSize = 0;
				err = -1;
			...
			printf(
			    "Getting size of log page : 0x%X failed with %d\n",
			    ucLogID, err);
			*nLogSize = 0;
		}
	}
...
static int NVMEGetLogPage(int nFD, unsigned char ucLogID,
			  unsigned char *pBuffer, int nBuffSize)
{
	int err = 0;
	struct nvme_passthru_cmd cmd = {0};
	unsigned int uiNumDwords =
	    (unsigned int)nBuffSize / sizeof(unsigned int);
			...
			long long llMaxSize)
{
	...
	while (err == 0 && llMaxSize > 0) {
		err = NVMEGetLogPage(nFD, ucLogID, (unsigned char *)pBuffer,
				     nBufferSize);
		if (err)
			return err;
 ...
 */
static int
micron_parse_options(int argc, char **argv, const char *desc,
...
static int micron_selective_download(int argc, char **argv, struct command *cmd,
				     struct plugin *plugin)
{
	const char *desc =
	    "This performs a selective firmware download, which allows the "
	    "user to "
	    "select which firmware binary to update for 9200 devices. This "
	    "requires "
	    "a power cycle once the update completes. The options available "
	    "are: \n\n"
	...
	OPT_ARGS(opts) = {
	    ...
	    OPT_STRING("select", 's', "flag", &cfg.select, select), OPT_END()};
	...
	if (err == 0x10B || err == 0x20B) {
		err = 0;
		fprintf(stderr, "Update successful! Power cycle for changes to "
				"take effect\n");
	}
...
static int micron_smbus_option(int argc, char **argv, struct command *cmd,
			       struct plugin *plugin)
{
	__u32 result = 0;
	__u32 cdw10 = 0;
	__u32 cdw11 = 0;
	const char *desc =
	    "Enable/Disable/Get status of SMBUS option on controller";
	const char *option = "enable or disable or status";
	const char *value =
	    "1 - hottest component temperature, 0 - composite "
	...
	OPT_ARGS(opts) = {
	    ...
	    OPT_UINT("save", 's', &opt.save, save), OPT_END()};
	...
	if (!strcmp(opt.option, "enable")) {
		cdw11 = opt.value << 1 | 1;
		err = nvme_set_features_simple(fd, fid, 1, cdw11, opt.save,
					       &result);
		if (err == 0) {
		...
		if (err == 0) {
			printf("SMBus status on the drive: %s (returns %s "
			       "temperature) \n",
			       (result & 1) ? "enabled" : "disabled",
			       (result & 2) ? "hottest component"
					    : "composite");
		} else {
			printf(
			    "Failed to retrieve SMBus status on the drive\n");
		}
		...
		printf("Invalid option %s, valid values are enable, disable or "
		       "status\n",
		       opt.option);
		close(fd);
		return -1;
			     ...
			     struct plugin *plugin)
{
	...
	const char *desc =
	    "Retrieve Micron temperature info for the given device ";
	...
	OPT_ARGS(opts) = {OPT_FMT("format", 'f', &cfg.fmt, fmt), OPT_END()};
	...
	if (!err) {
		...
		for (i = 0; i < SensorCount; i++) {
			...
			tempSensors[i] =
			    tempSensors[i] ? tempSensors[i] - 273 : 0;
		}
		if (is_json) {
			...
			json_object_add_value_array(
			    root, "Micron temperature information", logPages);
			sprintf(tempstr, "%u C", temperature);
			json_object_add_value_string(
			    stats, "Current Composite Temperature", tempstr);
			for (i = 0; i < SensorCount; i++) {
				char sensor_str[256] = {0};
				char datastr[64] = {0};
				sprintf(sensor_str, "Temperature Sensor #%d",
					(i + 1));
				sprintf(datastr, "%u C", tempSensors[i]);
				json_object_add_value_string(stats, sensor_str,
							     datastr);
			}
...
static int micron_pcie_stats(int argc, char **argv, struct command *cmd,
			     struct plugin *plugin)
{
	int i, fd, err = 0, bus = 0, domain = 0, device = 0, function = 0,
		   ctrlIdx;
	...
	if (is_json) {
		...
		for (i = 0; i < sizeof(pcie_correctable_errors) /
				    sizeof(pcie_correctable_errors[0]);
		     i++) {
			json_object_add_value_int(
			    stats, pcie_correctable_errors[i].err,
			    ((correctable_errors >>
			      pcie_correctable_errors[i].bit) &
			     1));
		}
		for (i = 0; i < sizeof(pcie_uncorrectable_errors) /
				    sizeof(pcie_uncorrectable_errors[0]);
		     i++) {
			json_object_add_value_int(
			    stats, pcie_uncorrectable_errors[i].err,
			    ((uncorrectable_errors >>
			      pcie_uncorrectable_errors[i].bit) &
			     1));
		}
						...
						struct plugin *plugin)
{
	...
	if (model == M51CX) {
		err = nvme_set_features_simple(fd, fid, 0, (1 << 31), false,
					       &result);
		if (err == 0 && (err = (int)result) == 0)
			...
			printf(
			    "Error clearing Device correctable errors = 0x%x\n",
			    err);
		goto out;
	}
	...
	err = snprintf(strTempFile, sizeof(strTempFile), "/sys/block/%s/device",
		       devicename);
	if (err < 0)
		goto out;
...
static struct logpage {
	const char *field;
	char datastr[128];
} d0_log_page[] = {{"NAND Writes (Bytes Written)", {0}},
...
static void init_d0_log_page(__u8 *buf, __u8 nsze)
{
	...
	if (count_hi != 0)
		...
		sprintf(d0_log_page[1].datastr, "0x%" PRIx64,
...
)
{
	...
	for (field = 0; field < field_count; field++) {
		...
		char *sfield = NULL;
		int size =
		    (spec == 0) ? log_page[field].size : log_page[field].size2;
		if (size == 0)
			continue;
		...
		if (size == 16) {
			if (strstr(sfield, "GUID")) {
				...
				if (lval_hi)
					sprintf(datastr,
						"0x%" PRIx64 "%016" PRIx64 "",
						le64_to_cpu(lval_hi),
						le64_to_cpu(lval_lo));
				else
					sprintf(datastr, "0x%" PRIx64 "",
						le64_to_cpu(lval_lo));
			}
		} else if (size == 8) {
			lval_lo = *((__u64 *)(&buf[offset]));
			...
			/* 7 bytes will be in little-endian format, with last
			 * byte as MSB */
...
static void print_smart_cloud_health_log(__u8 *buf, bool is_json)
{
	...
	if (is_json) {
		...
		logPages = json_create_array();
		json_object_add_value_array(
		    root, "OCP SMART Cloud Health Log: 0xC0", logPages);
	}
				...
				__u8 spec)
{
	...
	if (is_json) {
		...
		logPages = json_create_array();
		json_object_add_value_array(
		    root, "Extended Smart Log Page : 0xFB", logPages);
	}
...
static void print_nand_stats_d0(__u8 *buf, __u8 oacs, bool is_json)
{
	...
	if (is_json) {
		...
		json_object_add_value_array(
		    root, "Extended Smart Log Page : 0xD0", logPages);
...
static int micron_nand_stats(int argc, char **argv, struct command *cmd,
			     struct plugin *plugin)
{
	...
	if (has_fb_log) {
		...
		print_nand_stats_fb((__u8 *)logFB, (__u8 *)extSmartLog, nsze,
				    is_json, spec);
	} else if (has_d0_log) {
...
static void print_ext_smart_logs_e1(__u8 *buf, bool is_json)
{
	...
	if (is_json) {
		...
		stats = json_create_object();
		logPages = json_create_array();
		json_object_add_value_array(root, "SMART Extended Log:0xE1",
					    logPages);
	} else {
...
static int micron_smart_ext_log(int argc, char **argv, struct command *cmd,
				struct plugin *plugin)
{
			 ...
			 struct nvme_id_ctrl *ctrlp)
{
	...
	sprintf(
	    strBuffer,
	    "%-20s : /dev/nvme%d\n%-20s : %s\n%-20s : %-20s\n%-20s : %-20s\n",
	    "Device Name", nFD, "Model No", (char *)model, "Serial No",
	    (char *)serial, "FW-Rev", (char *)fwrev);
	...
	sprintf(strBuffer, "%-22s : %04X\n%-22s : %04X\n", "VendorId",
		vendor_id, "DeviceId", device_id);
	fprintf(fpOutFile, "%s", strBuffer);
	fclose(fpOutFile);
	free(strPDir);
}
...
static void GetTimestampInfo(const char *strOSDirName)
{
	...
	if (num) {
		...
		WriteData(outstr, num, strDest, "timestamp_info.txt",
			  "timestamp");
		free(strPDir);
	}
}
...
static void GetGenericLogs(int fd, const char *dir)
{
	...
	err =
	    nvme_get_log_persistent_event(fd, NVME_PEVENT_LOG_EST_CTX_AND_READ,
					  sizeof(pevent_log), &pevent_log);
	if (err) {
		fprintf(stderr,
			"Failed to set persistent event log read context");
		return;
	}
	...
	err = nvme_get_log_persistent_event(fd, NVME_PEVENT_LOG_READ, log_len,
					    pevent_log_info);
	if (err == 0) {
...
static void GetOSConfig(const char *strOSDirName)
{
	...
	struct {
	    ...
	    {(char *)"LINUX SYSTEM MEMORY INFORMATION",
	     (char *)"cat /proc/meminfo >> %s"},
	    {(char *)"SYSTEM INTERRUPT INFORMATION",
	     (char *)"cat /proc/interrupts >> %s"},
	    {(char *)"CPU INFORMATION", (char *)"cat /proc/cpuinfo >> %s"},
	    {(char *)"IO MEMORY MAP INFORMATION",
	     (char *)"cat /proc/iomem >> %s"},
	    {(char *)"MAJOR NUMBER AND DEVICE GROUP",
	     (char *)"cat /proc/devices >> %s"},
	    {(char *)"KERNEL DMESG", (char *)"dmesg >> %s"},
	    {(char *)"/VAR/LOG/MESSAGES",
	     (char *)"cat /var/log/messages >> %s"}};
...
static int micron_telemetry_log(int fd, __u8 type, __u8 **data, int *logSize,
				int da)
{
	...
	data_area[0] =
	    data_area[1] > data_area[2] ? data_area[1] : data_area[2];
	data_area[0] =
	    data_area[3] > data_area[0] ? data_area[3] : data_area[0];
	...
	*logSize = data_area[da] * bs;
	offset = bs;
	err = 0;
	if ((buffer = (unsigned char *)realloc(buffer, (size_t)(*logSize))) !=
	    NULL) {
		while (err == 0 && offset != *logSize) {
			if (ctrl_init)
				err = nvme_get_log_telemetry_ctrl(
				    fd, true, 0, *logSize, buffer + offset);
			else
				err = nvme_get_log_telemetry_host(
				    fd, 0, *logSize, buffer + offset);
			offset += bs;
		}
	}
...
static int GetTelemetryData(int fd, const char *dir)
{
	...
	for (i = 0; i < (int)(sizeof(tmap) / sizeof(tmap[0])); i++) {
		err =
		    micron_telemetry_log(fd, tmap[i].log, &buffer, &logSize, 0);
...
static int GetFeatureSettings(int fd, const char *dir)
{
	...
	for (i = 0; i < (int)(sizeof(fmap) / sizeof(fmap[0])); i++) {
		...
		if (err == 0) {
			...
			WriteData((__u8 *)&attrVal, sizeof(attrVal), dir,
				  fmap[i].file, msg);
			if (bufp != NULL) {
				WriteData(bufp, len, dir, fmap[i].file, msg);
			}
		} else {
			printf("Feature 0x%x data not retrieved, error %d "
			       "(ignored)!\n",
			       fmap[i].id, err);
			errcnt++;
		}
			     ...
			     struct plugin *plugin)
{
	...
	if (model == M5407) {
		...
		if (err) {
			fprintf(stderr,
				"ERROR : drive-info opcode failed with 0x%x\n",
				err);
			return -1;
		}
	} else {
		...
		if (err) {
			fprintf(stderr,
				"ERROR : identify_ctrl() failed with 0x%x\n",
				err);
			return -1;
		}
	...
	if (is_json) {
		...
		json_object_add_value_array(root, "Micron Drive HW Information",
					    driveInfo);
		sprintf(tempstr, "%hhu.%hhu", dinfo.hw_ver_major,
			dinfo.hw_ver_minor);
		json_object_add_value_string(pinfo, "Drive Hardware Version",
					     tempstr);

		if (dinfo.ftl_unit_size) {
			sprintf(tempstr, "%hhu KB", dinfo.ftl_unit_size);
			json_object_add_value_string(pinfo, "FTL_unit_size",
						     tempstr);
		}

		if (dinfo.bs_ver_major != 0 || dinfo.bs_ver_minor != 0) {
			sprintf(tempstr, "%hhu.%hhu", dinfo.bs_ver_major,
				dinfo.bs_ver_minor);
			json_object_add_value_string(pinfo, "Boot Spec.Version",
						     tempstr);
		}
				 ...
				 struct plugin *plugin)
{
	printf("nvme-cli Micron plugin version: %s.%s.%s\n", __version_major,
	       __version_minor, __version_patch);
	return 0;
}
...
static int display_fw_activate_entry(int entry_count,
				     ...
				     struct json_object *stats)
{
	...
	if (entry->version != 1 || entry->length != 64) {
		fprintf(stderr,
			"unsupported entry ! version: %x with length: %d\n",
	...
	timestamp =
	    (le64_to_cpu(entry->power_on_hour) & 0x0000FFFFFFFFFFFFUL) / 1000;
	hours = timestamp / 3600;
	minutes = (timestamp % 3600) / 60;
	seconds = (timestamp % 3600) % 60;
	sprintf(ptr, "|%" PRIu64 ":%hhu:%hhu", (uint64_t)hours, minutes,
		seconds);
	ptr += 12;
...
static int micron_fw_activation_history(int argc, char **argv,
					struct command *cmd,
					struct plugin *plugin)
{
	const char *desc =
	    "Retrieve Firmware Activation history of the given drive";
	...
	err = -EINVAL;
	if (eModel != M51CX) {
		fprintf(stderr, "Unsupported drive model for "
				"vs-fw-activate-history command\n");
		goto out;
	}
	...
	if (err) {
		fprintf(
		    stderr,
		    "Failed to retrieve fw activation history log, error: %x\n",
		    err);
		goto out;
	}
	...
	if (table->version != 2 || table->log_page != 0xC2) {
		fprintf(
		    stderr,
		    "Unsupported fw activation history page: %x, version: %x\n",
	...
	if (table->num_entries == 0) {
		fprintf(stderr,
			"No entries were found in fw activation history log\n");
		goto out;
	}
...
static int micron_ocp_smart_health_logs(int argc, char **argv,
					struct command *cmd,
					struct plugin *plugin)
{
	const char *desc =
	    "Retrieve Smart or Extended Smart Health log for the given device ";
	...
	if (eModel == M5410 || eModel == M5407) {
		...
		if (err) {
			if (err < 0)
				printf("Unable to retrieve smart log 0xFB for "
				       "the drive\n");
			goto out;
		}
	...
	if (eModel != M51CX) {
		printf(
		    "Unsupported drive model for vs-smart-add-log commmand\n");
		err = -1;
		goto out;
	}
	...
	if (err == 0) {
		...
		printf("Unable to retrieve extended smart log 0xC0 for the "
		       "drive\n");
	}
out:
	close(fd);
					    ...
					    struct plugin *plugin)
{
	...
	// err = nvme_set_feature(fd, 1, fid, cdw11, 0, opt.save, 0, 0,
	// &result);
					 ...
					 struct plugin *plugin)
{
	...
	__u32 result = 0;
	const char *desc =
	    "Enable or Disable Controller telemetry log generation";
	const char *option = "enable or disable or status";
	const char *select =
	    "select/save values: enable/disable options"
	...
	OPT_ARGS(opts) = {
	    OPT_STRING("option", 'o', "option", &opt.option, option),
	    OPT_UINT("select", 's', &opt.select, select), OPT_END()};
	...
	err = nvme_identify_ctrl(fd, &ctrl);
	if ((ctrl.lpa & 0x8) != 0x8) {
		printf("drive doesn't support host/controller generated "
		       "telemetry logs\n");
		close(fd);
		return err;
	}
	...
	if (!strcmp(opt.option, "enable")) {
		err = nvme_set_features(fd, fid, 1, 1, 0, (opt.select & 0x1), 0,
					0, 0, 0, NVME_DEFAULT_IOCTL_TIMEOUT,
					&result);
		if (err == 0) {
			printf(
			    "successfully set controller telemetry option\n");
		} else {
			printf("Failed to set controller telemetry option\n");
		}
	} else if (!strcmp(opt.option, "disable")) {
		err = nvme_set_features(fd, fid, 1, 0, 0, (opt.select & 0x1), 0,
					0, 0, 0, NVME_DEFAULT_IOCTL_TIMEOUT,
					&result);
		if (err == 0) {
			printf("successfully disabled controller telemetry "
			       "option\n");
		} else {
			printf(
			    "Failed to disable controller telemetry option\n");
		}
	} else if (!strcmp(opt.option, "status")) {
		opt.select &= 0x3;
		...
		if (err == 0) {
			...
			printf(
			    "Failed to retrieve controller telemetry option\n");
		}
	} else {
		printf("invalid option %s, valid values are enable,disable or "
		       "status\n",
				...
				struct plugin *plugin)
{
	...
	struct {
		...
		int nLogSize;
		int nMaxSize;
	} aVendorLogs[32] = {{0x03, "firmware_slot_info_log.bin", 512, 0},
	  ...
	  aM51XXLogs[] = {{0xFB, "nvmelog_FB.bin", 4096,
			   0}, /* this should be collected first for M51AX */
	  ...
	  aM51AXLogs[] = {{0xCA, "nvmelog_CA.bin", 512, 0},
	  ...
	  aM51BXLogs[] = {{0xFA, "nvmelog_FA.bin", 4096, 16376},
	...
	OPT_ARGS(opts) = {
	    ...
	    OPT_UINT("data_area", 'd', &cfg.data_area, data_area), OPT_END()};
	...
	if (strlen(cfg.type) != 0) {
		if (!strcmp(cfg.type, "controller")) {
			...
			printf("telemetry type (host or controller) should be "
			       "specified i.e. -t=host\n");
			close(fd);
			goto out;
		}

		if (cfg.data_area <= 0 || cfg.data_area > 3) {
			printf("data area must be selected using -d option ie "
			       "--d=1,2,3\n");
			close(fd);
			goto out;
		}
		...
		printf("data area option is valid only for telemetry option "
		       "(i.e --type=host|controller)\n");
		close(fd);
		goto out;
	}
	...
	if (strlen(cfg.package) == 0) {
		if (telemetry_option)
			printf("Log data file must be specified. ie "
			       "-p=logfile.bin\n");
		else
			printf("Log data file must be specified. ie "
			       "-p=logfile.zip or "
			       "-p=logfile.tgz|logfile.tar.gz\n");
		goto out;
	}
	...
	if ((eModel = GetDriveModel(ctrlIdx)) == UNKNOWN_MODEL) {
		printf(
		    "Unsupported drive model for vs-internal-log collection\n");
		close(fd);
		goto out;
	}
	...
	if (telemetry_option) {
		if ((ctrl.lpa & 0x8) != 0x8) {
			printf("telemetry option is not supported for "
			       "specified drive\n");
			close(fd);
			goto out;
		}
		...
		__u8 *buffer = NULL;
		const char *dir = ".";
		err = micron_telemetry_log(fd, cfg.log, &buffer, &logSize,
					   cfg.data_area);
		if (err == 0 && logSize > 0 && buffer != NULL) {
	...
	SetupDebugDataDirectories(ctrl.sn, cfg.package, strMainDirName,
				  strOSDirName, strCtrlDirName);
	...
	if (eModel != M5410 && eModel != M5407) {
		...
		if (eModel == M51AX)
			memcpy((char *)aVendorLogs + sizeof(aM51XXLogs),
			       aM51AXLogs, sizeof(aM51AXLogs));
		else
			memcpy((char *)aVendorLogs + sizeof(aM51XXLogs),
			       aM51BXLogs, sizeof(aM51BXLogs));
	}

	for (int i = 0;
	     i < (int)(sizeof(aVendorLogs) / sizeof(aVendorLogs[0])) &&
	     aVendorLogs[i].ucLogPage != 0;
	     i++) {
		err = -1;
		...
		case 0xC2:
		case 0xC4:
			err = GetLogPageSize(fd, aVendorLogs[i].ucLogPage,
					     &bSize);
			if (err == 0 && bSize > 0)
				err = GetCommonLogPage(fd,
		...
		case 0xE7:
			...
			if (uiMask == 0 ||
			    (aVendorLogs[i].ucLogPage == 0xE6 && uiMask == 2) ||
				...
				if (bSize % (16 * 1024)) {
					bSize +=
					    (16 * 1024) - (bSize % (16 * 1024));
				}
			}
			if (bSize != 0 && (dataBuffer = (unsigned char *)malloc(
					       bSize)) != NULL) {
				memset(dataBuffer, 0, bSize);
				if (eModel == M5410 || eModel == M5407)
					err = NVMEGetLogPage(
					    fd, aVendorLogs[i].ucLogPage,
					    dataBuffer, bSize);
				else
					err = nvme_get_log_simple(
					    fd, aVendorLogs[i].ucLogPage, bSize,
					    dataBuffer);
			}
			break;
		...
		case 0xFD:
			if (eModel == M51BX)
				(void)NVMEResetLog(fd, aVendorLogs[i].ucLogPage,
						   aVendorLogs[i].nLogSize,
						   aVendorLogs[i].nMaxSize);
		default:
			...
			while (err == 0 && maxSize > 0 &&
			       ((unsigned int *)dataBuffer)[0] != 0xdeadbeef) {
				sprintf(msg, "log 0x%x",
					aVendorLogs[i].ucLogPage);
				WriteData(dataBuffer, bSize, strCtrlDirName,
					  aVendorLogs[i].strFileName, msg);
				err = nvme_get_log_simple(
				    fd, aVendorLogs[i].ucLogPage, bSize,
				    dataBuffer);
				if (err || (((unsigned int *)dataBuffer)[0] ==
					    0xdeadbeef))
					break;
				maxSize -= bSize;
			}
