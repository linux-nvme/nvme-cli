				...
				struct command *cmd, struct plugin *plugin)
{
	...
	if (!opt.telemetry_log) {
		nvme_show_result("\nTelemetry log binary must be specified.\n");
		return -1;
	} else if (!opt.string_log) {
		nvme_show_result("\nString log C9 is not specified. String data will be missing.\n");
	} else if (!opt.output_fmt) {
		nvme_show_result("\n Output format is not specified. Using default format - JSON.\n");
	}
