...
static int ZipAndRemoveDir(char *strDirName, char *strFileName)
{
    ...
    sprintf(strBuffer, "zip -r \"%s\" \"%s\" >temp.txt 2>&1", strFileName, strDirName);
...
static int SetupDebugDataDirectories(char * strSN, char *strFilePath,
    char *strMainDirName, char *strOSDirName, char *strCtrlDirName)
{
    int err = 0;
    char strAppend[250];
    struct stat st;
...
static int NVMEResetLog(int nFD, unsigned char ucLogID, int nBufferSize, long long llMaxSize)
{
...
static int micron_pcie_stats(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
    ...
    {
        devicename = strrchr(argv[optind],'/');
        sprintf(devicename, "%s%s", devicename,"n1");
    }
    else
    {
    ...
    sprintf(command, "setpci -s %x:%x.%x ECAP_AER+10.L",bus,device,function);
    fp = popen(command, "r");
    if(fp == NULL)
    {
    ...
    sprintf(command, "setpci -s %x:%x.%x ECAP_AER+0x4.L",bus,device,function);
    fp = popen(command, "r");
    if(fp == NULL)
    {
        printf("Unable to retrieve error count\n");
...
static int micron_clear_pcie_correctable_errors(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
    int err = 0, bus = 0, domain = 0, device = 0, function = 0;
    char strTempFile[1024], strTempFile2[1024], command[1024];
    char *businfo = NULL;
    ...
    businfo = strrchr(strTempFile2,'/');
    sscanf(businfo,"/%x:%x:%x.%x", &domain, &bus, &device, &function);
    sprintf(command, "setpci -s %x:%x.%x ECAP_AER+0x10.L=0xffffffff",bus,device,function);
    ...
    sprintf(command, "setpci -s %x:%x.%x ECAP_AER+0x10.L",bus,device,function);
    fp = popen(command, "r");
    if(fp == NULL)
    {
        printf("Unable to retrieve error count\n");
...
static void GetDriveInfo(const char *strOSDirName, int nFD, struct nvme_id_ctrl *ctrlp)
{
    FILE *fpOutFile = NULL; 
    char tempFile[256] = { 0 };
...
static void GetCtrlIDDInfo(const char *strCtrlDirName, struct nvme_id_ctrl *ctrlp)
{
    char tempFolder[256] = { 0 };
    FILE *fpOutFile;
    sprintf(tempFolder,"%s/%s",strCtrlDirName, "nvme_controller_identify_data.bin");
    fpOutFile = fopen(tempFolder,"wb");
...
static void GetErrorlogData(int fd, int entries, const char *strCtrlDirName)
{
    ...
    {
        sprintf(tempFolder,"%s/%s",strCtrlDirName, "error_information_log.bin");
        fpOutFile = fopen(tempFolder,"wb");
...
static int micron_internal_logs(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
    ...
    int ctrlIdx;
    FILE *fpOutFile = NULL;
    char strOSDirName[256];
    char strCtrlDirName[256];
    ...
    for(int i = 0; i < (int)(sizeof(aVendorLogs) / sizeof(aVendorLogs[0])) && aVendorLogs[i].ucLogPage != 0; i++)
    {
        ...
        switch(aVendorLogs[i].ucLogPage)
        {
            ...
            case 0xE7:
                ...
                if(uiMask == 0 || (aVendorLogs[i].ucLogPage == 0xE6 && uiMask == 2) || (aVendorLogs[i].ucLogPage == 0xE7 && uiMask == 1))
                {
                    bSize = 0;
                }
                else
             ...
             default:
                ...
                {
                    sprintf(tempFolder,"%s/%s",strCtrlDirName,aVendorLogs[i].strFileName);
