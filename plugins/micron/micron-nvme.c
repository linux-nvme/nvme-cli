...
static int GetLogPageSize(int nFD, unsigned char ucLogID, int *nLogSize)
{
	...
	if (ucLogID == 0xC1 || ucLogID == 0xC2 || ucLogID == 0xC4) {
		err = nvme_get_log_simple(nFD, ucLogID, CommonChunkSize,
					  pTmpBuf);
		if (err == 0) {
			...
			*nLogSize =
				(int)(pLogHeader1->numDwordsInEntireLogPage) *
				4;
				     ...
				     struct plugin *plugin)
{
	...
	OPT_ARGS(opts) = { OPT_STRING("fw", 'f', "FILE", &cfg.fw, fw),
			   OPT_STRING("select", 's', "flag", &cfg.select,
				      select),
			   OPT_END() };
			       ...
			       struct plugin *plugin)
{
	...
	OPT_ARGS(opts) = { OPT_STRING("option", 'o', "option", &opt.option,
				      option),
	...
	if (!err) {
		...
		if (is_json) {
			...
			json_object_add_value_array(
				root, "Micron temperature information",
				logPages);
			sprintf(tempstr, "%u C", temperature);
			json_object_add_value_string(
				stats, "Current Composite Temperature",
				tempstr);
			for (i = 0; i < SensorCount; i++) {
...
)
{
	...
	for (field = 0; field < field_count; field++) {
		char datastr[1024] = { 0 };
		char *sfield = NULL;
		int size = (spec == 0) ? log_page[field].size :
					       log_page[field].size2;
		if (size == 0)
			continue;
...
static void GetGenericLogs(int fd, const char *dir)
{
	...
	err = nvme_get_log_persistent_event(fd,
...
static int GetTelemetryData(int fd, const char *dir)
{
	...
	for (i = 0; i < (int)(sizeof(tmap) / sizeof(tmap[0])); i++) {
		err = micron_telemetry_log(fd, tmap[i].log, &buffer, &logSize,
					   0);
				     ...
				     struct json_object *stats)
{
	...
	timestamp = (le64_to_cpu(entry->power_on_hour) & 0x0000FFFFFFFFFFFFUL) /
		    1000;
