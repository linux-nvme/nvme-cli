				...
				struct nvme_ocp_telemetry_offsets *ptelemetry_das_offset)
{
	...
	ptelemetry_das_offset->da2_size =
			(ptelemetry_common_header->da2_last_block - ptelemetry_common_header->da1_last_block) *
			OCP_TELEMETRY_DATA_BLOCK_SIZE;

	ptelemetry_das_offset->da3_start_offset = ptelemetry_das_offset->da2_start_offset +
					ptelemetry_das_offset->da2_size;
	ptelemetry_das_offset->da3_size =
			(ptelemetry_common_header->da3_last_block - ptelemetry_common_header->da2_last_block) *
			OCP_TELEMETRY_DATA_BLOCK_SIZE;

	ptelemetry_das_offset->da4_start_offset = ptelemetry_das_offset->da3_start_offset +
					ptelemetry_das_offset->da3_size;
	ptelemetry_das_offset->da4_size =
			(ptelemetry_common_header->da4_last_block - ptelemetry_common_header->da3_last_block) *
			OCP_TELEMETRY_DATA_BLOCK_SIZE;
...
int parse_ocp_telemetry_string_log(int event_fifo_num, int identifier, int debug_event_class,
								   enum ocp_telemetry_string_tables string_table,
								   char *description)
{
	...
	int vu_ests_entries = (int)vuests_table_size / sizeof(struct nvme_ocp_vu_event_string_table);

	if (string_table == STATISTICS_IDENTIFIER_STRING) {
		for (int sits_entry = 0; sits_entry < sits_entries; sits_entry++) {
			struct nvme_ocp_statistics_identifier_string_table *peach_statistic_entry = NULL;

			peach_statistic_entry = (struct nvme_ocp_statistics_identifier_string_table *)
			...
			if (identifier == (int)peach_statistic_entry->vs_statistic_identifier) {
				char *pdescription = (char *)(pstring_buffer +
								...
								(peach_statistic_entry->ascii_id_offset * SIZE_OF_DWORD));
				...
				if ((description == NULL) && (identifier < 0x1D)) {
					memcpy(description, statistic_identifiers_map[identifier].description,
		...
		for (int ests_entry = 0; ests_entry < ests_entries; ests_entry++) {
			struct nvme_ocp_event_string_table *peach_event_entry = NULL;

			peach_event_entry = (struct nvme_ocp_event_string_table *)(pstring_buffer +
							(pocp_ts_header->ests * SIZE_OF_DWORD) +
		...
		for (int vu_ests_entry = 0; vu_ests_entry < vu_ests_entries; vu_ests_entry++) {
			struct nvme_ocp_vu_event_string_table *peach_vu_event_entry = NULL;

			peach_vu_event_entry = (struct nvme_ocp_vu_event_string_table *)(pstring_buffer +
							(pocp_ts_header->vu_ests * SIZE_OF_DWORD) +
							(vu_ests_entry * sizeof(struct nvme_ocp_vu_event_string_table)));

			if (identifier == (int)peach_vu_event_entry->vu_event_identifier &&
							debug_event_class == (int)peach_vu_event_entry->debug_event_class) {
				char *pdescription = (char *)(pstring_buffer +
								(pocp_ts_header->ascts * SIZE_OF_DWORD) +
...
int parse_event_fifo(unsigned int fifo_num, unsigned char *pfifo_start,
					 ...
					 unsigned char *pstring_buffer, struct nvme_ocp_telemetry_offsets *poffsets,
					 __u64 fifo_size, FILE *fp)
{
	...
	int event_des_size = sizeof(struct nvme_ocp_telemetry_event_descriptor);
	...
	while ((fifo_size > 0) && (offset_to_move < fifo_size)) {
		...
		if (pevent_descriptor->event_data_size >= 0) {
			...
			parse_ocp_telemetry_string_log(0, pevent_descriptor->event_id,
										   ...
										   EVENT_STRING, description_str);

			if (pevent_fifos_object != NULL) {
				JSON_ADD_FORMATTED_UINT32_STR(pevent_descriptor_obj, STR_DBG_EVENT_CLASS_TYPE,
											  pevent_descriptor->debug_event_class_type);
				JSON_ADD_FORMATTED_UINT32_STR(pevent_descriptor_obj, STR_EVENT_IDENTIFIER,
											  pevent_descriptor->event_id);
				JSON_ADD_STR(pevent_descriptor_obj, STR_EVENT_STRING, description_str);
				JSON_ADD_FORMATTED_UINT32_STR(pevent_descriptor_obj, STR_EVENT_DATA_SIZE,
											  pevent_descriptor->event_data_size);

				if (pevent_descriptor->debug_event_class_type >= 0x80)
					JSON_ADD_FORMATTED_VAR_SIZE_STR(pevent_descriptor_obj, STR_VU_DATA,
			...
			case TIME_STAMP_CLASS_TYPE: {
				struct nvme_ocp_time_stamp_dbg_evt_class_format *ptime_stamp_event =
							(struct nvme_ocp_time_stamp_dbg_evt_class_format *)
				...
				if (pevent_fifos_object != NULL) {
					JSON_ADD_FORMATTED_VAR_SIZE_STR(pevent_descriptor_obj,
													...
													ptime_stamp_event->time_stamp,
													DATA_SIZE_8);
					JSON_ADD_FORMATTED_UINT32_STR(pevent_descriptor_obj,
			...
			case PCIE_CLASS_TYPE: {
				...
				if (pevent_fifos_object != NULL) {
					JSON_ADD_FORMATTED_VAR_SIZE_STR(pevent_descriptor_obj,
													STR_CLASS_SPECIFIC_DATA,
													ppcie_event->pCIeDebugEventData,
													DATA_SIZE_4);
					JSON_ADD_FORMATTED_UINT32_STR(pevent_descriptor_obj,
												  STR_VU_EVENT_ID_STRING, vu_event_id);
					JSON_ADD_STR(pevent_descriptor_obj, STR_VU_EVENT_STRING, description_str);
			...
			case NVME_CLASS_TYPE: {
				...
				if (pevent_fifos_object != NULL) {
					JSON_ADD_FORMATTED_VAR_SIZE_STR(pevent_descriptor_obj,
													STR_CLASS_SPECIFIC_DATA,
													pnvme_event->nvmeDebugEventData,
													DATA_SIZE_8);
					JSON_ADD_FORMATTED_UINT32_STR(pevent_descriptor_obj,
			...
			case MEDIA_CLASS_TYPE: {
				struct nvme_ocp_common_dbg_evt_class_format *pcommon_debug_event =
								(struct nvme_ocp_common_dbg_evt_class_format *)
								pevent_specific_data;
			...
			case MEDIA_WEAR_CLASS_TYPE: {
				struct nvme_ocp_media_wear_dbg_evt_class_format *pmedia_wear_event =
							(struct nvme_ocp_media_wear_dbg_evt_class_format *)
							pevent_specific_data;
				...
				if (pevent_fifos_object != NULL) {
					JSON_ADD_FORMATTED_VAR_SIZE_STR(pevent_descriptor_obj,
						STR_CLASS_SPECIFIC_DATA, pmedia_wear_event->currentMediaWear,
						DATA_SIZE_12);
					JSON_ADD_FORMATTED_UINT32_STR(pevent_descriptor_obj,
												  STR_VU_EVENT_ID_STRING, vu_event_id);
					JSON_ADD_STR(pevent_descriptor_obj, STR_VU_EVENT_STRING, description_str);
			...
			case STATISTIC_SNAPSHOT_CLASS_TYPE: {
				struct nvme_ocp_statistic_snapshot_evt_class_format *pStaticSnapshotEvent =
			...
			default: {
				//Reserved 7Fh-0Bh and Vendor Unique FFh-80h Classes will fall here,
				//Nothing to Parse
			}
			break;
		}
					...
					struct json_object *pstats_array, FILE *fp)
{
	...
	if (pstats_array != NULL) {
		...
		JSON_ADD_FORMATTED_UINT32_STR(pstatistics_object, STR_STATISTICS_IDENTIFIER,
									  pstatistic_entry->statistic_id);
		JSON_ADD_STR(pstatistics_object, STR_STATISTICS_IDENTIFIER_STR, description_str);
		JSON_ADD_FORMATTED_UINT32_STR(pstatistics_object, STR_STATISTICS_INFO_BEHAVIOUR_TYPE,
									  pstatistic_entry->statistic_info_behaviour_type);
		JSON_ADD_FORMATTED_UINT32_STR(pstatistics_object, STR_STATISTICS_INFO_RESERVED,
									  pstatistic_entry->statistic_info_reserved);
		JSON_ADD_FORMATTED_UINT32_STR(pstatistics_object, STR_NAMESPACE_IDENTIFIER,
									  pstatistic_entry->ns_info_nsid);
		JSON_ADD_FORMATTED_UINT32_STR(pstatistics_object, STR_NAMESPACE_INFO_VALID,
									  pstatistic_entry->ns_info_ns_info_valid);
		JSON_ADD_FORMATTED_UINT32_STR(pstatistics_object, STR_STATISTICS_DATA_SIZE,
									  pstatistic_entry->statistic_data_size);
		JSON_ADD_FORMATTED_UINT32_STR(pstatistics_object, STR_RESERVED,
	...
	if (output_file != NULL) {
		...
		if (fp) {
			...
			__u8 *pda1_smart_offset = pda1_header_offset + offsetof(struct nvme_ocp_header_in_da1,
																smart_health_info);//512+512 =1024
			...
			__u8 *pda1_smart_ext_offset = pda1_header_offset +
				offsetof(struct nvme_ocp_header_in_da1, smart_health_info_extended);//512+1024 =1536
		...
		__u8 *pda1_smart_ext_offset = pda1_header_offset + offsetof(struct nvme_ocp_header_in_da1,
												smart_health_info_extended);//512+1024 =1536
