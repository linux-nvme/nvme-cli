...
static int nvme_sct_op(struct dev_handle *hnd,  __u32 opcode, __u32 cdw10, __u32 cdw11, void* data, __u32 data_len )
{
	void  *metadata = NULL;
	const __u32 cdw2 = 0;
	const __u32 cdw3 = 0;
	...
	__u32 result;
	err = nvme_admin_passthru(hnd, opcode, flags, rsvd,
...
static int nvme_get_sct_status(struct dev_handle *hnd, __u32 device_mask)
{
	int err;
	void* data = NULL;
	size_t data_len = 512;
...
static int nvme_sct_command_transfer_log(struct dev_handle *hnd, bool current)
{
	int err;
	void *data = NULL;
	size_t data_len = 512;
	...
	err = nvme_sct_op(hnd, OP_SCT_COMMAND_TRANSFER, DW10_SCT_COMMAND_TRANSFER, DW11_SCT_COMMAND_TRANSFER, data, data_len);
	free(data);
	return err;
}

static int nvme_sct_data_transfer(struct dev_handle *hnd, void* data, size_t data_len, size_t offset)
{
	...
	dw11 = (offset >> 16);
	return nvme_sct_op(hnd, OP_SCT_DATA_TRANSFER, dw10, dw11, data, data_len);
}
...
static int nvme_get_internal_log(struct dev_handle *hnd, const char* const filename, bool current)
{
	int err;
	int o_fd = -1;
	void* page_data = NULL;
	...
	err = nvme_sct_command_transfer_log(hnd, current);
	if (err) {
	...
	err = nvme_sct_data_transfer(hnd, page_data, page_data_len, 0);
	if (err) {
	...
	for (i = 1; i < pages;) {
		...
		err = nvme_sct_data_transfer(hnd, page_data,
	...
	progress_runner(progress);
	fprintf(stdout,"\n");
	err = nvme_get_sct_status(hnd, MASK_IGNORE);
	if (err) {
...
static int nvme_get_internal_log_file(struct dev_handle *hnd, const char* const filename, bool current)
{
	int err;

	/* Check device supported */
	err = nvme_get_sct_status(hnd, MASK_0 | MASK_1);
	if (!err)
		err = nvme_get_internal_log(hnd, filename, current);
	return err;
}
