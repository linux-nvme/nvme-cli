...
static void get_ymtc_smart_info(struct nvme_ymtc_smart_log *smart, int index,
				u8 *nm_val, u8 *raw_val)
{
			       ...
			       struct nvme_ymtc_smart_log *smart)
{
	...
	printf("thermal_throttle_status         : %3d%%       %" PRIu64
	       "%%, cnt: %" PRIu64 "\n",
	       *nm, int48_to_long(raw), int48_to_long(raw + 1));
				    ...
				    struct plugin *plugin)
{
	...
	int err, fd;
	char *desc =
	    "Get Ymtc vendor specific additional smart log (optionally, "
	...
	if (!err) {
		if (!cfg.raw_binary)
			err = show_ymtc_smart_log(fd, cfg.namespace_id,
