...
static void vt_convert_smart_data_to_human_readable_format(struct vtview_smart_log_entry *smart, char *text)
{
	...
	strcat(text, tempbuff);
	snprintf(tempbuff, sizeof(tempbuff), "Data_Units_Read;%s;", uint128_to_string(le128_to_cpu(smart->raw_smart.data_units_read)));
	strcat(text, tempbuff);
	snprintf(tempbuff, sizeof(tempbuff), "Data_Units_Written;%s;", uint128_to_string(le128_to_cpu(smart->raw_smart.data_units_written)));
	strcat(text, tempbuff);
	snprintf(tempbuff, sizeof(tempbuff), "Host_Read_Commands;%s;", uint128_to_string(le128_to_cpu(smart->raw_smart.host_reads)));
	strcat(text, tempbuff);
	snprintf(tempbuff, sizeof(tempbuff), "Host_Write_Commands;%s;", uint128_to_string(le128_to_cpu(smart->raw_smart.host_writes)));
	strcat(text, tempbuff);
	snprintf(tempbuff, sizeof(tempbuff), "Controller_Busy_Time;%s;", uint128_to_string(le128_to_cpu(smart->raw_smart.ctrl_busy_time)));
	strcat(text, tempbuff);
	snprintf(tempbuff, sizeof(tempbuff), "Power_Cycles;%s;", uint128_to_string(le128_to_cpu(smart->raw_smart.power_cycles)));
	strcat(text, tempbuff);
	snprintf(tempbuff, sizeof(tempbuff), "Power_On_Hours;%s;", uint128_to_string(le128_to_cpu(smart->raw_smart.power_on_hours)));
	strcat(text, tempbuff);
	snprintf(tempbuff, sizeof(tempbuff), "Unsafe_Shutdowns;%s;", uint128_to_string(le128_to_cpu(smart->raw_smart.unsafe_shutdowns)));
	strcat(text, tempbuff);
	snprintf(tempbuff, sizeof(tempbuff), "Media_Errors;%s;", uint128_to_string(le128_to_cpu(smart->raw_smart.media_errors)));
	strcat(text, tempbuff);
	snprintf(tempbuff, sizeof(tempbuff), "Num_Err_Log_Entries;%s;", uint128_to_string(le128_to_cpu(smart->raw_smart.num_err_log_entries)));
	strcat(text, tempbuff);
