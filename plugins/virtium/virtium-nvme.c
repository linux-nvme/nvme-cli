...
static void vt_build_power_state_descriptor(const struct nvme_id_ctrl *ctrl)
{
	...
	printf("%6s%10s%5s%4s%6s%10s%10s%10s%4s%4s%4s%4s%10s%4s%6s%10s%4s%5s%6s\n",
	       ...
	       "0fs 12-15", "RWL", "RWT", "RRL", "RRT", "0fs 16-19", "IPS",
	       "IDLP", "0fs 20-23", "APS", "APW", "ACTP");
				       ...
				       struct plugin *plugin)
{
	...
	char path[256] = "";
	char *desc =
		"Save SMART data into log file with format that is easy to analyze (comma delimited). Maximum log file will be 4K.\n\n"
		"Typical usages:\n\n"
	...
	const char *output_file =
		"(optional) Name of the log file (give it a name that easy for you to remember what the test is). You can leave it blank too, we will take care it for you.";
	const char *test_name =
		"(optional) Name of the test you are doing. We use this as part of the name of the log file.";
