...
static int get_status(int argc, char **argv, struct command *cmd,
		      struct plugin *plugin)
{
	...
	printf("cap_status                          : %s\n",
	       log.cap_status == 0
		   ? "Normal"
		   : (log.cap_status == 1 ? "Warning" : "Critical"));
	...
	printf("fw_loader_version                   : %.*s\n", 8,
	       log.fw_loader_version);
	printf("uefi_driver_version                 : %.*s\n", 8,
	       log.uefi_driver_version);

	if (log.pcie_volt_status <=
	    sizeof(volt_status) / sizeof(const char *)) {
