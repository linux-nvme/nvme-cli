			       ...
			       unsigned int nsid, const char *devname)
{
	...
	printf("erase_fail_count                : %3d%%       %" PRIu64 "\n",
	       smart->erase_fail_cnt.norm,
	       int48_to_long(smart->erase_fail_cnt.raw));
	printf("wear_leveling                   : %3d%%       min: %u, max: "
	       "%u, avg: %u\n",
	       smart->wear_leveling_cnt.norm,
 ...
 * @param table_size	buffer size (BYTES), should at least has 8 bytes for
 * mf_bb_count and grown_bb_count
 */
...
static int change_sanity_check(int fd, __u64 trg_in_4k, int *shrink)
{
	...
	if (trg_in_4k < provisoned_cap_4k ||
		...
		if (trg_in_4k < provisoned_cap_4k) {
			fprintf(stderr, "WARNING: The target capacity is less "
					"than 1.0 x provisioned capacity!\n");
		} else {
			fprintf(stderr,
				"WARNING: The target capacity is larger than "
				"4.0 x provisioned capacity!\n");
		}
		return -1;
	}
	...
	if (extend) {
		...
		if (s_info.freeram <= 10 || mem_need > s_info.freeram) {
			fprintf(stderr,
				...
				"Please drop cache or extend more memory and "
				"retry\n"
			   ...
			   struct plugin *plugin)
{
	...
	if (cfg.feature_id == SFX_FEAT_CLR_CARD) {
		...
		if (!cfg.force &&
		    !sfx_confirm_change("Going to clean device's data, confirm "
					"umount fs and try again")) {
			return 0;
		} else {
	...
	if (cfg.feature_id == SFX_FEAT_ATOMIC && cfg.value != 0) {
		if (cfg.namespace_id != 0xffffffff) {
			 ...
			 */
			if ((ns.flbas & 0xf) != 1) {
				printf("Please change-sector size to 4K, then "
				       "retry\n");
				return EFAULT;
			}
		}
		...
		if (!cfg.force &&
		    !sfx_confirm_change(
			"Changing physical capacity may irrevocably delete "
			"this device's data")) {
			return 0;
		}
	}
