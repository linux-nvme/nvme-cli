...
int sfx_nvme_get_log(struct libnvme_transport_handle *hdl, __u32 nsid, __u8 log_id, __u32 data_len, void *data)
{
	struct libnvme_passthru_cmd cmd = {
...
static int get_bb_table(struct libnvme_transport_handle *hdl, __u32 nsid, unsigned char *buf, __u64 size)
{
	if (libnvme_transport_handle_get_fd(hdl) < 0 || !buf || size != 256*4096*sizeof(unsigned char)) {
		nvme_show_error("Invalid Param \r");
		return -EINVAL;
	}
...
static int sfx_get_bad_block(int argc, char **argv, struct command *acmd, struct plugin *plugin)
{
	...
	if (!data_buf) {
		nvme_show_error("malloc fail, errno %d\r", errno);
		return -1;
	}
...
static int change_sanity_check(struct libnvme_transport_handle *hdl, __u64 trg_in_4k, int *shrink)
{
			    ...
			    char *file, bool parse, char *output)
{
	...
	lsp = lsp_base + NVME_PEVENT_LOG_RELEASE_CTX;
	log = pevent;
	len = sizeof(*pevent);
	nvme_init_get_log(&cmd, NVME_NSID_NONE, NVME_LOG_LID_PERSISTENT_EVENT,
	...
	lsp = lsp_base + NVME_PEVENT_LOG_EST_CTX_AND_READ;
	nvme_init_get_log(&cmd, NVME_NSID_NONE, NVME_LOG_LID_PERSISTENT_EVENT,
			  NVME_CSI_NVM, log, len);
	cmd.cdw10 |= NVME_FIELD_ENCODE(lsp,
				       NVME_LOG_CDW10_LSP_SHIFT,
	...
	lsp = lsp_base + NVME_PEVENT_LOG_READ;
	log = pevent_log_info;
	length = log_len;
	while (length > 0) {
		lpo = offset;
...
static int sfx_status(int argc, char **argv, struct command *acmd, struct plugin *plugin)
{
	...
	memset(&chr_dev, 0, 8);
	strcpy(chr_dev, libnvme_transport_handle_get_name(hdl));
	for (len = 2; len < 8; len++) {
		if (chr_dev[len] == 'n')
			chr_dev[len] = '\0';
	}
	...
	if (strncmp("0x0020", pci_did, 6) == 0)
		err = nvme_get_log_simple(hdl, SFX_LOG_EXTENDED_HEALTH_ALT, (void *)&sfx_smart,
								  sizeof(sfx_smart));
	else
	...
	if (flags & JSON || cfg.json) {
		root = json_create_object();
		json_object_add_value_string(root, "ScaleFlux Status", libnvme_transport_handle_get_name(hdl));
