...
static void amzn_print_detail_io(struct amzn_latency_log_page *log)
{
	...
	for (int i = 0; i < num_of_hists; i++) {
		...
		/* print io histogram for this size range. The bound is the same for all the ranges */
	...
	for (int i = 0; i < num_of_hists; i++) {
		...
		for (int op = AMZN_NVME_OP_READ; op < AMZN_NVME_OP_MAX; op++) {
			...
			for (int bin = 0; bin < num_of_bins; bin++) {
				...
				json_object_add_value_uint64(bin_object, "lower", latency_histogram->bins[bin].lower);
				json_object_add_value_uint64(bin_object, "upper", latency_histogram->bins[bin].upper);
				if (op == AMZN_NVME_OP_READ)
					json_object_add_value_uint64(bin_object, "count", log->detail_io.io_hist_array[i].read_io_histogram_counts.counts[bin]);
				else
					json_object_add_value_uint64(bin_object, "count", log->detail_io.io_hist_array[i].write_io_histogram_counts.counts[bin]);
		     ...
		     struct plugin *plugin)
{
	...
	if (!strncmp((char *)ctrl.mn, AMZN_NVME_LOCAL_STORAGE_PREFIX, strlen(AMZN_NVME_LOCAL_STORAGE_PREFIX)))
		args.len = sizeof(log);
	else
