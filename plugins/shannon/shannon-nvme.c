				   ...
				   unsigned int nsid, const char *devname)
{
	...
	printf("timed_workload_media_wear       : %3d%%       %.3f%%\n",
	       ...
	       ((float)int48_to_long(
		   smart->items[TIME_WORKLOAD_MEDIA_WEAR].item_val)) /
		   1024);
	...
	printf("thermal_throttle_status         : %3d%%       CurTTSta: %u%%, "
	       "TTCnt: %u\n",
				    ...
				    struct plugin *plugin)
{
	...
	int err, fd;
	char *desc =
	    "Get Shannon vendor specific additional smart log (optionally, "
...
static int get_additional_feature(int argc, char **argv, struct command *cmd,
				  struct plugin *plugin)
{
	const char *desc =
...
static int set_additional_feature(int argc, char **argv, struct command *cmd,
				  struct plugin *plugin)
{
	const char *desc =
	...
	if (buf) {
		...
		if (err < 0) {
			fprintf(stderr,
				"failed to read data buffer from input file\n");
			err = EINVAL;
			goto free;
		}
	}
...
static int shannon_id_ctrl(int argc, char **argv, struct command *cmd,
			   struct plugin *plugin)
{
