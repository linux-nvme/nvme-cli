...
static int huawei_get_nvme_info(int fd, struct huawei_list_item *item,
				const char *node)
{
	int err;
	int len;
...
static void format(char *formatter, size_t fmt_sz, char *tofmt, size_t tofmtsz)
{
	...
	fmt_sz = snprintf(formatter, fmt_sz, "%-*.*s", (int)tofmtsz,
			  (int)tofmtsz, tofmt);
	/* trim() the obnoxious trailing white lines */
	while (fmt_sz) {
		if (formatter[fmt_sz - 1] != ' ' &&
		    formatter[fmt_sz - 1] != '\0') {
			formatter[fmt_sz] = '\0';
			break;
		}
		fmt_sz--;
					 ...
					 unsigned len)
{
	...
	for (i = 0; i < len; i++) {
		...
		json_object_add_value_string(device_attrs, "DevicePath",
					     list_items[i].node);

		if (sscanf(list_items[i].node, "/dev/nvme%d", &index) == 1)
			json_object_add_value_int(device_attrs, "Index", index);

		format(formatter, sizeof(formatter), list_items[i].ns_name,
		       sizeof(list_items[i].ns_name));

		json_object_add_value_string(device_attrs, "NS Name",
					     formatter);

		format(formatter, sizeof(formatter), list_items[i].array_name,
		       sizeof(list_items[i].array_name));

		json_object_add_value_string(device_attrs, "Array Name",
					     formatter);
...
static void huawei_print_list_head(struct huawei_list_element_len element_len)
{
	...
	printf("%-*.*s %-*.*s %-*.*s %-*.*s %-*.*s %-*.*s\n", element_len.node,
	       element_len.node, "Node", element_len.ns_name,
	       element_len.ns_name, "NS Name", element_len.nguid,
	       element_len.nguid, "Nguid", element_len.ns_id, element_len.ns_id,
	       "NS ID", element_len.usage, element_len.usage, "Usage",
	       element_len.array_name, element_len.array_name, "Array Name");

	printf("%-.*s %-.*s %-.*s %-.*s %-.*s %-.*s\n", element_len.node, dash,
	       element_len.ns_name, dash, element_len.nguid, dash,
	       element_len.ns_id, dash, element_len.usage, dash,
	       element_len.array_name, dash);
}
...
{
	long long int lba =
	    1 << list_item.ns.lbaf[(list_item.ns.flbas & 0x0f)].ds;
	...
	printf("%-*.*s %-*.*s %-*.*s %-*d %-*.*s %-*.*s\n", element_len.node,
	       element_len.node, list_item.node, element_len.ns_name,
	       element_len.ns_name, list_item.ns_name, element_len.nguid,
	       element_len.nguid, nguid_buf, element_len.ns_id, list_item.nsid,
	       element_len.usage, element_len.usage, usage,
	       element_len.array_name, element_len.array_name,
	       list_item.array_name);
}

static unsigned int choose_len(unsigned int old_len, unsigned int cur_len,
...
static unsigned int huawei_get_ns_len(struct huawei_list_item *list_items,
				      unsigned len, unsigned default_len)
{
	int i;
	unsigned int min_len = default_len;

	for (i = 0; i < len; i++)
		min_len = choose_len(min_len, strlen(list_items->ns_name),
				     default_len);

	return min_len;
}

static int huawei_get_array_len(struct huawei_list_item *list_items,
				unsigned len, unsigned default_len)
{
	int i;
	...
	for (i = 0; i < len; i++)
		min_len = choose_len(min_len, strlen(list_items->array_name),
				     default_len);

	return min_len;
}

static void huawei_print_list_items(struct huawei_list_item *list_items,
				    unsigned len)
{
	unsigned i;
	struct huawei_list_element_len element_len;
	...
	element_len.ns_name =
	    huawei_get_ns_len(list_items, len, MIN_NS_NAME_LEN);
	element_len.array_name =
	    huawei_get_array_len(list_items, len, MIN_ARRAY_NAME_LEN);
		       ...
		       struct plugin *plugin)
{
	...
	int fmt;
	const char *desc =
	    "Retrieve basic information for the given huawei device";
	struct config {
		char *output_format;
	};
...
static void huawei_do_id_ctrl(__u8 *vs, struct json_object *root)
{
	...
	if (root)
		json_object_add_value_string(root, "array name",
					     strlen(array_name) > 1 ? array_name
								    : "NULL");
	else
		printf("array name : %s\n",
...
static int huawei_id_ctrl(int argc, char **argv, struct command *cmd,
			  struct plugin *plugin)
{
