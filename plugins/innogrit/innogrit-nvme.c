...
static int innogrit_smart_log_additional(int argc, char **argv, struct command *command, 
...
static int sort_eventlog_fn(const void *a, const void *b)
{
	...
	if (((struct eventlog_addindex *)a)->ms > ((struct eventlog_addindex *)b)->ms) {
		rc = 1;
	} else if (((struct eventlog_addindex *)a)->ms < ((struct eventlog_addindex *)b)->ms) {
		rc = -1;
	} else {
		if (((struct eventlog_addindex *)a)->iindex < ((struct eventlog_addindex *)b)->iindex)
			rc = -1;
		else
			rc = 1;
	}
...
static int nvme_vucmd(int fd, unsigned char opcode, unsigned int cdw12, unsigned int cdw13, 
	unsigned int cdw14, unsigned int cdw15, char *data, int data_len)
{
...
static int innogrit_vsc_geteventlog(int argc, char **argv, struct command *command, 
	struct plugin *plugin)
{
	time_t timep;
	struct tm *logtime;
