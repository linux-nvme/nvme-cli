...
/* refer to systemd.link(5) */
#define LINK_FILE_NAME_FORMAT "10-nbft-iface-%s.link"

static void write_network_link_file(struct nbft_info_hfi *hfi, char *table_filename, char *path)
{
	_cleanup_free_ char *bname = NULL;
	char *table_id;
	char *s;
	_cleanup_free_ char *ifname = NULL;
	_cleanup_free_ char *link_fname = NULL;
	FILE *f;

	bname = strdup(table_filename);
	table_id = basename(table_filename);
	if (!table_id || !strstr(table_id, "NBFT")) {
		fprintf(stderr, "Warning: unable to determine NBFT table suffix: %s\n",
			table_filename);
		return;
	}

	/* Proposed predictable interface naming (nbftXhY):
	 *  'NBFT'    --> 'nbft0'
	 *  'NBFT1'   --> 'nbft1'
	 *  'NBFT999' --> 'nbft999'
	 *  'NBFT0'   --> 'nbft00'
	 ...
	 *  'NBFT099' --> 'nbft0099'
	 */
	s = strstr(table_id, "NBFT") + 4;
	if (*s == '\0') {
		if (asprintf(&ifname, "nbft0h%d", hfi->index) < 0)
			return;
	} else if (*s == '0') {
		if (asprintf(&ifname, "nbft0%sh%d", s, hfi->index) < 0)
			return;
	} else {
		if (asprintf(&ifname, "nbft%sh%d", s, hfi->index) < 0)
			return;
	}

	/* write the file */
	if (asprintf(&link_fname, "%s/" LINK_FILE_NAME_FORMAT, path, ifname) < 0)
		return;
	...
	fprintf(f, "# Generated by nvme-cli nbft gen-udev-link-files\n");
	fprintf(f, "[Match]\n");
...
int gen_udev_link_files(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	const char *desc = "Generate udev network link files.";
	char *nbft_path = NBFT_SYSFS_PATH;
	unsigned int verbose = 0;
	int ret;
	struct list_head nbft_list;
	...
	/* TODO: find a way to avoid adding '<device>' option */
	ret = argconfig_parse(argc, argv, desc, opts);
	if (ret)
		return ret;
	...
	if (optind >= argc) {
		fprintf(stderr, "Fatal: missing target udev network link dir\n\n");
		fprintf(stderr, "Usage: nvme nbft gen-udev-link-files [OPTIONS] <link_dir>\n");
		return -1;
	}
	...
	if (!ret) {
		list_for_each(&nbft_list, entry, node) {
			for (hfi = entry->nbft->hfi_list; hfi && *hfi; hfi++)
				write_network_link_file(*hfi, entry->nbft->filename, argv[optind]);
		}
	}
	free_nbfts(&nbft_list);
