						  ...
						  struct plugin *plugin)
{
	...
	if (err)
		return err;
	err = nvme_get_log_simple(dev_hdl(dev), 0xc5,
...
static int vs_smart_log(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (!stx_is_jag_pan(modelNo)) {
		err = nvme_get_log_simple(dev_hdl(dev), 0xC4, sizeof(ExtdSMARTInfo), &ExtdSMARTInfo);
		if (!err) {
			if (strcmp(cfg.output_format, "json")) {
			...
			err = nvme_get_log_simple(dev_hdl(dev), 0xCF, sizeof(logPageCF), &logPageCF);
			if (!err) {
				if (strcmp(cfg.output_format, "json")) {
		...
		err = nvme_get_log_simple(dev_hdl(dev), 0xC0, sizeof(ehExtSmart), &ehExtSmart);
	...
	err = nvme_get_log_simple(dev_hdl(dev), 0xC4,
	...
	if (!err) {
		...
		err = nvme_get_log_simple(dev_hdl(dev), 0xCF,
...
static int temp_stats(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	cf_err = nvme_get_log_simple(dev_hdl(dev), 0xCF,
...
static int vs_pcie_error_log(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_get_log_simple(dev_hdl(dev), 0xCB,
...
static int stx_vs_fw_activate_history(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_get_log_simple(dev_hdl(dev), 0xC2, sizeof(fwActivHis), &fwActivHis);
	if (!err) {
		if (strcmp(cfg.output_format, "json"))
...
static int vs_clr_pcie_correctable_errs(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (!stx_is_jag_pan(modelNo)) {
		err = nvme_set_features_simple(dev_hdl(dev), 0xE1, 0, 0xCB, cfg.save, &result);
	} else {
		struct nvme_set_features_args args = {
			.hdl	    = dev_hdl(dev),
			.args_size  = sizeof(args),
...
static int get_host_tele(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_get_nsid_log(dev_hdl(dev), false, cfg.log_id,
				cfg.namespace_id,
...
static int get_ctrl_tele(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	log_id = 0x08;
	err = nvme_get_nsid_log(dev_hdl(dev), false, log_id, cfg.namespace_id,
				sizeof(tele_log), (void *)(&tele_log));
	if (!err) {
		maxBlk = tele_log.tele_data_area3;
		offset += 512;
