 ...
 *
 * Copyright (c) 2017-2018 Seagate Technology LLC and/or its Affiliates, All
 * Rights Reserved
 *
 * ******************************************************************************************
 *
 ...
 *
 * \file seagate-nvme.c
 * \brief This file defines the functions and macros to make building a nvme-cli
 * seagate plug-in.
 */
			  ...
			  struct plugin *plugin)
{
	...
	const char *desc = "Retrieve Seagate Supported Log-Page information "
...
static char *print_ext_smart_id(__u8 attrId)
{
	...
	case VS_ATTR_ID_MAX_LIFE_TEMPERATURE:
		return "Max lifetime temperature"; /*VS_ATTR_ID_MAX_LIFE_TEMPERATURE
						      for extended*/
		break;
...
static void print_smart_log(__u16 verNo, SmartVendorSpecific attr, int lastAttr)
{
	...
	if (lastAttr == 1) {
		...
		printf("%-15d", VS_ATTR_ID_LIFETIME_WRITES_TO_FLASH_MSB << 8 |
				    VS_ATTR_ID_LIFETIME_WRITES_TO_FLASH_LSB);
		...
		printf("%-15d", VS_ATTR_ID_LIFETIME_WRITES_FROM_HOST_MSB << 8 |
				    VS_ATTR_ID_LIFETIME_WRITES_FROM_HOST_LSB);
				 ...
				 EXTENDED_SMART_INFO_T *ExtdSMARTInfo)
{
	...
	for (index = 0; index < NUMBER_EXTENDED_SMART_ATTRIBUTES; index++) {
		...
		if (ExtdSMARTInfo->vendorData[index].AttributeNumber != 0) {
			json_object_add_value_string(
			    lbaf, "attribute_name",
			    print_ext_smart_id(ExtdSMARTInfo->vendorData[index]
						   .AttributeNumber));
			json_object_add_value_int(
			    lbaf, "attribute_id",
...
static void print_smart_log_CF(vendor_log_page_CF *pLogPageCF)
{
	...
	printf(" 0x%016" PRIx64 "%016" PRIx64 "",
	       le64_to_cpu(
		   pLogPageCF->AttrCF.DataUnitsWrittenToDramNamespace.MS__u64),
	       le64_to_cpu(
		...
		if (strcmp(cfg.output_format, "json")) {
			...
			for (index = 0;
				...
				print_smart_log(
				    ...
				    ExtdSMARTInfo.vendorData[index],
				    index ==
					(NUMBER_EXTENDED_SMART_ATTRIBUTES - 1));
		      ...
		      struct plugin *plugin)
{
	...
	int err, cf_err;
	int index;
	const char *desc = "Retrieve Seagate Temperature Stats information for "
			   "the given device ";
	const char *output_format = "output in binary format";
		...
		for (index = 0; index < NUMBER_EXTENDED_SMART_ATTRIBUTES;
			...
			if (ExtdSMARTInfo.vendorData[index].AttributeNumber ==
				...
				maxTemperature = smart_attribute_vs(
				    ...
				    ExtdSMARTInfo.vendorData[index]);
				maxTemperature =
				    maxTemperature ? maxTemperature - 273 : 0;
				if (strcmp(cfg.output_format, "json"))
					printf("%-20s : %d C\n",
					       "Highest Temperature",
	...
	cf_err =
	    nvme_get_log_simple(fd, 0xCF, sizeof(ExtdSMARTInfo), &logPageCF);
...
static void print_vs_pcie_error_log(pcie_error_log_page pcieErrorLog)
{
	...
	uncorrectPcieEc =
	    pcieErrorLog.FCProtocolErrCnt + pcieErrorLog.DllpProtocolErrCnt +
			     ...
			     struct plugin *plugin)
{
	...
	err =
	    nvme_get_log_simple(fd, 0xCB, sizeof(pcieErrorLog), &pcieErrorLog);
	if (!err) {
		if (strcmp(cfg.output_format, "json")) {
			 ...
			 struct plugin *plugin)
{
	...
	const char *log_specific =
	    ...
	    "0 - controller shall not update the Telemetry Host Initiated "
	    "Data.";
	...
	if (!err) {
		...
		if (!cfg.raw_binary) {
			...
			printf("Data Block 1 Last Block:%d Data Block 2 Last "
			       "Block:%d Data Block 3 Last Block:%d\n",
	...
	while (blkCnt < maxBlk) {
		...
		err = nvme_get_log(fd, cfg.log_id, cfg.namespace_id, offset, 0,
				   0, true, 0, 0, blksToGet * 512, (void *)log,
			 ...
			 struct plugin *plugin)
{
	...
	log_id = 0x08;
	err = nvme_get_nsid_log(fd, log_id, cfg.namespace_id, sizeof(tele_log),
				(void *)(&tele_log));
	if (!err) {
		maxBlk = tele_log.tele_data_area3;
		offset += 512;
			   ...
			   struct plugin *plugin)
{
	...
	if (!err) {
		...
		/*
		printf("Data Block 1 Last Block:%d Data Block 2 Last Block:%d
		Data Block 3 Last Block:%d\n", tele_log.tele_data_area1,
		tele_log.tele_data_area2, tele_log.tele_data_area3);
		*/
