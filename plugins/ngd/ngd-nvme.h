...
PLUGIN(NAME("ngd", "NGDSysytems vendor specific extensions", NVME_VERSION),
	COMMAND_LIST(
		...
		ENTRY("lat-stats", "Retrieve Ngd IO Latency Statistics log, show it", get_lat_stats_log)
		...
		ENTRY("market-name", "Retrieve Ngd Marketing Name log, show it", get_market_log)
		ENTRY("smart-log-add", "Retrieve Ngd SMART Log, show it", get_additional_smart_log)
		ENTRY("temp-stats", "Retrieve Ngd Temperature Statistics log, show it", get_temp_stats_log)
	)
);
