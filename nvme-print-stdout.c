...
static void stdout_persistent_event_log_fdp_events(struct nvme_feat_fdp_events_cdw11 *cdw11,
						   unsigned int cdw12, unsigned char *buf)
{
	unsigned int num = cdw11->noet;
...
static void pel_set_feature_event(void *pevent_log_info, __u32 offset)
{
	...
	if (fid == NVME_FEAT_FID_FDP_EVENTS) {
		...
		stdout_persistent_event_log_fdp_events((struct nvme_feat_fdp_events_cdw11 *)
						       &set_feat_event->cdw_mem[1], cdw12, mem_buf);
	} else {
