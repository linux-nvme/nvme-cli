...
def ps_command_options(cmd):
    """The ('--opt', '-s', ...) option list for one command, sorted."""
    words = []
    for o in sorted(command_options(cmd), key=lambda o: o["long"]):
...
def generate_powershell(model, out):
    ...
    all_cmds = sorted(
        [n for c in builtin_commands(model) for n in cmd_names(c)] +
        [p["name"] for p in plugins(model)] +
        BUILTIN_COMMANDS)
    out.write("\n$script:NvmeCommands = @(\n")
    for name in all_cmds:
        out.write(f"    '{ps_escape(name)}'\n")
    out.write(")\n")
    ...
    opts_entries = []
    for c in builtin_commands(model):
        opts = ps_command_options(c)
        for n in cmd_names(c):
            opts_entries.append((ps_escape(n), opts))
    for p in plugins(model):
        for c in p["commands"]:
            opts = ps_command_options(c)
            for n in cmd_names(c):
                opts_entries.append((ps_escape("%s %s" % (p["name"], n)), opts))
    for key, opts in sorted(opts_entries, key=lambda e: e[0]):
        out.write(f"    '{key}' = @({opts})\n")
    out.write("}\n")
    ...
    for c in builtin_commands(model):
        for o in command_options(c):
            if not opt_is_file(o):
                continue
            for n in cmd_names(c):
                file_entries.append(f"{ps_escape(n)} --{o['long']}")
                if o.get("short"):
                    file_entries.append(f"{ps_escape(n)} -{o['short']}")
    for p in plugins(model):
        for c in p["commands"]:
            for o in command_options(c):
                if not opt_is_file(o):
                    continue
                for n in cmd_names(c):
                    key = ps_escape("%s %s" % (p["name"], n))
                    file_entries.append(f"{key} --{o['long']}")
                    if o.get("short"):
                        file_entries.append(f"{key} -{o['short']}")
    for entry in sorted(file_entries):
        out.write(f"    '{entry}' = $true\n")
    out.write("}\n")
    ...
    for c in builtin_commands(model):
        for o in command_options(c):
            if not o.get("values"):
                continue
            vals = ", ".join("'%s'" % ps_escape(v) for v in o["values"])
            for n in cmd_names(c):
                val_entries.append((f"{ps_escape(n)} --{o['long']}", vals))
                if o.get("short"):
                    val_entries.append((f"{ps_escape(n)} -{o['short']}", vals))
    for p in plugins(model):
        for c in p["commands"]:
            for o in command_options(c):
                if not o.get("values"):
                ...
                for n in cmd_names(c):
                    key = ps_escape("%s %s" % (p["name"], n))
                    val_entries.append((f"{key} --{o['long']}", vals))
                    if o.get("short"):
                        val_entries.append((f"{key} -{o['short']}", vals))
    for key, vals in sorted(val_entries, key=lambda e: e[0]):
        out.write(f"    '{key}' = @({vals})\n")
    out.write("}\n")
...
def main():
    ...
    ap.add_argument("--powershell", metavar="FILE",
                    help="write PowerShell completion to FILE ('-' for stdout)")
    args = ap.parse_args()
